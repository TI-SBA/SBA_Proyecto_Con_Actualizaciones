...
cmConcPor = {
		...
		if($('#pageWrapper [child=conc]').length<=0){
			...
			$.post('cm/navg/conc',function(data){
				...
				$p.find('[name=cmConcPor]').click(function(){ cmConcPor.init(); }).find('ul').addClass('ui-state-highlight');
				$p.find('[name=cmConcAll]').click(function(){ cmConcAll.init(); });
				$p.find('[name=cmConcVen]').click(function(){ cmConcVen.init(); });
				$p.find('[name=cmConcRec]').click(function(){ cmConcRec.init(); });
			},'json');
		}
		K.initMode({
			mode: 'cm',
			action: 'cmConcPor',
			titleBar: { title: 'Concesiones Por Vencer'}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				...
				$("#mainPanel .gridHeader").find('li:eq(2),li:eq(3),li:eq(4)').click( function(){
					...
					cmConcRec.loadData({page: 1,url: 'cm/oper/listaconcpor',filter: $(this).attr("filter"),order: order});
				});
				cmOper.contextMenuOper({$this: $('[name=btnOperacion]')});
				/*$mainPanel.find('[name=btnAgregar]').click(function(){
					//cmOper.newOper($(this));
				}).button({icons: {primary: 'ui-icon-folder-bookmark'}});*/
				...
				$('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
			            $("#mainPanel .gridBody").empty();
			            cmConcRec.loadData({page: 1,url: 'cm/oper/listaconcpor'});
					}else{
			            $("#mainPanel .gridBody").empty();
			            cmConcRec.loadData({page: 1,url: 'cm/oper/listaconcpor',bus: $('.divSearch [name=buscar]').val(),ncol: col});
		...
		$.post(params.url, params, function(data){
			if ( data.paging.total_page_items > 0 ) {
				for (i=0; i < data.paging.total_page_items; i++) {
					...
					$li.eq(6).html( cmOper.statesCondicionConc[result.concesion.condicion].descr );
					$li.eq(7).html( ciHelper.dateFormat(result.concesion.fecven) );*/
...
define(
	['cm/oper','cm/concrec','cm/concall','cm/concven'],
	function(cmOper,cmConcRec,cmConcAll,cmConcVen){
		return cmConcPor;
	}
);