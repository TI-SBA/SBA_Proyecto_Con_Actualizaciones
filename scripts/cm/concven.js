...
cmConcVen = {
		...
		if($('#pageWrapper [child=conc]').length<=0){
			...
			$.post('cm/navg/conc',function(data){
				...
				$p.find('[name=cmConcVen]').click(function(){ cmConcVen.init(); }).find('ul').addClass('ui-state-highlight');
		...
		K.initMode({
			mode: 'cm',
			action: 'cmConcVen',
			titleBar: { title: 'Concesiones Vencidas'}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				...
				$("#mainPanel .gridHeader").find('li:eq(2),li:eq(3),li:eq(4)').click( function(){
					...
					cmConcRec.loadData({page: 1,url: 'cm/oper/listaconcven',filter: $(this).attr("filter"),order: order});
				});
				...
				$('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
			            $("#mainPanel .gridBody").empty();
			            cmConcRec.loadData({page: 1,url: 'cm/oper/listaconcven'});
					}else{
			            $("#mainPanel .gridBody").empty();
			            cmConcRec.loadData({page: 1,url: 'cm/oper/listaconcven',bus: $('.divSearch [name=buscar]').val(),ncol: col});
...
define(
	['cm/oper','cm/concrec','cm/concall','cm/concpor'],
	function(cmOper,cmConcRec,cmConcAll,cmConcPor){
		return cmConcVen;
	}
);