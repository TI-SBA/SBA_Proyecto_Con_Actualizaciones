	...
	init: function(){
		if($('#pageWrapper [child=comp]').length<=0){
			...
			$.post('cm/navg/comp',function(data){
				...
				$p.find('[name=cmCompAll]').click(function(){ cmCompAll.init(); });
				$p.find('[name=cmCompPen]').click(function(){ cmCompPen.init(); }).addClass('ui-state-highlight');
			},'json');
		}
		K.initMode({
			mode: 'cm',
			action: 'cmCompPen',
			titleBar: { title: 'Comprobantes Pendientes'}
		});
			...
			onContentLoaded: function(){
				...
				$('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
			            $("#mainPanel .gridBody").empty();
			            cmCompPen.loadData({page: 1,url: 'cm/comp/lista_pen'});
					}else{
			            $("#mainPanel .gridBody").empty();
			            cmCompPen.loadData({page: 1,url: 'cm/comp/lista',bus: $('.divSearch [name=buscar]').val(),ncol: col});
