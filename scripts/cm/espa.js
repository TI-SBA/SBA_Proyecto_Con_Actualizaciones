...
cmEspa = {
	...
	init: function(){
		...
		new K.Panel({
			onContentLoaded: function(){
				var $grid = new K.grid({
					cols: [{n:'',f:'estado'},'',{n:'Tipo',f:'tipo'},{n:'Nombre',f:'nomb'},'Ocupantes',{n:'Sector',f:'sector'}],
					width: [10,40,200,300,100,120,100],
					data: 'cm/espa/search',
					params: {},
					itemdescr: 'espacio(s)',
					toolbarHTML: '<table><tr><td style="width:350px;">'+
						'<button name="btnAgregarNic">Agregar Nicho</button>'+
						'<button name="btnAgregarMau">Agregar Mausoleo</button>'+
						'<button name="btnAgregarTum">Agregar Tumba</button>'+
						'<button name="btnAgregarLot">Agregar Lote de Nichos</button></td><td>'+
						'<p>Tipo de Espacio: <select name="tipo">'+
							'<option value="">--</option>'+
						...
						'Piso: <input type="text" class="form-control" size="6" name="piso" /><button></button></p></td></tr></table>',
					onContentLoaded: function($el){
						...
						$el.find('[name=btnAgregarLot]').click(function(){
							cmEspa.windowLote();
						}).button({icons: {primary: 'ui-icon-plusthick'}});
						...
						$el.find('button:eq(4),button:eq(5),button:eq(6)').click(function(){
							var params = {
					    		sector: $el.find('[name=sector] option:selected').val(),
					    		tipo: $el.find('[name=tipo] option:selected').val(),
					    		num: $el.find('[name=num]').val(),
					...
					fill: function(data,$row){
						...
						if(data.estado=='D'&&data.historia==true)
							$row.find('td:last').css('background',cmEspa.estados['R'].color).addClass('vtip').attr('title',cmEspa.estados['R'].descr);
						...
						$row.data('id',data._id.$id).data('data',data).dblclick(function(){
							...
							onShowMenu: function(e, menu) {
								...
								if(K.tmp.data('estado')=='D'){
									$('#conMenCmEspa_cob,#conMenCmEspa_ope',menu).remove();
								}else{
									$('#conMenCmEspa_con,#conMenCmEspa_reg',menu).remove();
								}
							...
							bindings: {
								...
								'conMenCmEspa_ope': function(t) {
									cmOper.newOper({data: K.tmp.data('data')});
								},
								'conMenCmEspa_con': function(t) {
									cmOper.windowNewConcesion({espacio: K.tmp.data('data')});
								},
								'conMenCmEspa_reg': function(t) {
									cmOper.windowRegiOcup({espacio: K.tmp.data('data')});
								},
								'conMenCmEspa_cob': function(t) {
									cmEspa.windowNewServ({espacio: K.tmp.data('data')});
								},
								'conMenCmEspa_his': function(t){
									cmOper.windowNewHist({id: K.tmp.data('id')});
								},
			...
			buttons: {
				"Guardar": function(){
					...
					$.post("cm/espa/save",data,function(rpta){
						K.clearNoti();
						if(rpta.error!=null){
							p.$w.dialog('widget').find('.ui-dialog-buttonpane button').button('enable',true);
							K.notification({
								title: ciHelper.titleMessages.infoReq,
								text: "Este nicho ya fue agregado anteriormente!",
								type: 'error'
							});
						}else{
			...
			buttons: {
				"Guardar": function(){
					...
					data.mausoleo = {
						...
						medidas: {
							medida_total: p.$w.find('[name=medida_total]').val(),
		...
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				$.post('cm/espa/get',{_id: p.id},function(data){
					...
					if(data.mausoleo.medidas.medida_total!=null)
						p.$w.find('[name=medida_total]').val(data.mausoleo.medidas.medida_total);
					...
					if(data!=null){
						...
						for(var i=0; i<data.opers.length; i++){
							...
							if(result.anulacion!=null)
								$row.find('li:eq(2)').html( '<span style="color:red;">Anulada</span>'+'<br />'+mgEnti.formatName(result.trabajador) );
							else if(result.ejecucion!=null)
								$row.find('li:eq(2)').html( 'Ejecutado'+'<br />'+mgEnti.formatName(result.trabajador) );
							else
								$row.find('li:eq(2)').html( 'Activa'+'<br />'+mgEnti.formatName(result.trabajador) );
							if(result.recibos==null)
								$row.find('li:eq(3)').html( ciHelper.dateFormatLong(result.fecreg) );
							else{
								if(result.recibos[0] != null){
									$row.find('li:eq(3)').html( ciHelper.dateFormatLong(result.fecreg)+'<br />'
	...
	windowDetailsNicho: function(p){
		var params = {
			...
			onContentLoaded: function(){
				...
				p.$w.find('[name=spTrab]').html( mgEnti.formatName(p.data.trabajador) );
	...
	windowNewServ: function(p){
		...
		new K.Modal({
			id: 'windowInNewCuen',
			title: 'Registrar Cobro de Servicio Administrativo',
			...
			onContentLoaded: function(){
				p.$w = $('#windowInNewCuen');
				K.block({$element: p.$w});
				p.$w.find('[name=fecven]').closest('tr').hide();
				...
				p.$w.find('[name=fecven]').val(K.date())
					.datepicker();
				if(p.espacio!=null){
					$.post('cm/oper/get_conce','espacio='+p.espacio._id.$id,function(data){
						p.data = data;
	...
	windowLote: function(p){
		...
		new K.Window({ 
			...
			buttons: {
				"Guardar": function(){
					...
					var data = {
						...
						nicho: {
							fila: p.$w.find('[name=nomb]').val(),
							num_ini: p.$w.find('[name=num_ini]').val(),
							num_fin: p.$w.find('[name=num_fin]').val(),
							tipo: p.$w.find('[name=rbtn_tipo]:checked').val(),
							piso: p.$w.find('[name=pisos]').val(),
							pabellon: {}
						},
					...
					data.nicho.pabellon.num = tmp.num;
					data.sector = tmp.sector;
					data.nomb = data.nicho.pabellon.nomb+", "+data.nicho.pabellon.num+", Piso "+data.nicho.piso+", Fila "+data.nicho.fila+", Numero ";
					if(data.costo == ""){
						/*p.$w.find('[name=precio]').focus();
						return K.notification({title: ciHelper.titleMessages.infoReq,text: "Ingrese costo del nicho!",type:"error"});*/
					}
					...
					K.sendingInfo();
					p.$w.dialog('widget').find('.ui-dialog-buttonpane button').button('disable');
					$.post("cm/espa/save_lote",data,function(){
						K.notification({title: ciHelper.titleMessages.regiGua,text: "Nicho agregado con e&eacute;xito!"});
						K.closeWindow(p.$w.attr('id'));
						$('#pageWrapperLeft .ui-state-highlight').click();
					});
			...
			onContentLoaded: function(){
				...
				p.$w.find('[name=num]').attr('name','num_ini');
				p.$w.find('label:eq(3)').html('N&uacute;mero de Inicio');
				p.$w.find('[name=num_ini]').closest('tr').after('<tr>'+
						'<td>N&uacute;mero de Fin</td>'+
						'<td><input type="text" size="37" name="num_fin" /></td>'+
					'</tr>');
			}
		});
	}
...
define(
	['cm/pabe','cm/mapa','cm/ocup','cm/prop','cm/oper','cm/rehi'],
	function(cmPabe,cmMapa,cmOcup,cmProp,cmOper,cmRehi){
		return cmEspa;
	}
);