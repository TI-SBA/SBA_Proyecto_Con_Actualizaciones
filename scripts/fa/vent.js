		...
		$.extend(p,{
			...
			calcTot: function(){
				...
				for(var i=0,j=p.$w.find('[name=gridServ] tbody [name^=conc]').length; i<j; i++){
					var monto = parseFloat(p.$w.find('[name=gridServ] [name^=conc]').eq(i).data('precio'));
			...
			save: function(){
				...
				for(var i=0,j=p.$w.find('[name=gridServ] tbody [name^=conc]').length; i<j; i++){
					...
					item = {
						producto: faProd.dbRel($row.data('data')),
						monto: $row.data('monto'),
				...
				$.post('fa/comp/save_comp',data,function(comp){
					if(comp.error!=null){
					...
					/*K.windowPrint({
						...
						url: "fa/comp/print?print=1&_id="+comp._id.$id
					});*/
				},'json');
			}
		});
		new K.Panel({
			contentURL: 'fa/comp/venta',
			store: false,
			buttons: {
				'Generar Comprobante': {
					icon: 'fa-save',
			...
			onContentLoaded: function(){
							...
							faProd.windowSelect({
								...
								callback: function(data){
									if(p.$w.find('#'+data._id.$id).length==0){
										...
										$row.append('<td><h3><u>'+data.nomb+'</u></h3> <kbd>'+ciHelper.formatMon(data.precio_venta)+'</kbd><br />'+
											'<label class="label label-primary">Nombre Gen&eacute;rico: '+data.generico+'</label>'+'</td>');
										$row.append('<td><div class="form-group has-success">'+
												...
												'<span id="helpBlock2" class="help-block">Stock M&aacute;ximo: '+data.stock+' '+data.unidad.nomb+'.</span>'+
											'</div>'+
										'</td>');
										...
										$row.data('data',data).data('precio',data.precio_venta).data('stock',data.stock);
										$row.click(function(){
											var $row = $(this);
											setTimeout(function(){
				...
				$.post('fa/comp/get_var_comp',function(data){
					p.tc = data.tc.valor;
					p.ctban = data.ctban;
...
define(
	['mg/enti','mg/orga','fa/rein','mg/serv','cj/talo','fa/prod'],
	function(mgEnti,mgOrga,faRein,mgServ,cjTalo,faProd){
		return faVent;
	}
);