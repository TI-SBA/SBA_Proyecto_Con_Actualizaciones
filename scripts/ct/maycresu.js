/*******************************************************************************
mayorizacion de cuentas activo */
ctMaycResu = {
	init: function(){
		if($('#pageWrapper [child=mayc]').length<=0){
			$.post('ct/navg/mayc',function(data){
				...
				$p.find('[name=ctMaycResu]').click(function(){ ctMaycResu.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'ctMaycResu',
			titleBar: {
				title: 'Mayorizaci&oacute;n de las Cuentas de Resum&eacute;n'
			}
		});
