	...
	init: function(){
		if($('#pageWrapper [child=auxs]').length<=0){
			$.post('ct/navg/auxs',function(data){
				...
				$p.find('[name=ctAuxsPatr]').click(function(){ ctAuxsPatr.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'ctAuxsPatr',
			titleBar: {
				title: 'Auxiliares Standard de las Cuentas de Patrimonio'
			}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				...
				$mainPanel.find('[name=btnCtaMayor]').click(function(){
					ctPcon.windowSelect({tipo: 'PT',digit: 4,callback: function(data){
				...
				$mainPanel.find('[name=btnSubCta]').click(function(){
					ctPcon.windowSelect({tipo: 'PT',last: true,callback: function(data){
