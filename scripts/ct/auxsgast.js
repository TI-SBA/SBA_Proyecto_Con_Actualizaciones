	...
	init: function(){
		if($('#pageWrapper [child=auxs]').length<=0){
			$.post('ct/navg/auxs',function(data){
				...
				$p.find('[name=ctAuxsGast]').click(function(){ ctAuxsGast.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'ctAuxsGast',
			titleBar: {
				title: 'Auxiliares Standard de las Cuentas de Gasto'
			}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				...
				$mainPanel.find('[name=btnCtaMayor]').click(function(){
					ctPcon.windowSelect({tipo: 'G',digit: 4,callback: function(data){
				...
				$mainPanel.find('[name=btnSubCta]').click(function(){
					ctPcon.windowSelect({tipo: 'G',last: true,callback: function(data){
