	...
	init: function(){
		if($('#pageWrapper [child=auxs]').length<=0){
			$.post('ct/navg/auxs',function(data){
				...
				$p.find('[name=ctAuxsPasi]').click(function(){ ctAuxsPasi.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'ctAuxsPasi',
			titleBar: {
				title: 'Auxiliares Standard de las Cuentas del Pasivo'
			}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				...
				$mainPanel.find('[name=btnCtaMayor]').click(function(){
					ctPcon.windowSelect({tipo: 'P',digit: 4,callback: function(data){
				...
				$mainPanel.find('[name=btnSubCta]').click(function(){
					ctPcon.windowSelect({tipo: 'P',last: true,callback: function(data){
