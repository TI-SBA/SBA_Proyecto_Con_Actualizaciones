	...
	init: function(){
		if($('#pageWrapper [child=auxs]').length<=0){
			$.post('ct/navg/auxs',function(data){
				...
				$p.find('[name=ctAuxsResu]').click(function(){ ctAuxsResu.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'ctAuxsResu',
			titleBar: {
				title: 'Auxiliares Standard de las Cuentas de Resultados'
			}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				...
				$mainPanel.find('[name=btnCtaMayor]').click(function(){
					ctPcon.windowSelect({tipo: 'R',digit: 4,callback: function(data){
				...
				$mainPanel.find('[name=btnSubCta]').click(function(){
					ctPcon.windowSelect({tipo: 'R',last: true,callback: function(data){
