	...
	init: function(){
		if($('#pageWrapper [child=mocu]').length<=0){
			$.post('ct/navg/mocu',function(data){
				...
				$p.find('[name=ctMocuDeor]').click(function(){ ctMocuDeor.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'ctMocuDeor',
			titleBar: {
				title: 'Movimientos de las Cuentas de Orden'
			}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
			    ...
			    .data('ano',ciHelper.date.getYear()).change(function(){
			    	...
			    	ctMocuDeor.loadData({url: 'ct/mocu/lista'});
			    }).change();
