...
ctPcue = {
	...
	init: function(){
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				$('#No-Results').hide();
				$mainPanel = $('#mainPanel');
				$mainPanel.find('[name=buscar]').attr( 'placeholder' , 'Ingrese el nombre de nivel' ).width('250');
				$mainPanel.find('[name=obj]').html( 'nivel(es)' );
				...
				$mainPanel.find('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
						...
						ctPcue.loadData({page: 1,url: 'ct/pcue/lista'});
					}else{
						$("#mainPanel .gridBody").empty();
						ctPcue.loadData({page: 1,url: 'ct/pcue/search'});
					}
	    ...
	    $.post(params.url, params, function(data){
			if ( data.paging.total_page_items > 0 ) { 
				for (i=0; i < data.paging.total_page_items; i++) {
					...
					$li = $('li',$row);
					$li.eq(0).css('background',ctPcue.states[result.estado].color).addClass('vtip').attr('title',ctPcue.states[result.estado].descr);
					...
					$row.find('a').data('id',result._id.$id).dblclick(function(){
						if($(this).data('estado')=='D') return K.notification({title: 'Plan de Cuenta Deshabilitado',text: 'Debe habilitar para acceder a la edici&oacute;n!',type: 'info'});
						ctPcue.windowEdit({id: $(this).data('id'),nomb: $(this).find('li:eq(3)').html()});
							...
							bindings: {
								'conMenListEd_edi': function(t) {
									ctPcue.windowEdit({id: K.tmp.data('id'),nomb: K.tmp.find('li:eq(3)').html()});
								},
								'conMenListEd_hab': function(t) {
									K.sendingInfo();
									$.post('ct/pcue/save',{_id: K.tmp.data('id'),estado: 'H'},function(){
										K.clearNoti();
										K.notification({title: 'Plan de Cuenta Habilitado',text: 'La habilitaci&oacute;n se realiz&oacute; con &eacute;xito!'});
										$('#pageWrapperLeft .ui-state-highlight').click();
									});
								},
								'conMenListEd_des': function(t) {
									K.sendingInfo();
									$.post('ct/pcue/save',{_id: K.tmp.data('id'),estado: 'D'},function(){
										K.clearNoti();
										K.notification({title: 'Plan de Cuenta Deshabilitado',text: 'La deshabilitaci&oacute;n se realiz&oacute; con &eacute;xito!'});
										$('#pageWrapperLeft .ui-state-highlight').click();
									});
								}
							}
		...
		new K.Window({
			...
			buttons: {
				"Guardar": function(){
					...
					$.post('ct/pcue/save',data,function(){
						K.clearNoti();
						K.closeWindow(p.$w.attr('id'));
						K.notification({title: ciHelper.titleMessages.regiGua,text: 'El Plan de Cuenta fue registrado con &eacute;xito!'});
						$('#pageWrapperLeft .ui-state-highlight').click();
					});
				},
				"Cancelar": function(){
	...
	windowEdit: function(p){
		new K.Window({
			...
			title: 'Editar Plan de Cuenta: '+p.nomb,
			contentURL: 'ct/pcue/edit',
			icon: 'ui-icon-pencil',
			width: 445,
			height: 200,
			...
			onContentLoaded: function(){
				...
				K.block({$element: p.$w});
				$.post('ct/pcue/get','id='+p.id,function(data){
	...
	windowSelect: function(p){
		p.search = function(params){
			...
			params.page_rows = 20;
			params.page = (params.page) ? params.page : 1;
			$.post('ct/pcue/search',params,function(data){
				if ( data.paging.total_page_items > 0 ) {
					for (i=0; i < data.paging.total_page_items; i++) {
		...
		new K.Modal({
			id: 'windowSelectNive',
			title: 'Seleccionar Plan de Cuenta',
			contentURL: 'ct/pcue/select',
			icon: 'ui-icon-search',
			width: 510,
			height: 350,
			buttons: {
				"Seleccionar": function(){
					if(p.$w.find('.ui-state-highlight').length<=0){
						return K.notification({title: ciHelper.titleMessages.infoReq,text: 'Debe escoger un plan de cuenta!',type: 'error'});
					}
