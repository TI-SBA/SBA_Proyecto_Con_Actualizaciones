	...
	init: function(){
		if($('#pageWrapper [child=auxs]').length<=0){
			$.post('ct/navg/auxs',function(data){
				...
				$p.find('[name=ctAuxsPres]').click(function(){ ctAuxsPres.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'ctAuxsPres',
			titleBar: {
				title: 'Auxiliares Standard de las Cuentas de Presupuesto'
			}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				...
				$mainPanel.find('[name=btnCtaMayor]').click(function(){
					ctPcon.windowSelect({tipo: 'PR',digit: 4,callback: function(data){
				...
				$mainPanel.find('[name=btnSubCta]').click(function(){
					ctPcon.windowSelect({tipo: 'PR',last: true,callback: function(data){
