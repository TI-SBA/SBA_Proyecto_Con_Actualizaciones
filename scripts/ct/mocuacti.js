	...
	init: function(){
		if($('#pageWrapper [child=mocu]').length<=0){
			$.post('ct/navg/mocu',function(data){
				...
				$p.find('[name=ctMocuActi]').click(function(){ ctMocuActi.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'ctMocuActi',
			titleBar: {
				title: 'Movimientos de las Cuentas del Activo'
			}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
			    ...
			    .data('ano',ciHelper.date.getYear()).change(function(){
			    	...
			    	ctMocuActi.loadData({url: 'ct/mocu/lista'});
			    }).change();
