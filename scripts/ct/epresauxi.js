...
ctEpresAuxI = {
	...
	verifySald : function(){
		...
		if($mainPanel.find('[name=clas_sub]').data('data')==null){
		...
		data.subespecifica = $mainPanel.find('[name=clas_sub]').data('data')._id.$id;
		...
		$.post('ct/saldi/filter_one',data,function(data){
			if(data!=null){
				...
				/*$mainPanel.find('[name=clas_gen]').val(data.generica.cod).data('data',data.generica);
				$mainPanel.find('[name=descr_gen]').html(data.generica.nomb);
				$mainPanel.find('[name=clas_esp]').val(data.especifica.cod).data('data',data.especifica);
				$mainPanel.find('[name=descr_esp]').html(data.especifica.nomb);*/
				$mainPanel.find('[name=clas_sub]').html(data.subespecifica.cod).data('data',data.subespecifica);
				$mainPanel.find('[name=descr_sub]').html(data.subespecifica.nomb);
				if(data.meta){
				...
				ctEpresAuxI.loadData({
					e_p_debe_ini:data.ejec_pres.debe_ini,
					e_p_haber_ini:data.ejec_pres.haber_ini,
					e_i_debe_ini:data.ejec_ingr.debe_ini,
					e_i_haber_ini:data.ejec_ingr.haber_ini,
					estado:data.estado,
					url: 'ct/auxi/lista'
				});
			}else{
	...
	sumAll :function(){
		if($mainPanel.find('[name=estado]').data('data')!=null){
			...
			for(i=0;i<($mainPanel.find('.gridBody .item').length-1);i++){
				...
				var e_i_total_d_grid = $mainPanel.find('.gridBody .item').eq(i).find('li').eq(9).html();
				if(e_i_total_d_grid=="") e_i_total_d_grid="0";
				var e_i_total_h_grid = $mainPanel.find('.gridBody .item').eq(i).find('li').eq(10).html();
				if(e_i_total_h_grid=="") e_i_total_h_grid="0";	
				e_p_total_d = parseFloat(e_p_total_d_grid) + e_p_total_d;
				e_p_total_h = parseFloat(e_p_total_h_grid) + e_p_total_h;
				e_i_total_d = parseFloat(e_i_total_d_grid) + e_i_total_d;
				e_i_total_h = parseFloat(e_i_total_h_grid) + e_i_total_h;
			}
			...
			$mainPanel.find('.gridBody .total').eq(0).find('li').eq(9).html(K.round(e_i_total_d,2));
			$mainPanel.find('.gridBody .total').eq(0).find('li').eq(10).html(K.round(e_i_total_h,2));
			...
			if(tipo_saldo=="O1"){
				...
				$mainPanel.find('.gridBody .total').eq(0).find('li').eq(11).html(K.round(e_i_total_d-e_i_total_h,2));
	...
	init: function(){
		if($('#pageWrapper [child=epres]').length<=0){
			$.post('ct/navg/epres',function(data){
				...
				$p.find('[name=ctEpresAuxI]').click(function(){ ctEpresAuxI.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'ctEpresAuxI',
			titleBar: {
				title: 'Auxiliares Standard de ingresos'
			}
		});
		new K.Panel({
			id: 'mainPanel',
			...
			onContentLoaded: function(){
				...
				/*$mainPanel.find('[name=btnGen]').click(function(){
					prClas.windowSelect({callback: function(data){
						$mainPanel.find('[name=clas_gen]').val(data.cod).data('data',data);
						$mainPanel.find('[name=descr_gen]').html(data.nomb);
						ctEpresAuxI.verifySald();
					}});
				}).button({icons: {primary:'ui-icon-search'},text:false});
				$mainPanel.find('[name=btnEsp]').click(function(){					
					prClas.windowSelect({callback: function(data){
						$mainPanel.find('[name=clas_esp]').val(data.cod).data('data',data);
						$mainPanel.find('[name=descr_esp]').html(data.nomb);
						ctEpresAuxI.verifySald();
					}});
				}).button({icons: {primary:'ui-icon-search'},text:false});*/
				$mainPanel.find('[name=btnSub]').click(function(){				
					prClas.windowSelect({callback: function(data){
						$mainPanel.find('[name=clas_sub]').html(data.cod).data('data',data);
						$mainPanel.find('[name=descr_sub]').html(data.nomb);
						ctEpresAuxI.verifySald();
					}});
				}).button({icons: {primary:'ui-icon-search'},text:false});
				...
				$mainPanel.find('[name=btnCerrarperiodo]').click(function(){
					...
					data.ejec_ingr = new Object;
					data.ejec_ingr.debe_ini = $mainPanel.find('[name=estado]').data('data').ejec_ingr.debe_ini;
					data.ejec_ingr.haber_ini = $mainPanel.find('[name=estado]').data('data').ejec_ingr.haber_ini;
					data.ejec_ingr.debe_fin = $mainPanel.find('.gridBody .total').find('li').eq(9).html();
					data.ejec_ingr.haber_fin = $mainPanel.find('.gridBody .total').find('li').eq(10).html();
					$.post('ct/saldi/cerrar',data,function(){
	...
	loadData: function(params){
	    ...
	    $.post(params.url, params, function(data){
			if ( data.items!=null ) { 
				...
				$li1.eq(9).html(K.round(params.e_i_debe_ini,2));
				$li1.eq(10).html(K.round(params.e_i_haber_ini,2));
				$li1.eq(11).html(K.round(parseFloat(params.e_i_debe_ini)-parseFloat(params.e_i_haber_ini),2));
				...
				for (i=0; i < data.items.length; i++) {
					...
					if(result.ejec_ingr.monto=="") var result_ejec_ingr_monto="0";
					else var result_ejec_ingr_monto=result.ejec_ingr.monto;
					if(result.ejec_ingr.tipo=="D"){
						$li.eq(9).html( K.round(result_ejec_ingr_monto,2) );					
						if(result.tipo_saldo=="O1"){
							$li.eq(11).html(K.round(parseFloat($mainPanel.find('.gridBody .item').eq($mainPanel.find('.gridBody .item').length-2).find('li').eq(11).html())+parseFloat(result_ejec_ingr_monto),2));
						}else if(result.tipo_saldo=="O2"){
							$li.eq(11).html(K.round(parseFloat($mainPanel.find('.gridBody .item').eq($mainPanel.find('.gridBody .item').length-2).find('li').eq(11).html())-parseFloat(result_ejec_ingr_monto),2));
						}
					}
					else if(result.ejec_ingr.tipo=="H"){
						$li.eq(10).html( K.round(result_ejec_ingr_monto,2) );					
						if(result.tipo_saldo=="O1"){
							$li.eq(11).html(K.round(parseFloat($mainPanel.find('.gridBody .item').eq($mainPanel.find('.gridBody .item').length-2).find('li').eq(11).html())-parseFloat(result_ejec_ingr_monto),2));
						}else if(result.tipo_saldo=="O2"){
							$li.eq(11).html(K.round(parseFloat($mainPanel.find('.gridBody .item').eq($mainPanel.find('.gridBody .item').length-2).find('li').eq(11).html())+parseFloat(result_ejec_ingr_monto),2));
	...
	windowNew: function(p){
		...
		new K.Window({
			id: 'windowNewctEpresAuxi',
			title: 'Nuevo Registro del Auxiliar de Ingresos',
			contentURL: 'ct/auxi/edit',
			...
			buttons: {
				"Guardar": function(){
					...
					if($mainPanel.find('[name=estado]').data('data')==null){
						...
						data.saldo.subespecifica = new Object;
						data.saldo.subespecifica._id = $mainPanel.find('[name=clas_sub]').data('data')._id.$id;
						data.saldo.subespecifica.cod = $mainPanel.find('[name=clas_sub]').data('data').cod;
						data.saldo.subespecifica.tipo = $mainPanel.find('[name=clas_sub]').data('data').tipo;
						data.saldo.subespecifica.nomb = $mainPanel.find('[name=clas_sub]').data('data').nomb;
						if($mainPanel.find('[name=meta]').data('data')!=null){
					...
					data.ejec_ingr = new Object;
					data.ejec_ingr.tipo = p.$w.find('[name=rbtnRadEjeIng]:checked').val();
					data.ejec_ingr.monto = p.$w.find('[name=monto_ejec_ing]').val();
					data.tipo_saldo = p.$w.find('[name=saldos]:checked').val();
					K.sendingInfo();
					p.$w.dialog("widget").find('.ui-dialog-buttonpane button').button('disable');
					$.post('ct/auxi/save',data,function(){
						K.clearNoti();
						K.closeWindow(p.$w.attr('id'));
						K.notification({title: ciHelper.titleMessages.regiGua,text: 'El auxiliar de ingresos fue registrado con &eacute;xito!'});
						ctEpresAuxI.verifySald();
					});
				},
				"Cancelar": function(){
			...
			onContentLoaded: function(){
				p.$w = $('#windowNewctEpresAuxi');
				K.block({$element: p.$w});
				p.$w.find('[name=periodo]').html($mainPanel.find('[name=ano]').val());
				p.$w.find('[name=clasif]').html($mainPanel.find('[name=clas_sub]').data('data').cod+' '+$mainPanel.find('[name=clas_sub]').data('data').nomb);
				...
				p.$w.find('[name=RadEjeIng]').buttonset();
	...
	windowEdit: function(p){
		...
		new K.Window({
			id: 'windowNewctEpresAuxi',
			title: 'Editar Registro del Auxiliar de Ingresos',
			contentURL: 'ct/auxi/edit',
			icon: 'ui-icon-plusthick',
			width: 580,
			height: 450,
			...
			onContentLoaded: function(){
				...
				$.post('ct/auxi/get','id='+p.id,function(data){
					...
					p.$w.find('[name=clasif]').html(data.saldo.subespecifica.cod);
					...
					if(data.ejec_ingr.tipo=="H")p.$w.find('#RadEjeIngrHaber').click();
					p.$w.find('[name=monto_ejec_pres]').val(data.ejec_pres.monto);
					p.$w.find('[name=monto_ejec_ing]').val(data.ejec_ingr.monto);
					if(data.tipo_saldo=="O2")p.$w.find('[name=saldos]').eq(2).click();
				},'json');
				K.unblock({$element: p.$w});
			}
		});
