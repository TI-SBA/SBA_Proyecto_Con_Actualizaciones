	...
	init: function(){
		...
		new K.Panel({
			onContentLoaded: function(){
		   		var $grid = new K.grid({
					cols: ['','','Orden',{n:'Proveedor',f:'proveedor'},'Almacen',{n:'&Uacute;ltima Modificaci&oacute;n',f:'fecmod'}],
					data: 'lg/orde/lista_cont',
					params: {},
					...
					fill: function(data,$row){
						$row.append('<td>');
						$row.append('<td>'+lgOrde.states[data.estado_cont].label+'</td>');
						$row.append('<td>'+'Orden de Compra N&deg;'+data.cod +'</td>');
						...
						$row.data('id',data._id.$id).data('estado',data.estado).data('data', data).contextMenu("conMenCtVeoc", {
							...
							bindings: {
								'conMenCtVeoc_ver': function(t) {
									//ctVeoc.windowDetails({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(2)').html()});
									lgOrde.windowDetails({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(2)').html(),goBack: function(){
										ctVeoc.init(); 
									}});
								},
								'conMenCtVeoc_ing': function(t) {
									ctVeoc.windowEdit({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(2)').html()});
								},
					...
					$.post('lg/orde/get',{_id: p.id},function(data){
						...
						for(var i=0; i<data.productos.length; i++){
							...
							for(var ii=0; ii<data.productos[i].asignacion.length; ii++){
								var $ul = $('<ul><li>'+data.productos[i].asignacion[ii].organizacion.nomb+': cant. '+data.productos[i].asignacion[ii].cantidad+' S/.'+data.productos[i].subtotal+'</li></ul>');
...
define(
	['mg/enti','ct/pcon','lg/orde'],
	function(mgEnti,ctPcon, lgOrde){
		return ctVeoc;
	}
);