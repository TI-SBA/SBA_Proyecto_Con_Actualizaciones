	...
	init: function(){
		if($('#pageWrapper [child=mocu]').length<=0){
			$.post('ct/navg/mocu',function(data){
				...
				$p.find('[name=ctMocuActi]').click(function(){ ctMocuActi.init(); });
				$p.find('[name=ctMocuPasi]').click(function(){ ctMocuPasi.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'ctMocuPasi',
			titleBar: {
				title: 'Movimientos de las Cuentas del Pasivo'
			}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
			    ...
			    .data('ano',ciHelper.date.getYear()).change(function(){
			    	...
			    	ctMocuPasi.loadData({url: 'ct/mocu/lista'});
			    }).change();
