			...
			onContentLoaded: function(){
		   		var $grid = new K.grid({
					...
					fill: function(data,$row){
						...
						$row.append('<td>'+chPaci.categoria[data.categoria]+'</td>');
						$row.append('<td>'+mgEnti.formatName(data.paciente)+'</td>');
						$row.append('<td>'+mgEnti.formatDire(data.paciente)+'</td>');
						$row.append('<td>'+moment(data.fe_regi.sec,'X').format('DD/MM/YYYY')+'</td>');
						...
						$row.data('id',data._id.$id).data('data',data).dblclick(function(){
							K.WindowPrint({
								...
								url:"ch/paci/print?_id="+$(this).data('id')
							});
							...
							bindings: {
								'conMenMheresi_fron': function(t) {
									chPaci.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenMheresi_edi': function(t) {
									chPaci.windowEdit({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								'conMenMheresi_pabe': function(t) {
									chPaci.windowModulo({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								...
								'conMenMheresi_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('ch/paci/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenMheresi_fron':function(t){
									K.windowPrint({
										id:'windowPrint',
										title:"Informe Frontal",
										url:"ch/paci/print?_id="+K.tmp.data('id')
									});
								},
								'conMenMheresi_soci':function(t){
									K.windowPrint({
										...
										url:"ch/paci/get_fichasalud?_id="+K.tmp.data('id')
									});
								},
								'conMenMheresi_tarje':function(t){
									K.windowPrint({
										...
										url:"ch/paci/get_tarjeta?_id="+K.tmp.data('id')
									});
								},
								'conMenMheresi_carnet':function(t){
									K.windowPrint({
										...
										url:"ch/paci/get_carnet?_id="+K.tmp.data('id')
									});
								},
	...
	windowNew: function(p){
		...
		new K.Panel({
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								$.post("ch/paci/save",data,function(result){
									K.clearNoti();
			...
			onContentLoaded: function(){
					...
					mgEnti.windowSelect({callback: function(data){
						...
						if(data.roles!=null){
							if(data.roles.paciente!=null){
								if(data.roles.paciente.centro=='CH'){
				...
				$.post("ch/paci/get_historial",function(data){
	...
	windowEdit: function(p){
		new K.Panel({ 
			...
			onContentLoaded: function(){
				...
				$.post('ch/paci/get',{_id: p.id},function(data){
			  	    
			  	    p.$w.find('[name=his_Cli]').val(data.his_cli);
					p.$w.find('[name=estado]').val(data.estado);
					p.$w.find('[name=modulo]').val(data.modulo);
					p.$w.find('[name=fe_regi]').val(moment(data.fe_regi.sec,'X').format('YYYY-MM-DD'));
...
define(
	['mg/enti','ct/pcon','ch/dini','mg/ubig'],
	function(mgEnti,ctPcon,chdini, Ubig){
		return chPaci;
	}
);