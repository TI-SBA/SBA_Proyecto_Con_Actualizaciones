					...
					fill: function(data,$row){
						...
						*/
						...
						$row.data('id',data._id.$id).dblclick(function(){
							chEvol.windowDetails({_id: $(this).data('id'),nomb: $(this).find('td:eq(2)').html()});
							...
							bindings: {
								'conMenFMedica_info': function(t) {
									chEvol.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenFMedica_edi': function(t) {
									chEvol.windowEdit({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								...
								'conMenFMedica_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('ch/Evol/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
									...
									K.windowPrint({
										id:'windowPrint',
										title:"Evolucion Medica",
										url:"ch/Evol/print?_id="+K.tmp.data('id')
									});
								},
			...
			buttons: {
				"Guardar": {
					...
					if ( p.$w.find('[name=gridEvol] tbody tr').length>0) {
						...
						$.post("ch/evol/save",data,function(result){
							K.clearNoti();
		...
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				new K.grid({
			     ...
			     $.post('ch/evol/get',{_id: p.id},function(data){
