						...
						$row.data('id',data._id.$id).data('data',data).data('estado',data.estado).contextMenu("conMenFPsico", {
							...
							bindings: {
								'conMenFPsico_info': function(t) {
									chPsic.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenFPsico_edi': function(t) {
				 					chPsic.windowEdit({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
				 				},
								...
								'conMenFPsico_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('ch/psic/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenFPsico_info':function(t){
									K.windowPrint({
										id:'windowPrint',
										title:"Ficha Psicologica",
										url:"ch/psic/print?_id="+K.tmp.data('id')
									});
								},
		...
		new K.Modal({
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								$.post("ch/psic/save",data,function(result){
									K.clearNoti();
			...
			onContentLoaded: function(){
				...
				p.$w.find("[name=btnDiag]").click(function(){
					chPaci.windowSelect_({callback: function(data){
	...
	windowEdit: function(p){
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				K.block();
				$.post('ch/psic/get',{_id: p.id},function(data){
	...
	windowEvolucion: function(p){
		...
		new K.Modal({
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								p.$w.find('#div_buttons button').attr('disabled','disabled');
								$.post("ch/hist/save",data,function(result){
									K.clearNoti();
			...
			onContentLoaded: function(){
				...
				K.block();
				$.post('ch/hist/get',{_id: p.id},function(data){
...
define(
	['mg/enti','ct/pcon','ch/paci','ch/hist'],
	function(mgEnti,ctPcon,chpaci,chhist){
		return chPsic;
	}
);