						...
						$row.data('id',data._id.$id).data('data',data).data('estado',data.estado).contextMenu("conMenFMedica", {
							...
							bindings: {
								'conMenFMedica_info': function(t) {
									chChar.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenFMedica_edi': function(t) {
									chChar.windowEdit({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								...
								'conMenFMedica_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('ch/char/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenFMedica_info':function(t){
									K.windowPrint({
										id:'windowPrint',
										title:"Charlas",
										url:"ch/char/print?_id="+K.tmp.data('id')
									});
								},
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								$.post("ch/char/save",data,function(result){
									K.clearNoti();
		...
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				K.block();
				    ...
				    $.post('ch/char/get',{_id: p.id},function(data){
