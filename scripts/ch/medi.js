					...
					fill: function(data,$row){
						...
						$row.data('id',data._id.$id).data('data',data).dblclick(function(){
							K.WindowPrint({
								id:'windowPrint',
								title:"Informe ",
								url:"ch/medi/print?_id="+$(this).data('id')
							});
							...
							bindings: {
								'conMenFMedica_info': function(t) {
									chMedi.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenFMedica_edi': function(t) {
									chMedi.windowEdit({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								...
								'conMenFMedica_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('ch/medi/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenFMedica_info':function(t){
									K.windowPrint({
										...
										url:"ch/medi/get_reporte?_id="+K.tmp.data('id')
									});
								},
	...
	windowNew: function(p){
		...
		new K.Modal({
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								$.post("ch/medi/save",data,function(result){
									K.clearNoti();
			...
			onContentLoaded: function(){
				...
				$.post("ch/medi/get_reporte",function(data){
		...
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				K.block();
				     ...
				     $.post('ch/medi/get',{_id: p.id},function(data){
...
define(
	['mg/enti','ct/pcon','ch/paci'],
	function(mgEnti,ctPcon,chpaci ){
		return chMedi;
	}
);