chHospi = {
			...
			onContentLoaded: function(){
		   		var $grid = new K.grid({
					...
					fill: function(data,$row){
						...
						var tipo = '--';
						if(data.tipo_hosp){
							tipo = chHospi.tipos[data.tipo_hosp];
						}
						...
						$row.data('id',data._id.$id).data('data',data).dblclick(function(){
							K.WindowPrint({
								id:'windowPrint',
								title:"Informe ",
								url:"ch/hospi/if_fron?_id="+$(this).data('id')
							});
							...
							bindings: {
								'conMenFMedica_info': function(t) {
									chHospi.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenFMedica_edi': function(t) {
									chHospi.windowEdit({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								...
								'conMenFMedica_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('ch/hospi/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenFhospica_info':function(t){
									K.windowPrint({
										...
										url:"ch/hospi/print?_id="+K.tmp.data('id')
									});
								},'conMenFMedica_info':function(t){
									K.windowPrint({
										id:'windowPrint',
	...
	windowNew: function(p){
		if(p==null) p = {};
		chPaci.windowSelect_({callback: function(paci){
			new K.Panel({
				title: 'Nueva Hospitalizacion',
				contentURL: 'ch/hospi/edit',
				store:false,
				buttons: {
					"Guardar": {
						icon: 'fa-save',
						...
						f: function(){
							...
							var form = ciHelper.validator(p.$w.find('form'),{
								onSuccess: function(){
									...
									$.post("ch/hospi/save",data,function(result){
				...
				onContentLoaded: function(){
					...
					$.post('ch/hospi/get_ingreso',{_id:paci.paciente._id.$id}, function(data){
						var ing = 1;
						if(data!=null){
	...
	windowEdit: function(p){
		new K.Panel({ 
			...
			onContentLoaded: function(){
				...
				K.block();
			    $.post('ch/hospi/get',{_id: p.id},function(data){
