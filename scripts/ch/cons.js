				...
				$row.append('<td><kbd>'+ciHelper.date.format.bd_ymdhi(data.fecmod)+'</kbd><br />'+mgEnti.formatName(data.modificado)+'</td>');
						$row.data('id',data._id.$id).data('data',data).dblclick(function(){
							K.WindowPrint({
								id:'windowPrint',
								title:"Informe ",
								url:"ch/cons/if_fron?_id="+$(this).data('id')
							});
							...
							bindings: {
								'conMenFMedica_info': function(t) {
									chCons.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenFMedica_edi': function(t) {
									chCons.windowEdit({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								...
								'conMenFMedica_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('ch/cons/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenFconsca_info':function(t){
									K.windowPrint({
										id:'windowPrint',
										title:"Consulta",
										url:"ch/cons/print?_id="+K.tmp.data('id')
									});
								},'conMenFMedica_info':function(t){
									K.windowPrint({
										id:'windowPrint',
...
define(
	['ch/paci','ch/dini','ch/padi'],
	function(chpaci,chdini,chpadi ){
		return chCons;
	}
);