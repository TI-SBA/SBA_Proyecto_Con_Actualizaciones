						...
						$row.data('id',data._id.$id).data('data',data).data('estado',data.estado).contextMenu("conMenFMedica", {
							bindings: {
								'conMenFMedica_info': function(t) {
									chPsiq.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenFMedica_edi': function(t) {
									$.post('ch/psiq/permiso',{_id: K.tmp.data('id')},function(rpta){
				 						if(rpta.data.permiso==true){
				 							K.msg({title:'Mensaje del Sistema!',type: rpta.status,text: rpta.message});
				 							chPaci.windowEdit({_id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
				 						}else{
				 							K.msg({title:'Mensaje del Sistema!',type: rpta.status,text: rpta.message});
				 						}
				 						chPsiq.init();
								...
								'conMenFMedica_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('ch/psiq/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenFMedica_info':function(t){
									K.windowPrint({
										id:'windowPrint',
										title:"Ficha Psiquiatrica",
										url:"ch/psiq/print?_id="+K.tmp.data('id')
									});
								},
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								$.post("ch/psiq/save",data,function(result){
									K.clearNoti();
	...
	windowEdit: function(p){
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				$.post('ch/psiq/get',{_id: p.id},function(data){
