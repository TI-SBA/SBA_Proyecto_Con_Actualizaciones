						...
						$row.data('id',data._id.$id).dblclick(function(){
							...
							bindings: {
								'conMenMhCama_edi': function(t) {
									chCama.windowDetails({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(2)').html()});
								},							
								'conMenMhCama_eli': function(t) {
									ciHelper.confirm('&#191;Desea <b>Eliminar</b> la Ficha Medica:  <b>'+K.tmp.find('td:eq(2)').html()+'</b>&#63;',
									function(){
										K.sendingInfo();
										$.post('ch/cama/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
			...
			buttons: {
				"Guardar": {
					...
					f: function(){
						...
						$.post("ch/cama/save",data,function(result){
							K.clearNoti();
	...
	windowEdit: function(p){
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				$.post('ch/cama/get',{_id: p.id},function(data){
