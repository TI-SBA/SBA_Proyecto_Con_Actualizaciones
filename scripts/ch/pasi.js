chPasi = {
	...
	init: function(){
		K.initMode({
			...
			action: 'chPasi',
			titleBar: {
				title: 'Parte Diario Psicologico'
			}
		});
		new K.Panel({
					...
					fill: function(data,$row){
						...
						$row.data('id',data._id.$id).data('data',data).dblclick(function(){
							K.WindowPrint({
								id:'windowPrint',
								title:"Informe ",
								url:"ch/pasi/if_fron?_id="+$(tº).data('id')
							});
							...
							bindings: {
								'conMenPDiario_info': function(t) {
									chPasi.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenPDiario_edi': function(t) {
									chPasi.windowEdit({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								'conMenPDiario_cons': function(t) {
									chPasi.windowConsultaMedica({id: K.tmp.data('id'),padi: K.tmp.data('data')});
								},
							
								'conMenPDiario_eli': function(t) {
									ciHelper.confirm('&#191;Desea <b>Eliminar</b> la Parte Diario Psicologico:  <b>'+K.tmp.find('td:eq(2)').html()+'</b>&#63;',
									function(){
										K.sendingInfo();
										$.post('ch/pasi/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
											K.notification({title: 'Parte Diario Psicologico Eliminada',text: 'La eliminaci&oacute;n se realiz&oacute; con &eacute;xito!'});
											chPasi.init();
										});
									},function(){
										$.noop();
									},'Eliminaci&oacute;n de Parte Diario Psicologico');
								},
								'conMenFpadica_info':function(t){
									K.windowPrint({
										id:'windowPrint',
										title:"Parte Diario Psicologico",
										url:"ch/pasi/print?_id="+K.tmp.data('id')
									});
								},'conMenPDiario_info':function(t){
									K.windowPrint({
										id:'windowPrint',
	...
	windowNew: function(p){
		...
		new K.Modal({
			...
			title: 'Nueva Parte Diario Psicologico',
			contentURL: 'ch/pasi/edit',
			width: 500,
			height: 450,
			store:false,
			buttons: {
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								$.post("ch/pasi/save",data,function(result){
									K.clearNoti();
									K.msg({title: ciHelper.titles.regiGua,text: "Parte Diario Psicologico Agregado!"});
			...
			onContentLoaded: function(){
				...
				$.post("ch/pasi/get_parte",function(data){
	...
	windowEdit: function(p){
		new K.Modal({ 
			...
			title: 'Editar Parte Diario Psicologico de: ' + p.nom,
			contentURL: 'ch/pasi/edit',
			width: 500,
			height: 450,
			buttons: {
				"Guardar": {
					...
					f: function(){
						K.clearNoti();
						var data = {
							//DATOS DEL Parte Diario Psicologico
							_id: p.id,
									num:p.$w.find('[name=num]').val(),
						...
						$.post("ch/pasi/save",data,function(result){
							K.clearNoti();
							K.msg({title: ciHelper.titles.regiAct,text: "Parte Diario Psicologico Actualizado!"});
							chPasi.init();
							K.closeWindow(p.$w.attr('id'));
						},'json');
					}
			...
			onContentLoaded: function(){
				...
				$.post('ch/pasi/get',{_id: p.id},function(data){
	...
	windowConsultaMedica: function(p){
		...
		new K.Panel({
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								p.$w.find('#div_buttons button').attr('disabled','disabled');
								$.post("ch/pasi/save_diario",data,function(result){
									K.clearNoti();
			...
			onContentLoaded: function(){
				...
				var cuar = 0;
				K.block();
				$.post('ch/pasi/get',{_id: p.id,all:true},function(data){
					p.padi = data;
					...
					for(var i=0;i<p.padi.consulta.length;i++){
						...
						$row.append('<td>'+chPaci.sexo[p.padi.consulta[i]._paciente.sexo]+'</td>');
						$row.append('<td>'+edad+'</td>');
						$row.append('<td>'+chPasi.estadoConsulta[p.padi.consulta[i].esta]+'</td>');
						$row.append('<td>'+p.padi.consulta[i].cie10+'</td>');
						$row.append('<td>'+chPaci.categoria[p.padi.consulta[i].cate]+'</td>');
						...
						$row.find('[name=btnEli]').click(function(){
							...
							function(){
								K.sendingInfo();
								$.post('ch/pasi/delete_consulta',{index:$_row.data('i'), _id: p.id}, function(rpta){
									$_row.remove();
								},'json')
							},function(){
								$.noop();
		...
		new K.Modal({ 
			...
			buttons: {
				"Guardar": {
					...
					f: function(){
						...
						K.sendingInfo();
						p.$w.find('#div_buttons button').attr('disabled','disabled');
						$.post("ch/cons/save",data,function(result){
							K.clearNoti();
			...
			onContentLoaded: function(){
				...
				K.block();
				     ...
				     $.post('ch/cons/get',{_id: p.id},function(data){
...
define(
	['ch/paci','ch/dini','ch/cons'],
	function(chpaci,chdini,chCons){
		return chPasi;
	}
);