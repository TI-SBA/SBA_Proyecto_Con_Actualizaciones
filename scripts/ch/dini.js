			...
			onContentLoaded: function(){
		   		var $grid = new K.grid({
					...
					fill: function(data,$row){
						...
						var estado = '';
						if(data.estado!=null){
							estado = chDini.states[data.estado].label
						}
						...
						$row.data('id',data._id.$id).dblclick(function(){
							chDini.windowDetails({_id: $(this).data('id'),nomb: $(this).find('td:eq(2)').html()});
							...
							bindings: {
								'conMenList_edi': function(t) {
									chDini.windowDetails({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(2)').html()});
								},							
								'conMenList_eli': function(t) {
									ciHelper.confirm('&#191;Desea <b>Eliminar</b> la Ficha Medica:  <b>'+K.tmp.find('td:eq(2)').html()+'</b>&#63;',
									function(){
										K.sendingInfo();
										$.post('ch/dini/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
			...
			buttons: {
				"Guardar": {
					...
					f: function(){
						...
						$.post("ch/dini/save",data,function(result){
							K.clearNoti();
	...
	windowEdit: function(p){
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				$.post('ch/dini/get',{_id: p.id},function(data){
