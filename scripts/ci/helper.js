ciHelper = {
	...
	titleMessages: {
		infoReq: "Informaci&oacute;n requerida",
	...
	dateAAMM00: function(date){
		if(date==null){
			return '--';
		}
		var ano = date[0]+date[1]+date[2]+date[3];
		var mes = parseFloat(date[4]+date[5])-1;
		mes = ciHelper.meses[mes];
		return mes+" - "+ano;
	},
	dateFormatBD: function(dateOri){
		if(dateOri==null){
			return '--';
		}
		if(dateOri.sec==null){
	...
	dateFormatBDNotSec: function(dateOri){
		if(dateOri==null){
			return '--';
		}
		if(dateOri.sec==null){
		...
		var date = new Date(dateOri.sec*1000);
		var month = date.getMonth()+1;
		var dateFor = date.getFullYear()+"-"+(month<10?'0'+month:month)+"-"+(date.getDate()<10?'0'+date.getDate():date.getDate())+" "+(date.getHours()<10?'0'+date.getHours():date.getHours())+':'+(date.getMinutes()<10?'0'+date.getMinutes():date.getMinutes());
		return dateFor;
	},
	dateFormatBDNotHour: function(dateOri){
		if(dateOri==null){
			return '--';
		}
		if(dateOri.sec==null){
	...
	dateFormat: function(dateOri){
		...
		var date = new Date(dateOri.sec*1000);
		var month = date.getMonth()+1;
		return date.getDate()+"/"+month+"/"+date.getFullYear()+" "+(date.getHours()<10?'0'+date.getHours():date.getHours())+':'+(date.getMinutes()<10?'0'+date.getMinutes():date.getMinutes());
	},
	dateFormatOnlyDay: function(dateOri){
		if(dateOri==null){
			return '--';
		}
		if(dateOri.sec==null){
	...
	dateFormatLong: function(dateOri){
		if(dateOri==null){
			return '--';
		}
		if(dateOri.sec==null){
	...
	dateFormatNow: function(){
		var date = new Date();
		var month = date.getMonth()+1;
		return date.getDate()+"/"+month+"/"+date.getFullYear()+" "+(date.getHours()<10?'0'+date.getHours():date.getHours())+':'+(date.getMinutes()<10?'0'+date.getMinutes():date.getMinutes());
	},
	dateFormatNowOnlyDay: function(){
		var date = new Date();
		var month = date.getMonth()+1;
		return date.getDate()+"/"+month+"/"+date.getFullYear();
	},
	dateFormatNowLong: function(){
		var date = new Date();
	...
	dateFormatNowBDLong: function(){
		var date = new Date();
		var month = date.getMonth()+1;
		var dateFor = date.getFullYear()+"-"+(month<10?'0'+month:month)+'-'+(date.getDate()<10?'0'+date.getDate():date.getDate())+' '+(date.getHours()<10?'0'+date.getHours():date.getHours())+':'+(date.getMinutes()<10?'0'+date.getMinutes():date.getMinutes())+':00';
		return dateFor;
	},
	dateFormatNowBDNotHour: function(){
		var date = new Date();
		var month = date.getMonth()+1;
		var dateFor = date.getFullYear()+"-"+(month<10?'0'+month:month)+'-'+(date.getDate()<10?'0'+date.getDate():date.getDate());
		return dateFor;
	},
	dateFormatCalToBDHour: function(date){
		var dateFor = (date.getHours()<10?'0'+date.getHours():date.getHours())+':'+(date.getMinutes()<10?'0'+date.getMinutes():date.getMinutes());
		return dateFor;
	},
	dateFormatCalToBD: function(date){
		var month = date.getMonth()+1;
		var dateFor = date.getFullYear()+"-"+(month<10?'0'+month:month)+"-"+(date.getDate()<10?'0'+date.getDate():date.getDate())+" "+(date.getHours()<10?'0'+date.getHours():date.getHours())+':'+(date.getMinutes()<10?'0'+date.getMinutes():date.getMinutes())+':00';
		return dateFor;
	},
	...
	*/
	...
	date: {
	    ...
	    diffDays: function(date1,date2){
			var diferencia = date1.getTime() - date2.getTime();
			if(ciHelper.date.format.dayBD(date1)==ciHelper.date.format.dayBD(date2))
				return 0;
			else
				return Math.floor(diferencia / 1000 / 60 / 60 / 24);
	    },
		format: {
			...
			hi: function(date){
				return ciHelper.date.format.h(date)+':'+ciHelper.date.format.i(date);
			},
	...
	enti: {
		...
		dbTrabRel: function(data){
			...
			if(data.roles!=null){
				if(data.roles.trabajador!=null){
					enti.cargo = {
						/*organizacion: {
							_id: data.roles.trabajador.cargo.organizacion._id.$id,
							nomb: data.roles.trabajador.cargo.organizacion.nomb
						}*/
					};
					if(data.roles.trabajador.programa!=null){
						enti.programa = {
							_id: data.roles.trabajador.cargo._id.$id,
		...
		dbTrabRelAll: function(data){
			...
			enti.organizacion = enti.cargo.organizacion;
			if(data.roles.trabajador.cargo._id!=null){
				enti.cargo._id = data.roles.trabajador.cargo._id.$id;
				enti.cargo.nomb = data.roles.trabajador.cargo.nomb;
			}else{
		...
		new K.Modal({
			...
			buttons: {
				"Seleccionar": function(){
					p.callback(ciHelper.date.format.ymd(p.$c.find('#picker').datepicker('getDate')));
			...
			onContentLoaded: function(){
				...
				p.$c = p.$w;
				p.$c.append('<div id="picker" data-date="'+p.date+'" data-date-format="yyyy-mm-dd"></div>');
  ...
  selectIniFin: function(p){
	...
	validator: function(p){
		...
		if(controls.length>0){
			p.$w.find('.has-error .has-success').removeClass('has-error has-success')
				...
				switch(type_control){
					case "text":
						...
						if(item_control.val()==''){
							...
							error_message = '<strong>'+name_control+':</strong> Este campo es requerido';
							is_error = false;
						}
						break;
					case "textarea":
