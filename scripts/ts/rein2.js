tsRein2 = {
			...
			onContentLoaded: function(){
		   		var $grid = new K.grid({
					...
					fill: function(data,$row){
						$row.append('<td>');
						$row.append('<td>'+tsRein2.modul[data.modulo].label+'</td>');
						//$row.append('<td>'+tsRein2.states[data.estado].label+'</td>');
						...
						if(data.organizacion._id.$id == '51a50edc4d4a13441100000e' )
							$row.append('<td>'+tsRein2.tipo_inm[data.tipo_inm]+'</td>');
						else
							$row.append('<td>--</td>');
						*/
						...
						$row.data('id',data._id.$id).dblclick(function(){
							tsRein2.windowPrint({data: $(this).closest('.item').data('data')});
							...
							bindings: {
								'conMenTsRein_ver': function(t) {
									tsRein2.windowPrint({data: K.tmp.data('data')});
								},
								'conMenTsRein_rec': function(t) {
									tsRein2.windowAprobar({id: K.tmp.data('data')._id.$id});
								}
							}
						});
						return $row;
	...
	windowPrint: function(p){
		var url = "";
		if(p.data.organizacion != null){
			switch(p.data.organizacion._id.$id){
				case '51a50edc4d4a13441100000e': url = "in/comp/reci_ing2?_id="; break;
		...
		}else if(p.data.modulo != null){
			switch(p.data.modulo){
				case 'IN': url = "in/comp/reci_ing2?_id="; break;
				case 'CM': url = "cj/repo/reci_ing?_id="; break;
				case 'MH': url = "in/comp/reci_ing2?_id="; break;
				case 'FA': url = "fa/comp/reci_ing?_id="; break;
				case 'AG': url = "ag/comp/reci_ing?_id="; break;
				case 'LM': url = "in/comp/reci_ing2?_id="; break;
				case 'AD': url = "in/comp/reci_ing2?_id="; break;
				default: url = "cj/repo/reci_ing?_id="; break;
			}
		}
		var params = {
