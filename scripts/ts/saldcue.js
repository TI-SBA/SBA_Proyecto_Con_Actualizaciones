	...
	init: function(){
		if($('#pageWrapper [child=sald]').length<=0){
			...
			$.post('ts/navg/sald',function(data){
				...
				$p.find('[name=tsSaldCue]').click(function(){ tsSaldCue.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'tsSaldCue',
			titleBar: {
				title: 'Saldos: Cuentas corrientes'
			}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				...
				$.post('ts/ctban/all_nr?tipo=C',function(data){
					if(data.length<1){
				...
				$mainPanel.find('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
						...
						tsSaldCue.loadData({page: 1,url: 'ts/sald/lista'});
					}else{
						$("#mainPanel .gridBody").empty();
						tsSaldCue.loadData({page: 1,url: 'ts/sald/search'});
					}
	    ...
	    $.post(params.url, params, function(data){
			if ( data.paging.total_page_items > 0 ) { 
				for (i=0; i < data.paging.total_page_items; i++) {
					...
					$li = $('li',$row);
					$li.eq(0).css('background',tsSaldCue.states[result.estado].color).addClass('vtip').attr('title',tsSaldCue.states[result.estado].descr);
		...
		new K.Window({
			...
			buttons: {
				"Guardar": function(){
					...
					var data = new Object;	
					data.tipo = "C";
					data.periodo = p.$w.find('[name=ano]').val()+(parseFloat(p.$w.find('[name=mes] :selected').val()<10)?"0":"")+p.$w.find('[name=mes] :selected').val()+'00';
					data.estado = "A";
					...
					cuenta_banco = p.$w.find('[name=ct_ban] :selected').data('data');
					data.cuenta_banco = new Object;
					data.cuenta_banco._id = cuenta_banco._id.$id;
					data.cuenta_banco.cod_enti_financiera = cuenta_banco.cod_banco;
					...
					$.post('ts/sald/save_cue',data,function(){
						K.clearNoti();
