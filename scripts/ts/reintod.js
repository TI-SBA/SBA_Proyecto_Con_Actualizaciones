	...
	init: function(){
		if($('#pageWrapper [child=rein]').length<=0){
			...
			$.post('ts/navg/rein',function(data){
				...
				$p.find('[name=tsReinTod]').click(function(){ tsReinTod.init(); }).addClass('ui-state-highlight');
				$p.find('[name=tsReinPor]').click(function(){ tsReinPor.init(); });
			},'json');
		}
		K.initMode({
			mode: 'ts',
			action: 'tsReinTod',
			titleBar: {
				title: 'Recibo de Ingresos: Todas las Dependencias'
			}
		});
			...
			onContentLoaded: function(){
				...
				$mainPanel.find('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
						...
						tsReinTod.loadData({page: 1,url: 'ts/rein/lista'});
					}else{
						$("#mainPanel .gridBody").empty();
						tsReinTod.loadData({page: 1,url: 'ts/rein/search'});
					}
	    ...
	    $.post(params.url, params, function(data){
			if ( data.paging.total_page_items > 0 ) { 
				for (i=0; i < data.paging.total_page_items; i++) {
					...
					}).data('estado',result.estado).data('dolar',parseFloat(result.efectivos[1].monto)).contextMenu("conMenTsRein", {
						onShowMenu: function(e, menu) {
							$(e.target).closest('.gridBody').find('ul').removeClass('ui-state-highlight');
							$(e.target).closest('.item').find('ul').addClass('ui-state-highlight');
							$(e.target).closest('.item').click();
							K.tmp = $(e.target).closest('.item');
							$('#conMenTsRein_anu',menu).remove();
							if(K.tmp.data('estado')=='RG') $('#conMenTsRein_mov,#conMenTsRein_cta',menu).remove();
							else if(K.tmp.data('estado')=='RB') $('#conMenTsRein_rec,#conMenTsRein_cta',menu).remove();
							else if(K.tmp.data('estado')=='RE') $('#conMenTsRein_mov,#conMenTsRein_rec',menu).remove();
							else if(K.tmp.data('estado')=='RC') $('#conMenTsRein_rec,#conMenTsRein_mov,#conMenTsRein_cta',menu).remove();
							return menu;
						},
						bindings: {
							...
							'conMenTsRein_cta': function(t) {
								tsRein.windowCta({id: K.tmp.data('id'),nomb: K.tmp.find('li:eq(3)').html(),dolar: K.tmp.data('dolar')});
							},
							'conMenTsRein_mov': function(t) {
								tsRein.windowMov({id: K.tmp.data('id'),nomb: K.tmp.find('li:eq(3)').html()});
							},
							'conMenTsRein_rec': function(t) {
								tsRein.windowRec({id: K.tmp.data('id'),nomb: K.tmp.find('li:eq(3)').html()});
							}
						}
					});
