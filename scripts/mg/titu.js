...
mgTitu = {
	init: function(data){
		...
		new K.Panel({
			id: 'mainPanel',
			title: 'Informaci&oacute;n de titular',
			contentURL: 'mg/titu',
			onContentLoaded: function(){
				...
				$p.find('.ui-layout-west').find('.[name=btnGuardar]').click(function(){
					...
					$.post('mg/titu/save',data,function(rpta){
						K.clearNoti();
				...
				if(data.nomb!=null){
					...
					$.post('mg/titu/titular',function(data){
						if(mgTitu.loadData(data)){
							$('#pageWrapperMain').layout();
							$('#mainPanel').layout({
								west__size:			170,
		...
		new K.Modal({
			...
			onContentLoaded: function(){
				...
				p.$grid = new K.grid({
					...
					cols: ['','Descripci&oacute;n','Direcci&oacute;n'],
					data: 'mg/titu/locales',
					params: {},
					itemdescr: 'local(es)',
					onLoading: function(){ 
						K.block({$element: p.$w});
