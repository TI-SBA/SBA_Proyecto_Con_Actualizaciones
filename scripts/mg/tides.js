...
mgTides = {
	init: function(){
		K.initMode({
			mode: 'mg',
			action: 'mgTides',
			titleBar: { title: 'Tipos de Identidad' }
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				$('#No-Results').hide();
				$mainPanel = $('#mainPanel');
				$mainPanel.find('[name=obj]').html( 'identidades' );
				...
				$li.click( function(){
					...
					mgTides.loadData({page: 1,url: 'mg/tiden/lista',filter: $(this).attr("filter"),order: order});
				});
				$mainPanel.find('[name=btnAgregar]').click(function(){
					mgTides.windowNew({tiden: $(this).attr('tiden'),oper: $(this).attr('oper')});
				}).button({icons: {primary: 'ui-icon-plusthick'}});
				$('.divSearch [name=buscar]').keyup(function(e){
					if(e.keyCode == 13) $('.divSearch [name=btnBuscar]').click();
				}).attr('placeholder',' Buscar Tipo de Identidad').width('250');
				$('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
			            $("#mainPanel .gridBody").empty();
			            mgTides.loadData({page: 1,url: 'mg/tiden/lista'});
					}else{
			            K.notification({text: 'B&uacute;squeda realizada!',layout:"topLeft"});
			            $("#mainPanel .gridBody").empty();
			            mgTides.loadData({page: 1,url: 'mg/tiden/search'});
					}
			...
			if ( data.paging.total_page_items > 0 ) {
				for (i=0; i < data.paging.total_page_items; i++) {
					...
					$row.find('a').data('data',result).dblclick(function(){
						mgTides.windowEdit({id: $(this).data('data')._id.$id,data: $(this).data('data')});
	...
	windowNew: function(p){
		...
		K.Window({
			id: "windowNew",
			title: "Agregar Tipo de Identidad",
			contentURL: "mg/tiden/edit",
			width : 400,
			height : 70,
			maximizable : false,
			buttons: {
	...
	windowEdit: function(p){
		K.Window({
			id: "windowEdit"+p.id,
			title: "Actualizar Tipo de Identidad",
			contentURL: "mg/tiden/edit",
			width : 400,
			height : 80,
			maximizable : false,
			buttons: {
				"Guardar": function(){
					...
					$('#'+p.$w.attr('id')).dialog('widget').find('.ui-dialog-buttonpane button:first').button('disable');
					$.post("mg/tiden/update",data,function(){
						K.notification({text: "Registro Guardado",layout:"topLeft"});
	...
	windowDelete: function(p){
		new K.Modal({
			id: 'windowDelete',
			title: 'Eliminar Tipo de Identidad '+p.nomb,
			content: '&iquest;Desea <b>eliminar</b> tipo de identidad <strong>'+p.nomb+'</strong>&#63;',
			type: 'modal',
			width: 350,
			height: 40,
			...
			buttons: {
				...
				$.post("mg/tiden/delete",'id='+p.id,function(){
