	...
	init: function(){
		if($('#pageWrapper [child=aldili]').length<=0){
			...
			$.post('al/navg/dili',function(data){
				...
				$p.find('[name=alDiliSusp]').click(function(){ alDiliSusp.init(); }).addClass('ui-state-highlight');
			},'json');
		}
		K.initMode({
			mode: 'al',
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				...
				$mainPanel.find('[name=obj]').html( 'diligencia(s) suspendida(s)' );
				...
				$mainPanel.find('#rbtnSelectJ').click(function(){
					...
					alDiliSusp.loadData({page: 1,url: 'al/dili/lista'});
				});
				...
				$mainPanel.find('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
						...
						$("#mainPanel .gridBody").empty();
						alDiliSusp.loadData({page: 1,url: 'al/dili/search'});
					}
				...
				for (i=0; i < data.paging.total_page_items; i++) {
					...
					.contextMenu("conMenAlDili", {
							onShowMenu: function(e, menu) {
								...
								$('#conMenAlDili_edi,#conMenAlDili_susp,#conMenAlDili_verExpd',menu).remove();
								return menu;
							},
							bindings: {
