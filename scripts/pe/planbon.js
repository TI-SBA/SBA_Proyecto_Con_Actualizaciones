...
pePlanBon = {
	...
	init: function(){
		if($('#pageWrapper [child=plan]').length<=0){
			...
			$.post('pe/navg/plan',function(data){
				...
				$p.find('[name=pePlanBon]').click(function(){ pePlanBon.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'pePlanBon',
			titleBar: {
				title: 'Liquidaciones por Beneficios Sociales'
			}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				$('#pageWrapperLeft [name=pePlanBon]').addClass('ui-state-highlight');
				$('#No-Results').hide();
				$mainPanel = $('#mainPanel');
				$mainPanel.find('label:first').remove();
				$mainPanel.find('[name=periodo]').remove();
				...
				$mainPanel.find('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
						...
						pePlanBon.loadData({page: 1,url: 'pe/docs/lista'});
					}else{
						$("#mainPanel .gridBody").empty();
						pePlanBon.loadData({page: 1,url: 'pe/docs/search'});
					}
	    ...
	    $.post(params.url, params, function(data){
			...
			if ( data.paging.total_page_items > 0 ) { 
				for (i=0; i < data.paging.total_page_items; i++) {
					...
					$li = $('li',$row);
					$li.eq(0).css('background',pePlanBon.states[result.estado].color).addClass('vtip').attr('title',pePlanBon.states[result.estado].descr);
					...
					$row.find('a').data('id',result._id.$id).dblclick(function(){
						pePlanBon.windowDetails({id: $(this).data('id'),nomb: $(this).find('li:eq(2)').html()});
						...
						bindings: {
							'conMenPeSubs_ver': function(t) {
								pePlanBon.windowDetails({id: K.tmp.data('id'),nomb: K.tmp.find('li:eq(2)').html()});
							},
	...
	windowNew: function(p){
		...
		new K.Window({
			id: 'windowNewBon',
			title: 'Liquidaci&oacute;n Compensaci&oacute;n por Tiempo de Servicio y Vacacional',
			contentURL: 'pe/docs/bon_edit',
	...
	windowDetails: function(p){
		new K.Window({
			...
			buttons: {
				"Imprimir": function(){
					//pePlanBon.windowPrint({id: p.id,nomb: p.nomb});
					K.windowPrint({
						id:'windowPeBolePrint',
						title: "Boleta",
