			...
			onContentLoaded: function(){
				...
				p.$l.find('[name=btnTra]').click(function(){
					ciSearch.windowSearchEnti({callback: function(data){
						...
						$.post('pe/lice/get','enti='+data._id.$id,function(data){
							if(data!=null){
								p.asig = [];
				...
				p.$l.find('[name=btnGuardar]').click(function(){
					...
					$.post('pe/lice/save',{
						...
						K.clearNoti();
						K.notification({title: 'Licencias guardadas!',text: 'Se realiz&oacute; con &eacute;xito el guardado de bloques programados para '+ciHelper.enti.formatName(p.enti)+'!'});
	...
	edit: function(p){
		...
		new K.Modal({
			...
			onContentLoaded: function(){
				...
				$.post('pe/tipo/lice',function(data){
