...
pePlanTre = {
	...
	init: function(){
		if($('#pageWrapper [child=plan]').length<=0){
			...
			$.post('pe/navg/plan',function(data){
				...
				$p.find('[name=pePlanTre]').click(function(){ pePlanTre.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'pePlanTre',
			titleBar: {
				title: 'Bonificaci&oacute;n de 30 a&ntilde;os'
			}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				$('#pageWrapperLeft [name=pePlanTre]').addClass('ui-state-highlight');
				$('#No-Results').hide();
				$mainPanel = $('#mainPanel');
				...
				$mainPanel.find('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
						...
						pePlanTre.loadData({page: 1,url: 'pe/docs/lista_tre'});
					}else{
						$("#mainPanel .gridBody").empty();
						pePlanTre.loadData({page: 1,url: 'pe/docs/search_tre'});
					}
	    ...
	    $.post(params.url, params, function(data){
			...
			if ( data.paging.total_page_items > 0 ) { 
				for (i=0; i < data.paging.total_page_items; i++) {
					...
					$li = $('li',$row);
					$li.eq(0).css('background',pePlanTre.states[result.estado].color).addClass('vtip').attr('title',pePlanTre.states[result.estado].descr);
					...
					$row.find('a').data('id',result._id.$id).dblclick(function(){
						pePlanTre.windowDetails({id: $(this).data('id'),nomb: $(this).find('li:eq(2)').html()});
						...
						bindings: {
							'conMenPeSubs_ver': function(t) {
								pePlanTre.windowDetails({id: K.tmp.data('id'),nomb: K.tmp.find('li:eq(2)').html()});
							},
	...
	windowNew: function(p){
		...
		$.extend(p,{
			...
			calcTot: function(){
				...
				p.$w.find('[name=neto_pagar]').html(ciHelper.formatMon(parseFloat(p.$w.find('[name=neto_pagar]').html())*3));
			}
		});
		new K.Window({
			id: 'windowNewVei',
			title: 'Compensaci&oacute;n de 30 a&ntilde;os',
			...
			buttons: {
				"Guardar": function(){
					...
					data.neto = parseFloat(data.neto)*3;
					K.sendingInfo();
	...
	windowDetails: function(p){
		new K.Window({
			...
			buttons: {
				"Imprimir": function(){
					//pePlanTre.windowPrint({id: p.id,nomb: p.nomb});
					K.windowPrint({
						id:'windowPeBolePrint',
						title: "Boleta",
