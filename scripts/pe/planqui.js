...
pePlanQui = {
	...
	init: function(){
		if($('#pageWrapper [child=plan]').length<=0){
			...
			$.post('pe/navg/plan',function(data){
				...
				$p.find('[name=pePlanQui]').click(function(){ pePlanQui.init(); }).addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'pePlanQui',
			titleBar: {
				title: 'Liquidaci&oacute;n de 5&deg; Categor&iacute;a'
			}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				$('#pageWrapperLeft [name=pePlanQui]').addClass('ui-state-highlight');
				$('#No-Results').hide();
				$mainPanel = $('#mainPanel');
				...
				$mainPanel.find('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
						...
						pePlanQui.loadData({page: 1,url: 'pe/docs/lista_qui'});
					}else{
						$("#mainPanel .gridBody").empty();
						pePlanQui.loadData({page: 1,url: 'pe/docs/search_qui'});
					}
	    ...
	    $.post(params.url, params, function(data){
			...
			if ( data.paging.total_page_items > 0 ) { 
				for (i=0; i < data.paging.total_page_items; i++) {
					...
					$li = $('li',$row);
					$li.eq(0).css('background',pePlanQui.states[result.estado].color).addClass('vtip').attr('title',pePlanQui.states[result.estado].descr);
					...
					$row.find('a').data('id',result._id.$id).dblclick(function(){
						pePlanQui.windowDetails({id: $(this).data('id'),nomb: $(this).find('li:eq(2)').html()});
						...
						bindings: {
							'conMenPeSubs_ver': function(t) {
								pePlanQui.windowDetails({id: K.tmp.data('id'),nomb: K.tmp.find('li:eq(2)').html()});
							},
		...
		new K.Window({
			...
			buttons: {
				"Guardar": function(){
					...
					data.neto = parseFloat(data.neto);
					K.sendingInfo();
			...
			onContentLoaded: function(){
				...
				p.$w.find('[name=ini]').replaceWith('<input type="text" name="ini">');
				p.$w.find('[name=ini]').datepicker();
				p.$w.find('[name=btnSelEnt]').click(function(){
					ciSearch.windowSearchEnti({callback: function(data){
						...
						if(data.roles.trabajador.fecing!=null){
							p.$w.find('[name=ini]').val(ciHelper.dateFormatBD(data.roles.trabajador.fecing));
						}
	...
	windowDetails: function(p){
		new K.Window({
			...
			buttons: {
				"Imprimir": function(){
					//pePlanQui.windowPrint({id: p.id,nomb: p.nomb});
					K.windowPrint({
						id:'windowPeBolePrint',
						title: "Boleta",
