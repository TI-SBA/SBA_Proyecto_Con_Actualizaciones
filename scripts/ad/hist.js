						...
						$row.data('id',data._id.$id).data('data',data).data('estado',data.estado).contextMenu("conMenMhEvol", {
							...
							bindings: {
								'conMenMhEvol_evol': function(t) {
									adHist.windowEdit({id: K.tmp.data('id'),paciente: K.tmp.data('data').paciente, clin:K.tmp.data('data').clin});
								},
								'conMenMhEvol_eli': function(t) {
									ciHelper.confirm('&#191;Desea <b>Eliminar</b> la Historia Clinica:  <b>'+K.tmp.find('td:eq(2)').html()+'</b>&#63;',
									function(){
										K.sendingInfo();
										$.post('ad/hist/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenMhEvol_info':function(t){
									K.windowPrint({
										id:'windowPrint',
										title:"Historia Clinica",
										url:"ad/hist/print?_id="+K.tmp.data('id')
									});
								},
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								$.post("ad/hist/save",data,function(result){
									K.clearNoti();
	...
	windowEdit: function(p){
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				new K.grid({
						...
						onContentLoaded: function($el){
							$el.find('button').click(function(){
								...
								'</select></td>');
								*/
								$row.append('<td><input type="text" class="form-control" name="tipo" disabled="disabled" /></td>');
				...
				K.block();
				$.post('ad/hist/get',{_id: p.id},function(data){
