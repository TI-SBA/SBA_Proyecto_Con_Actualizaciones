			...
			onContentLoaded: function(){
		   		var $grid = new K.grid({
					...
					fill: function(data,$row){
						...
						var tipo = '--';
						if(data.tipo_hosp){
							tipo = adHospi.tipos[data.tipo_hosp];
						}
						...
						$row.data('id',data._id.$id).data('data',data).dblclick(function(){
							K.WindowPrint({
								id:'windowPrint',
								title:"Informe ",
								url:"ad/hospi/if_fron?_id="+$(this).data('id')
							});
							...
							bindings: {
								'conMenFMedica_info': function(t) {
									adHospi.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenFMedica_edi': function(t) {
									adHospi.windowEdit({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								...
								'conMenFMedica_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('ad/hospi/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenFhospica_info':function(t){
									K.windowPrint({
										...
										url:"ad/hospi/print?_id="+K.tmp.data('id')
									});
								},'conMenFMedica_info':function(t){
									K.windowPrint({
										id:'windowPrint',
		...
		adPaci.windowSelect({callback: function(paci){
			new K.Panel({
						...
						f: function(){
							...
							var form = ciHelper.validator(p.$w.find('form'),{
								onSuccess: function(){
									...
									$.post("ad/hospi/save",data,function(result){
				...
				onContentLoaded: function(){
					...
					$.post('ad/hospi/get_ingreso',{_id:paci.paciente._id.$id}, function(data){
						var ing = 1;
						if(data!=null){
		...
		new K.Panel({ 
					...
					f: function(){
						...
						*/
						...
						if(data.cie10==''){
							...
							if(data.domi==''){
							p.$w.find('[name=domi]').focus();
							return K.msg({title: ciHelper.titles.infoReq,text: 'Debe ingresar el campo!',type: 'error'});

						}
							if(data.reli==''){
							p.$w.find('[name=reli]').focus();
							return K.msg({title: ciHelper.titles.infoReq,text: 'Debe ingresar el campo!',type: 'error'});
						}
							if(data.es_civil==''){
							p.$w.find('[name=es_civil]').focus();
							return K.msg({title: ciHelper.titles.infoReq,text: 'Debe ingresar el campo!',type: 'error'});
						}
							if(data.ocup==''){
							p.$w.find('[name=ocup]').focus();
							return K.msg({title: ciHelper.titles.infoReq,text: 'Debe ingresar el campo!',type: 'error'});
						}
							if(data.trab==''){
							p.$w.find('[name=trab]').focus();
							return K.msg({title: ciHelper.titles.infoReq,text: 'Debe ingresar el campo!',type: 'error'});
						}
							if(data.deso==''){
							p.$w.find('[name=deso]').focus();
							return K.msg({title: ciHelper.titles.infoReq,text: 'Debe ingresar el campo!',type: 'error'});
						}
						if(data.resi==''){
							p.$w.find('[name=resi]').focus();
							return K.msg({title: ciHelper.titles.infoReq,text: 'Debe ingresar el campo!',type: 'error'});
						}
						if(data.instr==''){
							p.$w.find('[name=instr]').focus();
							return K.msg({title: ciHelper.titles.infoReq,text: 'Debe ingresar el campo!',type: 'error'});
						}
						if(data.idio==''){
							p.$w.find('[name=idio]').focus();
							return K.msg({title: ciHelper.titles.infoReq,text: 'Debe ingresar el campo!',type: 'error'});
						}
						*/
						if(data.refe==''){
			...
			onContentLoaded: function(){
			    ...
			    $.post('ad/hospi/get',{_id: p.id},function(data){
					...
					p.$w.find('[name=fegr]').val(moment(data.fecfin.sec,'X').format('YYYY-MM-DD'));
