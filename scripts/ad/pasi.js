					...
					fill: function(data,$row){
						...
						$row.data('id',data._id.$id).data('data',data).dblclick(function(){
							K.WindowPrint({
								id:'windowPrint',
								title:"Informe ",
								url:"mh/pasi/if_fron?_id="+$(tº).data('id')
							});
							...
							bindings: {
								'conMenPDiario_info': function(t) {
									mhPasi.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenPDiario_edi': function(t) {
									mhPasi.windowEdit({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								'conMenPDiario_cons': function(t) {
									mhPasi.windowConsultaMedica({id: K.tmp.data('id'),padi: K.tmp.data('data')});
								},
								...
								'conMenPDiario_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('mh/pasi/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenFpadica_info':function(t){
									K.windowPrint({
										id:'windowPrint',
										title:"Parte Diario Psicologico",
										url:"mh/pasi/print?_id="+K.tmp.data('id')
									});
								},'conMenPDiario_info':function(t){
									K.windowPrint({
										id:'windowPrint',
	...
	windowNew: function(p){
		...
		new K.Modal({
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								$.post("mh/pasi/save",data,function(result){
									K.clearNoti();
			...
			onContentLoaded: function(){
				...
				$.post("mh/pasi/get_parte",function(data){
	...
	windowEdit: function(p){
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				$.post('mh/pasi/get',{_id: p.id},function(data){
		...
		new K.Panel({
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								p.$w.find('#div_buttons button').attr('disabled','disabled');
								$.post("mh/pasi/save_diario",data,function(result){
									K.clearNoti();
			...
			onContentLoaded: function(){
				...
				p.$w.find("[name=btnDoct]").click(function(){
					mhPasi.windowSelect({callback: function(data){
				...
				var cuar = 0;
				K.block();
				$.post('mh/pasi/get',{_id: p.id,all:true},function(data){
					p.padi = data;
					...
					for(var i=0;i<p.padi.consulta.length;i++){
						...
						$row.append('<td>'+mhPasi.estadoConsulta[p.padi.consulta[i].esta]+'</td>');
						...
						$row.find('[name=btnEli]').click(function(){
							...
							function(){
								K.sendingInfo();
								$.post('mh/pasi/delete_consulta',{index:$_row.data('i'), _id: p.id}, function(rpta){
									$_row.remove();
								},'json')
							},function(){
								$.noop();
