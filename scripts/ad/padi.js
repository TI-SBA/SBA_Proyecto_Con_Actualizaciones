					...
					fill: function(data,$row){
						...
						$row.data('id',data._id.$id).data('data',data).dblclick(function(){
							K.WindowPrint({
								id:'windowPrint',
								title:"Informe ",
								url:"ad/padi/if_fron?_id="+$(tº).data('id')
							});
							...
							bindings: {
								'conMenPDiario_info': function(t) {
									adPadi.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenPDiario_edi': function(t) {
									adPadi.windowEdit({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								'conMenPDiario_cons': function(t) {
									adPadi.windowConsultaMedica({id: K.tmp.data('id'),padi: K.tmp.data('data')});
								},
								...
								'conMenPDiario_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('ad/padi/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenFpadica_info':function(t){
									K.windowPrint({
										id:'windowPrint',
										title:"Parte Diario",
										url:"ad/padi/print?_id="+K.tmp.data('id')
									});
								},'conMenPDiario_info':function(t){
									K.windowPrint({
										id:'windowPrint',
	...
	windowNew: function(p){
		...
		new K.Modal({
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								$.post("ad/padi/save",data,function(result){
									K.clearNoti();
			...
			onContentLoaded: function(){
				...
				$.post("ad/padi/get_parte",function(data){
	...
	windowEdit: function(p){
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				$.post('ad/padi/get',{_id: p.id},function(data){
	...
	windowConsultaMedica: function(p){
		...
		new K.Panel({
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								p.$w.find('#div_buttons button').attr('disabled','disabled');
								$.post("ad/padi/save_diario",data,function(result){
									K.clearNoti();
			...
			onContentLoaded: function(){
				...
				var cuar = 0;
				K.block();
				$.post('ad/padi/get',{_id: p.id,all:true},function(data){
					p.padi = data;
					...
					for(var i=0;i<p.padi.consulta.length;i++){
						...
						$row.append('<td>'+adPaci.sexo[p.padi.consulta[i]._paciente.sexo]+'</td>');
						$row.append('<td>'+edad+'</td>');
						$row.append('<td>'+adPadi.estadoConsulta[p.padi.consulta[i].esta]+'</td>');
						$row.append('<td>'+p.padi.consulta[i].cie10+'</td>');
						$row.append('<td>'+adPaci.categoria[p.padi.consulta[i].cate]+'</td>');
						...
						$row.find('[name=btnEli]').click(function(){
							...
							function(){
								K.sendingInfo();
								$.post('ad/padi/delete_consulta',{index:$_row.data('i'), _id: p.id}, function(rpta){
									$_row.remove();
								},'json')
							},function(){
								$.noop();
...
define(
	['ad/paci','ad/dini','ad/cons','mh/dini'],
	function(adpaci,addini,adCons,mhDini){
		return adPadi;
	}
);