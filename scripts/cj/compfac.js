			...
			onContentLoaded: function(){
				...
				$mainPanel.find('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
						...
						cjCompFac.loadData({page: 1,url: 'cj/comp/lista'});
					}else{
						$("#mainPanel .gridBody").empty();
						cjCompFac.loadData({page: 1,url: 'cj/comp/search'});
					}
			...
			if ( data.paging.total_page_items > 0 ) { 
				for (i=0; i < data.paging.total_page_items; i++) {
					...
					$row.find('a').data('id',result._id.$id).dblclick(function(){
						cjCompFac.windowDetails({id: $(this).data('id'),nomb: $(this).find('li:eq(3)').html()});
						...
						bindings: {
							'conMenCjComp_imp': function(t) {
								K.windowPrint({
									...
									title: "Factura",
									url: "cj/comp/print_fact?id="+K.tmp.data('id')
								});
							},
