...
prPlanEjec = {
	...
	init: function(){
		...
		new K.Panel({
			...
			contentURL: 'pr/plan/index_ejec_v1',
			onContentLoaded: function(){
				...
				$mainPanel.find('[name=btnImprimir]').click(function(){	
					...
					var url = 'pr/plan/ejec_print_v1?'+$.param(params);
					...
					$mainPanel.find('#rbtnOrgaSelect').click(function(){
						...
						*/
						mgProg.windowSelect({callback: function(data){
							...
							$mainPanel.find('[name=organomb]').html(data.nomb);
							$('#mainPanel .gridBody').empty();
							prPlanEjec.loadData({page: 1,url: 'pr/plan/ejec_lista_v1'});
						}});
					});
				...
				$mainPanel.find('[name=btnAgregar]').click(function(){
					...
					if($mainPanel.find('[name=programa]').val()!=""){
						if($mainPanel.find('.gridBody .item').length>0){
							...
							//console.log("Programa del trabajador: "+ K.session.enti.roles.trabajador.programa._id.$id);
							//console.log("Programa actual: "+ $mainPanel.find('.gridBody .item').data('eval').programa._id.$id);
							//console.log("Estado del programa actual: "+ $mainPanel.find('.gridBody .item').data('eval').estado);
							if(K.session.enti.roles.trabajador.programa._id.$id!=$mainPanel.find('.gridBody .item').data('eval').programa._id.$id || $mainPanel.find('.gridBody .item').data('eval').estado=="C"){
									...
									if($mainPanel.find('.gridBody .item').data('eval').dificultades!=""){
										if($mainPanel.find('.gridBody .item').data('eval').recomedaciones!=""){
											if($mainPanel.find('.gridBody .item').data('eval').logros!=""){
												...
												data.estado = "C";
												K.sendingInfo();
												//$.post('pr/plan/save_create_eval_v1',data,function(){
												$.post('pr/plan/save_create_eval_v1',data,function(){
													K.clearNoti();
				...
				$mainPanel.find('[name=btnImprimirEva]').click(function(){	
					...
					var url = 'pr/plan/planevac_print_v1?'+$.param(params);
					K.windowPrint({
						id:'windowPlanEjecPrint',
						title: "Reporte / Informe",
						url: url
	...
	windowDetails: function(p){
		new K.Window({
			...
			onContentLoaded: function(){
				...
				p.$w.find("[name=programa]").html(p.data.programa.nomb);
