	...
	init: function(){
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				...
				$mainPanel.find('[name=prognomb]').html("Programa");
				...
				*/
				//PROGRAMA
				$mainPanel.find('[name=FilProg]').buttonset();
				$mainPanel.find('#rbtnProgSelect').click(function(){
					mgProg.windowSelect({callback: function(data){
						$mainPanel.find('[name=programa]').val(data._id.$id);
						$mainPanel.find('[name=prognomb]').html(data.nomb);
						$('#mainPanel .gridBody').empty();
						prPresAper.loadData({
							page: 1,
							url: 'pr/pres/aper_lista_v1'
				...
				$mainPanel.find('#rbtnProgX').click(function(){
						$mainPanel.find('[name=programa]').val("");
						$mainPanel.find('[name=prognomb]').html("Programa");
						$('#mainPanel .gridBody').empty();
						prPresAper.loadData({
							page: 1,
			...
			onContentLoaded: function(){
				...
				*/
				...
				p.$w.find('[name=btnClas]').click(function(){
					prClas.windowSelect({callback: function(data){
						if (data.clasificadores == null){
							return K.notification({title: 'Error de Selecci&oacute;n',text: 'Este clasificador no cuenta con niveles anteriores o posteriores!!',type: 'error'});
						} else if(data.clasificadores.hijos!=null){
