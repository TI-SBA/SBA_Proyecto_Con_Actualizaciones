mhPajo = {
	...
	init: function(){
		K.initMode({
			...
			action: 'mhPajo',
			titleBar: {
				title: 'Pacientes Hospitalizados Adicciones'
			}
		});
		new K.Panel({
			onContentLoaded: function(){
		   		var $grid = new K.grid({
					...
					fill: function(data,$row){
						$row.append('<td>');
						$row.append('<td>'+mhPajo.states[data.estado].label+'</td>');
						$row.append('<td>'+data.hist_cli+'</td>');
						$row.append('<td>'+mgEnti.formatName(data.paciente)+'</td>');
						$row.append('<td>'+data.pabellon+'</td>');
						$row.append('<td>'+mhPajo.sala[data.sala]+'</td>');
						$row.append('<td>'+mhPajo.Categoria[data.cate]+'</td>');
						...
						$row.data('id',data._id.$id).data('data',data).dblclick(function(){
							K.WindowPrint({
								id:'windowPrint',
								title:"Informe ",
								url:"mh/pajo/if_fron?_id="+$(this).data('id')
							});
							...
							bindings: {
								'conMenCon_Paciente_info': function(t) {
									mhPajo.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenCon_Paciente_edi': function(t) {
									mhPajo.Mover({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								'conMenCon_Paciente_alta': function(t) {
									mhPajo.Alta({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(2)').html()});
								},
								'conMenCon_Paciente_eli': function(t) {
									ciHelper.confirm('&#191;Desea <b>Eliminar</b> la Hospitalizacion:  <b>'+K.tmp.find('td:eq(2)').html()+'</b>&#63;',
									function(){
										K.sendingInfo();
										$.post('mh/pajo/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenFhospica_info':function(t){
									K.windowPrint({
										...
										url:"mh/pajo/print?_id="+K.tmp.data('id')
									});
								},'conMenCon_Paciente_info':function(t){
									K.windowPrint({
										id:'windowPrint',
	...
	windowNew: function(p){
		if(p==null) p={};
		mhHospi.windowSelect({callback: function(paciente){
			new K.Panel({
				title: 'Nuevo Internamiento',
				contentURL: 'mh/pajo/edit',
				store:false,
				buttons: {
					"Guardar": {
						icon: 'fa-save',
						...
						f: function(){
							...
							var form = ciHelper.validator(p.$w.find('form'),{
								onSuccess: function(){
									...
									$.post("mh/pajo/save",data,function(result){
		...
		new K.Panel({
			...
			buttons: {
				...
				onContentLoaded: function(){
					...
					K.block();
					$.post('mh/pajo/get',{_id: p.id},function(data){
