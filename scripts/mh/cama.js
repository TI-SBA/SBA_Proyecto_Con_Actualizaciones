						...
						$row.data('id',data._id.$id).dblclick(function(){
							...
							bindings: {
								'conMenMhCama_edi': function(t) {
									mhCama.windowDetails({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(2)').html()});
								},							
								'conMenMhCama_eli': function(t) {
									ciHelper.confirm('&#191;Desea <b>Eliminar</b> la Ficha Medica:  <b>'+K.tmp.find('td:eq(2)').html()+'</b>&#63;',
									function(){
										K.sendingInfo();
										$.post('mh/cama/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
			...
			buttons: {
				"Guardar": {
					...
					f: function(){
						...
						$.post("mh/cama/save",data,function(result){
							K.clearNoti();
	...
	windowEdit: function(p){
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				$.post('mh/cama/get',{_id: p.id},function(data){
