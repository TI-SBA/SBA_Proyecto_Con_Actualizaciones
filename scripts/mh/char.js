						...
						$row.data('id',data._id.$id).data('data',data).data('estado',data.estado).contextMenu("conMenFMedica", {
							...
							bindings: {
								'conMenFMedica_info': function(t) {
									mhChar.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenFMedica_edi': function(t) {
									mhChar.windowEdit({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								...
								'conMenFMedica_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('mh/char/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenFMedica_info':function(t){
									K.windowPrint({
										id:'windowPrint',
										title:"Charlas",
										url:"mh/char/print?_id="+K.tmp.data('id')
									});
								},
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								$.post("mh/char/save",data,function(result){
									K.clearNoti();
		...
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				K.block();
				    ...
				    $.post('mh/char/get',{_id: p.id},function(data){
								...
								atmh:p.$w.find('[name=atmh]').val(data.atmh);
