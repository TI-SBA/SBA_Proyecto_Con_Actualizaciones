mhConpa = {
						...
						$row.data('id',data._id.$id).data('data',data).data('estado',data.estado).contextMenu("conMenCon_Paciente", {
							...
							bindings: {
								'conMenCon_Paciente_info': function(t) {
									mhConpa.windowDetails({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
				 				'conMenCon_Paciente_edi': function(t) {
									mhConpa.windowEdit({id: K.tmp.data('id'),nom: K.tmp.find('td:eq(2)').html()});
								},
								...
								'conMenCon_Paciente_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('mh/conpa/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenCon_Paciente_info':function(t){
									K.windowPrint({
										id:'windowPrint',
										title:"Control de Pacientes",
										url:"mh/conpa/print?_id="+K.tmp.data('id')
									});
								},
	...
	windowNew: function(p){
		if(p==null) p = {};
		//mhPaci.windowSelect({callback: function(paciente){
			new K.Panel({
			title: 'Nueva Hospitalizacion de Pacientes',
			contentURL: 'mh/conpa/edit',
			store:false,
			buttons: {
				"Guardar": {
					icon: 'fa-save',
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								$.post("mh/conpa/save",data,function(result){
									K.clearNoti();
	...
	windowEdit: function(p){
		...
		new K.Panel({ 
			...
			onContentLoaded: function(){
				...
				K.block();
				$.post('mh/conpa/get',{_id: p.id},function(data){
