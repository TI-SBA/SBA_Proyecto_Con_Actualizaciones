			...
			onContentLoaded: function(){
		   		var $grid = new K.grid({
					...
					fill: function(data,$row){
						...
						$row.append('<td>'+mhCmmo.estado[data.estado]+'</td>');
						
						$row.data('id',data._id.$id).dblclick(function(){
							mhCmmo.windowDetails({_id: $(this).data('id'),nomb: $(this).find('td:eq(2)').html()});
						}).data('estado',data.estado).contextMenu("conMenMhMov", {
							bindings: {
								'conMenMhMov_ingr': function(t) {
									mhCmmo.windowIngreso({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(2)').html()});
								},
								'conMenMhMov_movi': function(t) {
									mhCmmo.windowTraslado({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(2)').html()});
								},							
		...
		new K.Modal({ 
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								var data = {
									//DATOS DEL cmmoENTE
									_id: p.id,
									paciente: mhHospi.dbRel(p.$w.find('[name=paciente]').data('data')),
									cama:{
								...
								$.post("mh/cmmo/paciente",data,function(result){
									K.clearNoti();
			...
			onContentLoaded: function(){
				...
				p.$w.find("[name=btnDiag]").click(function(){
					mhHospi.windowSelect({callback: function(data){
						p.$w.find('[name=paciente]').html(mgEnti.formatName(data.paciente)).data('data',data);				
					},bootstrap: true});
				});
				K.block();
				$.post('mh/cmmo/get',{_id: p.id},function(data){
					p.$w.find('[name=paciente]').html(mgEnti.formatName(data.paciente)).data('data',data);
	...
	windowTraslado: function(p){
		new K.Modal({ 
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								p.$w.find('#div_buttons button').attr('disabled','disabled');
								$.post("mh/cmmo/traslado",data,function(result){
			...
			onContentLoaded: function(){
				p.$w = $('#windowTraslado');
				p.$w.find("[name=btnDiag]").click(function(){
					mhCama.windowSelect({callback: function(data){
						p.$w.find('[name=paciente]').html(mgEnti.formatName(data.paciente)).data('data',data);				
