						...
						$row.data('id',data._id.$id).data('data',data).data('estado',data.estado).contextMenu("conMenMhEvol", {
							...
							bindings: {
								'conMenMhEvol_evol': function(t) {
									mhCapu.windowEdit({id: K.tmp.data('id'),paciente: K.tmp.data('data').paciente, clin:K.tmp.data('data').clin});
								},
								'conMenMhEvol_eli': function(t) {
									ciHelper.confirm('&#191;Desea <b>Eliminar</b> la Historia Clinica:  <b>'+K.tmp.find('td:eq(2)').html()+'</b>&#63;',
									function(){
										K.sendingInfo();
										$.post('mh/capu/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
								...
								'conMenMhEvol_info':function(t){
									K.windowPrint({
										...
										url:"mh/capu/print?_id="+K.tmp.data('id')
									});
								},
					...
					f: function(){
						...
						var form = ciHelper.validator(p.$w.find('form'),{
							onSuccess: function(){
								...
								$.post("mh/capu/save",data,function(result){
									K.clearNoti();
	...
	windowEdit: function(p){
		new K.Modal({ 
			...
			onContentLoaded: function(){
				...
				K.block();
				$.post('mh/capu/get',{_id: p.id},function(data){
