		...
		new K.Panel({
			onContentLoaded: function(){
		   		var $grid = new K.grid({
					...
					data: 'ho/hosp/lista',
					params: {estado: 'P',pend: true,modulo:'AD'},
					...
					fill: function(data,$row){
						...
						$row.data('id',data._id.$id).dblclick(function(){
							haPend.windowDetails({_id: $(this).data('id'),nomb: $(this).find('td:eq(2)').html()});
							...
							bindings: {
								'conMenHoHosp_edi': function(t) {
									if(K.session.enti.roles.cajero==null){
										...
										haPend.windowEdit({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(2)').html(),goBack: function(){
											haPend.init();
										}});
									}
								},
								...
								'conMenHoHosp_eli': function(){
									...
									function(){
										K.sendingInfo();
										$.post('ha/hosp/delete',{_id: K.tmp.data('id')},function(){
											K.clearNoti();
	...
	windowEdit: function(p){
		...
		new K.Panel({
			...
			buttons: {
				"Guardar": {
					...
					f: function(){
						...
						$.post("ha/hosp/save_rec",data,function(result){
							K.clearNoti();
			...
			onContentLoaded: function(){
				...
				$.post('ha/hosp/get',{_id: p.id,tarifa: true,recibo: true},function(data){
					p.data = data;
