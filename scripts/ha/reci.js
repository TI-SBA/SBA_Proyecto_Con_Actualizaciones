			...
			onContentLoaded: function(){
		   		var $grid = new K.grid({
					...
					toolbarHTML: '<button name="btnAgregar" class="btn btn-primary"><i class="fa fa-plus"></i> Agregar Recibo</button>'+
						'<button name="btnGen" class="btn btn-success"><i class="fa fa-gears"></i> Generar Recibo de Ingresos</button>',
					...
					fill: function(data,$row){
						$row.append('<td>');
						$row.append('<td>'+haReci.states[data.estado].label+'</td>');
						var en_reemplazo_de = '';
						...
						$row.data('id',data._id.$id).dblclick(function(){
							K.windowPrint({
								...
								url: "ha/reci/print?_id="+$(this).data('id')
							});
							...
							bindings: {
								'conMenMhComp_imp': function(t) {
									K.windowPrint({
										id:'windowcjFactPrint',
										title: "Recibo de Caja",
										url: "ha/reci/print?_id="+K.tmp.data('id')
									});
								},
								'conMenMhComp_anu': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('ha/reci/anular',{_id: K.tmp.data('data')._id.$id},function(){
											K.clearNoti();
								...
								'conMenMhComp_eli': function(t) {
									...
									function(){
										K.sendingInfo();
										$.post('ha/reci/eliminar',{_id: K.tmp.data('data')._id.$id},function(){
											K.clearNoti();
		...
		new K.Panel({
			...
			buttons: {
				"Guardar": {
					...
					f: function(){
						...
						$.post("ha/reci/save",data,function(result){
							K.clearNoti();
							...
							K.windowPrint({
								...
								url: "ha/reci/print?_id="+result._id.$id+"&print=1"
							});
						},'json');
					}
				},
			...
			onContentLoaded: function(){
				...
				p.$w.find('[name=btnCod]').click(function(){
					K.block();
					$.post('ad/paci/get_historia',{cod: p.$w.find('[name=cod]').val()},function(data){
				...
				K.block();
				$.post('ha/reci/next_num',{modulo:'AD'},function(data){
					p.num = data.num;
...
define(
	['mg/enti','ct/pcon','cj/comp','mg/serv','mg/orga','ha/tara','ha/targ','ha/rein'],
	function(mgEnti,ctPcon,cjComp,mgServ,mgOrga,hoTara,hoTarg,hoRein){
		return haReci;
	}
);