			...
			onContentLoaded: function(){
		   		var $grid = new K.grid({
					...
					fill: function(data,$row){
						$row.append('<td>');
						$row.append('<td>'+haHosp.states[data.estado].label+'</td>');
						...
						}).data('tipo',tipo).data('data',data).contextMenu("conMenhaHosp", {
							onShowMenu: function($row, menu) {
								if($row.data('data').estado!='P')
									$('#conMenhaHosp_eli',menu).remove();
								$('#conMenhaHosp_alt',menu).remove();
								return menu;
							},
							bindings: {
								'conMenhaHosp_edi': function(t) {
