		   		...
		   		var $grid = new K.grid({
					...
					toolbarHTML: '<button name="btnAgregar" class="btn btn-success"><i class="fa fa-plus"></i> Agregar Lista de Medicinas para Paciente</button>'+
						'&nbsp;<select class="form-control" name="estado">'+
							...
							'<option value="D">Historico</option>'+
						'</select>',
					...
					fill: function(data,$row){
						...
						$row.data('id',data._id.$id).dblclick(function(){
							haCont.windowDetails({id: $(this).data('id'),nomb: $(this).find('td:eq(1)').html()});
							...
							bindings: {
								'conMenHoCont_ver': function(t) {
									haCont.windowDetails({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(1)').html()});
								},
								'conMenHoCont_aum': function(t) {
									haCont.windowAum({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(1)').html()});
								},
								'conMenHoCont_dis': function(t) {
									haCont.windowDis({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(1)').html()});
								},
								'conMenHoCont_dev': function(t){
									...
									function(){
										K.sendingInfo();
										$.post('ha/cont/save_devolver',{_id: K.tmp.data('id')},function(){
											window.open("ha/cont/print?_id="+K.tmp.data('id'));
											K.clearNoti();
								...
								'conMenHoCont_des': function(t){
									haCont.windowDeshacer({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(1)').html()});
								}
							}
						});
						return $row;
		...
		new K.Modal({ 
			...
			buttons: {
				"Guardar": {
					...
					f: function(){
						...
						$.post("ha/cont/save",data,function(result){
							K.clearNoti();
			...
			onContentLoaded: function(){
				...
				p.$w.find('[name=btnHist]').click(function(){
					K.block();
					$.post('ci/skmmh/search_hist',{cod: p.$w.find('[name=cod_hist]').val()},function(data){
						if(data.nomb!=null){
	...
	windowDis: function(p){
		new K.Modal({ 
			...
			buttons: {
				"Guardar": {
					...
					f: function(){
						...
						K.sendingInfo();
						p.$w.find('#div_buttons button').attr('disabled','disabled');
						$.post("ha/cont/save_dis",data,function(result){
							K.clearNoti();
			...
			onContentLoaded: function(){
				...
				K.block();
				$.post('ha/cont/get',{_id: p.id},function(data){
			...
			buttons: {
				"Guardar": {
					...
					f: function(){
						...
						K.sendingInfo();
						p.$w.find('#div_buttons button').attr('disabled','disabled');
						$.post("ha/cont/save_aum",data,function(result){
							K.clearNoti();
							K.notification({title: ciHelper.titleMessages.regiAct,text: "Control actualizado!"});
							haCont.init();
							K.closeWindow(p.$w.attr('id'));
							...
							new K.Modal({
								...
								onContentLoaded: function(){
									...
									for(var i=0; i<med.hist.length; i++){
										...
										$row.append('<td>'+haCont.tipoMov[hist.tipo]+'</td>');
				...
				$.post('ha/cont/get',{_id: p.id},function(data){
					...
					for(var i=0; i<data.medicinas.length; i++){
						...
						$row.find('button').click(function(){
							...
							function(){
								K.sendingInfo();
								$.post('ha/cont/save_deshacer',{_id: p.id,med: med.med},function(){
									K.clearNoti();
									K.notification({title: 'Medicinas Devueltas',text: 'La deshabilitaci&oacute;n se realiz&oacute; con &eacute;xito!'});
									haCont.init();
								});
