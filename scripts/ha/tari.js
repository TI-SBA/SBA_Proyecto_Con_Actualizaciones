					...
					load: function(data_all,$tbody){
						if(data_all.items!=null){
							for(var i=0; i<data_all.items.length; i++){
								...
								$row = $('<tr class="item">');
								$row.append('<td>');
								$row.append('<td>'+haTari.tipo_hosp[data.tipo_hosp]+'</td>');
								$row.append('<td>'+haTari.categoria[data.categoria]+'</td>');
								...
								$row.data('id',data._id.$id).dblclick(function(){
									haTari.windowDetails({_id: $(this).data('id'),nomb: $(this).find('td:eq(2)').html()});
									...
									bindings: {
										'conMenListEd_ver': function(t) {
											haTari.windowDetails({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(2)').html()});
										},
										'conMenListEd_edi': function(t) {
											haTari.windowEdit({id: K.tmp.data('id'),nomb: K.tmp.find('td:eq(2)').html()});
										},
