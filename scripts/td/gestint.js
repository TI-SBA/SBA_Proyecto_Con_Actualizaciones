	...
	init: function(){
		if($('#pageWrapper [child=gest]').length<=0){
			...
			$.post('td/navg/gest',function(data){
				...
				$p.find('[name=tdGestInt]').click(function(){ tdGestInt.init(); }).find('ul').addClass('ui-state-highlight');
		...
		K.initMode({
			...
			action: 'tdGestInt',
			titleBar: {
				title: 'Cuentas: Gestores Internos',
				toolbar: true
			}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				...
				$mainPanel.find('[name=btnGestor]').click(function(){
					ciCreate.windowNewEntidad({callBack: tdGestInt.init,roles: {gestor: 1}});
				...
				$mainPanel.find('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
						...
						tdGest.loadData({page: 1,url: 'td/gest/lista_trab'});
					}else{
						$("#mainPanel .gridBody").empty();
						tdGest.loadData({page: 1,url: 'td/gest/search'});
					}
				...
				$("#mainPanel .gridHeader").find('li:eq(0),li:eq(1)').click( function(){
					...
					$("#mainPanel .gridBody").empty();
					tdGest.loadData({page: 1,url: 'td/gest/lista_trab',filter: $(this).attr("filter"),order: order});
				});
				tdGest.loadData({page: 1,url: 'td/gest/lista_trab'});
			}
		});
