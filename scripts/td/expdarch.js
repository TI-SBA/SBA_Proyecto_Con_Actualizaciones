...
tdExpdArch = {
	...
	if($('#pageWrapper [child=exps]').length<=0){
		...
		$.post('td/navg/exps',function(data){
			...
			$p.find('[name=tdExpdArch]').click(function(){ tdExpdArch.init(); }).find('ul').addClass('ui-state-highlight');
	...
	K.initMode({
		mode: 'td',
		action: 'tdExpdArch',
		titleBar: { title: 'Expedientes Archivados'}
	});
	
	new K.Panel({
		id: 'mainPanel',
		contentURL: 'td/expd/arch',
		onContentLoaded: function(){
			...
			$li.eq(5).attr('filter','fecreg');
			$("#mainPanel .gridHeader").find('li:eq(2),li:eq(3),li:eq(4),li:eq(5)').click( function(){
				$this = $(this);
				var order = 1;
				...
				tdExpdArch.loadData({page: 1,url: 'td/expd/listaexpdarch',filter: $(this).attr("filter"),order: order});
			});
			...
			$('.divSearch [name=btnBuscar]').click(function(){
				...
				if($('.divSearch [name=buscar]').val().length<=0){
		            $("#mainPanel .gridBody").empty();
		            tdExpdArch.loadData({page: 1,url: 'td/expd/listaexpdarch'});
				}else{
		            $("#mainPanel .gridBody").empty();
		            tdExpdArch.loadData({page: 1,url: 'td/expd/listaexpdarch',bus: $('.divSearch [name=buscar]').val(),ncol: col});
	...
	$.post(params.url, params, function(data){
		...
		if ( data.paging.total_page_items > 0 ) {
			for (i=0; i < data.paging.total_page_items; i++) {
				...
				$d = result.traslados.length-1;
				$li.eq(7).html( ciHelper.dateFormat(result.traslados[$d].origen.fecarc) );
				...
				tdExpd.contextMenu({$row: $row.find('a'),data: $row.find('a').data('data'),band: 'arch'});
