	...
	init: function(){
		if($('#pageWrapper [child=his]').length<=0){
			...
			$.post('td/navg/his',function(data){
				...
				$p.find('[name=tdExpdHistEnvi]').click(function(){ tdExpdHistEnvi.init(); }).find('ul').addClass('ui-state-highlight');
		...
		K.initMode({
			mode: 'td',
			action: 'tdExpdHistEnvi',
			titleBar: { title: 'Hist&oacute;rico de Expedientes: Enviados'}
		});
		...
		new K.Panel({
			...
			onContentLoaded: function(){
				...
				//$li.eq(5).attr('filter','fecenv');
				//$li.eq(6).attr('filter','fecrec');
				$li.eq(5).html("Observaciones");
				$li.eq(6).html("Destino");
				...
				$("#mainPanel .gridHeader").find('li:eq(2),li:eq(3),li:eq(4)').click( function(){
					...
					tdExpdHistEnvi.loadData({page: 1,url: 'td/expd/listaexpdenvi',filter: $(this).attr("filter"),order: order});
				});
				...
				$('.divSearch [name=btnBuscar]').click(function(){
					if($('.divSearch [name=buscar]').val().length<=0){
			            $("#mainPanel .gridBody").empty();
			            tdExpdHistEnvi.loadData({page: 1,url: 'td/expd/listaexpdenvi'});
					}else{
			            $("#mainPanel .gridBody").empty();
			            tdExpdHistEnvi.loadData({page: 1,url: 'td/expd/listaexpdenvi',bus: $('.divSearch [name=buscar]').val(),ncol: col});
		...
		$.post(params.url, params, function(data){
			...
			if ( data.paging.total_page_items > 0 ) {
				for (i=0; i < data.paging.total_page_items; i++) {
					...
					$li.eq(5).html( result.destino.nomb);
					if(result.fecenv==null) $li.eq(6).html( " " );
					else $li.eq(6).html( ciHelper.dateFormat(result.fecenv) );
					if(result.fecrec==null) $li.eq(7).html( " " );
					else $li.eq(7).html( ciHelper.dateFormat(result.fecrec) );
				...
				if (parseFloat(data.paging.page) < parseFloat(data.paging.total_pages)) {
					...
					$moreresults.click( function(){
						...
						tdExpdHistEnvi.loadData(params);
