...
class SchemaDebitNote {
	...
	public function generate($data, $filename, $config){
        ...
        ******************************/
        ...
        $cbc_ID = $DebitNote->appendChild($xml->createElement("cbc:ID", $data['serie']."-".$data['numero']));

        /*Fecha de emisión*/
        $cbc_IssueDate = $DebitNote->appendChild($xml->createElement("cbc:IssueDate", date('Y-m-d',$data['fecemi']->sec)));
        ...
        $DebitNote->appendChild($xml->createElement("cbc:DocumentCurrencyCode", $data['moneda']));// PEN: Nuevos soles | USD: Dolares estadounidanses

        /*****************************
        * DOCUMENTO RELACIONADO
        ******************************/
        $DiscrepancyResponse = $DebitNote->appendChild($xml->createElement("cac:DiscrepancyResponse"));
        $DiscrepancyResponse->appendChild($xml->createElement("cbc:ReferenceID",$data['documento_modificar']['serie'].'-'.$data['documento_modificar']['numero']));//FILL DATA
        ...
        $BillingReference = $DebitNote->appendChild($xml->createElement("cac:BillingReference"));
        ...
        ******************************/
        ...
        $cac_RequestedMonetaryTotal = $DebitNote->appendChild($xml->createElement("cac:RequestedMonetaryTotal"));

        $cbc_PayableAmount = $cac_RequestedMonetaryTotal->appendChild($xml->createElement("cbc:PayableAmount",$data['total']));
        ...
        if(count($data['items'])>0){
            ...
            foreach($data['items'] as $row){
                ...
                $this->addItemLineXml($xml, $DebitNote, $orden_item, $row);
            }
        }
        //print_r($data);die();
        ...
        ************************/
        ...
        $f->response->view('ecom.print',array('data'=>$data,'ruta_pdf'=>$ruta_pdf));
        $xml->save(IndexPath.DS.$ruta_sin_firmar);
        return array(
        	'status'=>'success',
        	'success_message'=>'Se ha creado el XML y el formato PDF del modelo DebitNote',
        	'success_data'=>array(
	...
	function addItemLineXml(&$xml, &$DebitNote, &$orden_item, $data = array()){
        if(floatval($data['valor_unitario'])==0 && floatval($data['valor_gratuito'])==0) return false;
        $cac_InvoiceLine = $DebitNote->appendChild($xml->createElement("cac:DebitNoteLine"));
        ...
        $cbc_InvoicedQuantity = $cac_InvoiceLine->appendChild($xml->createElement("cbc:DebitedQuantity", $data['cant']));//FILL DATA
