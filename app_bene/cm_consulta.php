		...
		*/
			...
			*******************************************************************************************/
			...
			if(isset($data['nomb'])) {
				if(is_null($data['nomb']))	throw new Exception("Error: el campo nomb esta recibiendo un nulo");
				if(empty($data['nomb']))	throw new Exception("Error: el campo nomb esta vacio");
				$find_espacios['ocupantes.nomb']=array('$regex'=> strtoupper($data['nomb']));
			}
			...
			if(!empty($lista)){
					foreach ($lista as $espa) {
						...
						$temp_espa = array(							
							'sector' => $espa['sector'],
							'estado' => $espa['estado'], 
							'ubicacion' => ucwords(strtolower( $espa['nomb'])),
							'tipo'=>$tipo,
						);
						if(isset($espa['ocupantes']) && !empty($espa['ocupantes']) ){
							$ocup=array();
							foreach ($espa['ocupantes'] as $ocupante) {
								$ocupante=array(
									'appat' => ucwords(strtolower($ocupante['appat'])),
									'apmat' => ucwords(strtolower($ocupante['apmat'])),
									'nomb'=>ucwords(strtolower($ocupante['nomb'])),
								);
								array_push($ocup,$ocupante);
							}
							$temp_espa=array_merge($temp_espa,array('ocupantes'=>$ocup));
			...
			****************************************************************************/
			if(isset($data['appat'])){
				$find_espacios['ocupantes.appat']=ucwords(strtolower(($data['appat'])));
			}
			if(isset($data['apmat'])){
				$find_espacios['ocupantes.apmat']=ucwords(strtolower(($data['apmat'])));
			}
			if(isset($data['nomb'])){
				$find_espacios['ocupantes.nomb']=array('$regex'=> ucwords(strtolower(($data['nomb']))));
			}
			$lista = $espacios->find($find_espacios)->limit($limite);
			if(!empty($lista)){
					foreach ($lista as $espa) {
			...
			****************************************************************************/
			if(isset($data['appat']))	{
				$find_espacios['ocupantes.appat']=strtolower($data['appat']);
			}
			if(isset($data['apmat']))	{
				$find_espacios['ocupantes.apmat']=strtolower($data['apmat']);
			}
			if(isset($data['nomb']))	{
				$find_espacios['ocupantes.nomb']=array('$regex'=> strtolower($data['nomb']));
			}
			$lista = $espacios->find($find_espacios)->limit($limite);
			if(!empty($lista)){
				foreach ($lista as $espa) {
