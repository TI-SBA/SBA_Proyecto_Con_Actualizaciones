...
function lista(){
	...
	try{
		...
		if(isset($data['descrip']))	{
			if(is_null($data['descrip']))	throw new Exception("Error: el campo descripcion esta recibiendo un nulo");
			if(empty($data['descrip']))	throw new Exception("Error: el campo descripcion esta vacio");
			$find_espacios['descrip']=$data['descrip'];
		}
		...
		else $lista = $espacios->find($find_espacios);
