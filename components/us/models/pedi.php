<?php
class Model_us_pedi extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->us_pedidos;
	}
	...
	protected function get_semana(){
		global $f;
		$data = $this->db->find(array('ini'=>$this->params['ini']));
		foreach ($data as $obj) {
		    $this->items[] = $obj;
		}
	}
	protected function get_last(){
		global $f;
		$this->items = $this->db->findOne(array('oficina._id'=>$this->params['oficina']));
	}
	...
	protected function save_atender(){
		global $f;
		$this->db->update(array( 'ini' => $this->params['ini'] ),array('estado'=>'A'));
	}
}
?>