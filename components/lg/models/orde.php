<?php
class Model_lg_orde extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->lg_ordenes;
	}
	...
	protected function get_cod(){
		...
		$sort = array();
		if(isset($this->params['etapa'])){
			$sort = array($this->params['etapa'].'.cod'=>-1);
		}
		$cursor = $this->db->find(array(),array('solicitud.cod'=>true,'certificacion.cod'=>true,'orden.cod'=>true,'orden_servicio.cod'=>true,'recepcion.cod'=>true))->sort($sort)->limit(1);
