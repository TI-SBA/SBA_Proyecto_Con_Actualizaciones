<?php
class Controller_in_tipo extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("in/tipo")->params($params)->get("lista") );
	}
	function execute_get(){
		global $f;
		$items = $f->model("in/tipo")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
		$f->response->json( $items );
	}
	function execute_all(){
		global $f;
		$items = $f->model("in/tipo")->get("all")->items;
		$f->response->json( $items );
	}
