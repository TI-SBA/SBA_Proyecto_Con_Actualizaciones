<?php
class Model_in_inmu extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->in_inmuebles;
	}
	...
	protected function get_lista(){
		...
		if(isset($this->params["texto"])){
			if($this->params["texto"]!=''){
				...
				$criteria = $helper->paramsSearch($this->params["texto"], array('direccion','tipo.nomb','sublocal.nomb','trabajador.fullname'));
			}
		}
		$sort = array('nomb'=>1);
	...
	protected function get_all(){
		...
		if(isset($this->params['sublocal'])){
			$filter['sublocal._id'] = $this->params['sublocal'];
		}
		$filter['direccion'] = array('$ne'=>'');
		$filter['estado'] = 'H';
		$data = $this->db->find($filter,$fields)->sort(array('direccion'=>1));
		foreach ($data as $obj) {
		    $this->items[] = $obj;
		}
	}
