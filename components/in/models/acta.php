<?php
class Model_in_acta extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->in_actas_conciliacion;
	}
	...
	protected function get_lista(){
		...
		if(isset($this->params['texto'])){
			if($this->params["texto"]!=''){
				...
				$criteria = $helper->paramsSearch($this->params["texto"], array('arrendatario.nomb','arrendatario.appat','arrendatario.apmat','inmueble.direccion'));
			}
		}
		$sort = array('nomb'=>1);
	...
	protected function get_all(){
		...
		if (isset($this->params['arrendatario'])) {
            $filter['arrendatario._id'] = $this->params['arrendatario'];
        }
