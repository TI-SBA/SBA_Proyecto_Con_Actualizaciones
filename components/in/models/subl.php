<?php
class Model_in_subl extends Model
{
    private $db;
    public $items;
    ...
    {
        global $f;
        $this->db = $f->datastore->in_sublocales;
    }
    ...
    {
        global $f;
        if (isset($this->params["texto"]) && $this->params["texto"]!='') {
            $f->library('helpers');
            $helper=new helper();
    ...
    {
        global $f;
        $data = $this->db->find(array('tipo._id'=>$this->params['tipo']));
        foreach ($data as $obj) {
            $this->items[] = $obj;
        }
    }
