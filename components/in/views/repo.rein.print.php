...
class repo extends FPDF{
	...
	function Publicar($items){
			...
			if($item['monto']!=0||$item['cuenta']['cod']=='1201.0303.42.01'){
				if($cta!=$item['cuenta']['cod']){
					...
					if(substr($cta,0,9)=='1202.0901'){
						...
						if($cta=='1201.99.47'){
							$this->setXY(30,$y);$this->MultiCell(40,$height,'1201.99','0','L');
							$this->setXY(60,$y);$this->MultiCell(100,$height,'CUENTAS POR COBRAR DE DUDOSA RECUPERACION','0','L');
							if($this->GetY()>$max_y_) $max_y_ = $this->GetY();
							$y=$this->GetY();
							$fy=$this->GetY();
						}
						...
						$this->setXY(60,$y);$this->MultiCell(100,$height,$helper->format_word($item['cuenta']['descr']),'0','L');
						if($this->GetY()>$max_y_) $max_y_ = $this->GetY();
						$y=$this->GetY();
						$fy=$this->GetY();
					}
				...
				//}else if($this->recibo['organizacion']['_id']->{'$id'}=='51a50edc4d4a13441100000e'){
					...
					if($this->recibo['tipo_inm']=='A'){
						...
						$width_word = $this->GetStringWidth($helper->format_word($item['comprobante']['cliente']['fullname']));
						if($width_word>60){
							$this->SetFont('Arial','',$size_font-2);
							$this->setXY(30,$y);$this->MultiCell(65,$height,$helper->format_word($item['comprobante']['cliente']['fullname']),'0','L');
							$this->SetFont('Arial','',$size_font);
						}else{
							$this->setXY(30,$y);$this->MultiCell(65,$height,$helper->format_word($item['comprobante']['cliente']['fullname']),'0','L');
						}
					...
					if(isset($item['comprobante']['inmueble'])){
						$width_word = $this->GetStringWidth($helper->format_word($item['comprobante']['inmueble']['abrev']));
						if($width_word>35){
							$this->SetFont('Arial','',$size_font-2);
							$this->setXY($yt_inm,$y);$this->MultiCell(35,$height,$helper->format_word($item['comprobante']['inmueble']['abrev']),'0','L');
							$this->SetFont('Arial','',$size_font);
						}else{	
							$this->setXY($yt_inm,$y);$this->MultiCell(35,$height,$helper->format_word($item['comprobante']['inmueble']['abrev']),'0','L');
						}
					...
					if(isset($item['comprobante']['alquiler'])){
						...
						if(sizeof($item['comprobante']['items'])==1){
							if(isset($item['comprobante']['items'])){
								...
								if(isset($item['comprobante']['items'][0]['dia_ini'])){
									if($item['comprobante']['items'][0]['dia_ini']=='16'){
									...
									}elseif($item['comprobante']['items'][0]['dia_ini']=='1'){
										$mes = intval($item['comprobante']['items'][0]['pago']['mes'])-1;
										$ano = intval(substr($item['comprobante']['items'][0]['pago']['ano'],2));
										if($mes==-1){
											$mes = 11;
										...
										$dia_ini = intval($item['comprobante']['items'][0]['dia_ini']);
										$mes = intval($item['comprobante']['items'][0]['pago']['mes'])-1;
										$ano = intval(substr($item['comprobante']['items'][0]['pago']['ano'],2));
										if($mes==-1){
											$mes = 11;
							...
							$cada = ' cm S/.';
							foreach ($item['comprobante']['items'][0]['conceptos'] as $ik=>$conc_tmp) {
								if($conc_tmp['cuenta']['cod']!='1202.0901.47' && $conc_tmp['cuenta']['cod']!='2101.010503.47'){
									$cada .= $conc_tmp['monto'];
								}
							}
							if($ano_ini==$ano_fin){
								/*$dia_ini = floatval($item['comprobante']['items'][0]['dia_ini']);
								$dia_fin = $dia_ini-1;*/
								if($dia_ini=='16'){
									//$mes_ini--;
									//$mes_fin--;
								}
								$this->setXY($yt_obs,$y);$this->MultiCell(23,$height,$meses[$mes_ini].' a '.$meses[$mes_fin].' \''.$ano_ini.$cada,'0','L');
								//$this->setXY($yt_obs,$y);$this->MultiCell(23,$height,$dia_ini.'/'.$mes_ini.' al '.$dia_fin.'/'.($mes_fin+1).' \''.$ano_ini.$cada,'0','L');
								if($this->GetY()>$max_y_) $max_y_ = $this->GetY();
							}else{
								$this->setXY($yt_obs,$y);$this->MultiCell(23,$height,$meses[$mes_ini].' \''.$ano_ini.' a '.$meses[$mes_fin].' \''.$ano_fin.$cada,'0','L');
								if($this->GetY()>$max_y_) $max_y_ = $this->GetY();
							}
						...
						$_word = "";
						if(count($item['comprobante']['items'][0]['conceptos'])>0){
							for($ii=0;$ii<count($item['comprobante']['items'][0]['conceptos']);$ii++){
								if($item['comprobante']['items'][0]['conceptos'][$ii]['cuenta']['cod']!='2101.010503.47'){
									$_word.=$helper->format_word($item['comprobante']['items'][0]['conceptos'][$ii]['concepto'])." \n";
								}
							}
						}
						//$this->setXY($yt_obs,$y);$this->MultiCell(20,$height,$helper->format_word($item['comprobante']['items'][0]['conceptos'][0]['concepto']),'0','L');
						$this->setXY($yt_obs,$y);$this->MultiCell(20,$height,$helper->format_word($_word),'0','L');
						if($this->GetY()>$max_y_) $max_y_ = $this->GetY();
						$this->SetFont('Arial','',$size_font);
					}elseif(!isset($item['comprobante']['playa'])){
						if(substr($cta,0,7)!='2103.03'){
							$this->SetFont('Arial','',$size_font-2);
							if(isset($item['comprobante']['items'][0]['cuenta_cobrar']['servicio']['abrev']))
								$word = $helper->format_word($item['comprobante']['items'][0]['cuenta_cobrar']['servicio']['abrev']);
							else
								$word = $helper->format_word($item['comprobante']['items'][0]['cuenta_cobrar']['servicio']['nomb']);
							$width_word = $this->GetStringWidth($word);
							if($width_word>26){
								$this->SetFont('Arial','',$size_font-3);
								$this->setXY($yt_obs,$y);$this->MultiCell(26,$height,$word,'0','L');
							}else{
								$this->setXY($yt_obs,$y);$this->MultiCell(26,$height,$word,'0','L');
							}
							if($this->GetY()>$max_y_) $max_y_ = $this->GetY();
						...
						$word = $item['comprobante']['items'][0]['conceptos'][0]['concepto'];
						$width_word = $this->GetStringWidth($helper->format_word($word));
						if($width_word>20){
							$this->SetFont('Arial','',$size_font-3);
							$this->setXY($yt_obs,$y);$this->MultiCell(20,$height,$helper->format_word($item['comprobante']['items'][0]['conceptos'][0]['concepto']),'0','L');
						}else{
					...
					if($this->GetY()>$max_y_) $max_y_ = $this->GetY();
					if($this->GetY()>$fy) $fy=$this->GetY();
					$this->setXY($yt_comp,$y);$this->MultiCell(20,$height,number_format($item["monto"], 2, '.', ''),'0','R');
					if($this->GetY()>$max_y_) $max_y_ = $this->GetY();
					if($this->GetY()>$fy) $fy=$this->GetY();
			...
			if(isset($items[$key+1])){
				...
				if($y>160){
					...
					if($items[$key+1]['cuenta']['cod']!=$cta){
						...
						if($cta_tot!=0){
							$this->setXY($yt_comp,$y);$this->MultiCell(20,$height,number_format($cta_tot, 2, '.', ''),'0','R');
						}
						$fy=$this->GetY();
						$y=$fy;
					}
					...
					/*if(($total-$tmp_cta_tot)==0){
						...
						//echo $total.'<br />';
						//echo $tmp_cta_tot.'<br />';
						if($this->recibo['_id']->{'$id'}=='5848641a6803fa244c8b4567'){ //A PEDIDO DE DEYANIRA
							if($this->page==2){
								$total = 24556.94;
		...
		if($y>160){
			...
			$this->setXY(60,$y);$this->MultiCell(65,$height,'VAN ...','0','L');
			if($this->recibo['_id']->{'$id'}=='584f05ba6803fa4f158b4567'){
				if($this->page==2){
					$total = 17170.06;
					$this->setXY(168,$y);$this->MultiCell(20,$height,number_format($total, 2, '.', ''),'0','L');
				}else{
		...
		if($fy>$y) $y = $fy;
		$this->SetFont('Arial','B',$size_font);
		$this->setXY(30,$y);$this->MultiCell(40,$height,'1202','0','L');
		$this->setXY(60,$y);$this->MultiCell(65,$height,'CUENTAS POR COBRAR DIVERSAS','0','L');
		$this->SetFont('Arial','',$size_font);
		$this->setXY(168,$y);$this->MultiCell(20,$height,number_format($tmp_1202, 2, '.', ''),'0','L');
		$y=$this->GetY();
		$this->SetFont('Arial','B',$size_font);
		$this->setXY(60,$y);$this->MultiCell(65,$height,'MULTAS Y SANCIONES','0','L');
		$y=$this->GetY();
		...
		$fy=$this->GetY();
      	//$y += $height;
		$cta_y = $y;
		$this->setXY(30,$y);$this->MultiCell(40,$height,'2101','0','L');
		$this->setXY(60,$y);$this->MultiCell(65,$height,'IMPUESTOS, CONTRIBUCIONES Y OTROS','0','L');
		$y=$this->GetY();
		$this->setXY(30,$y);$this->MultiCell(40,$height,'2101.0105','0','L');
		$this->setXY(60,$y);$this->MultiCell(65,$height,'IMPUESTO GENERAL A LAS VENTAS','0','L');
		...
		//$this->setXY(168,184);$this->MultiCell(30,$height,number_format($total, 2, '.', ''),'0','L');
		$this->setXY(168,174);$this->MultiCell(30,$height,number_format($this->recibo['total'], 2, '.', ''),'0','L');
