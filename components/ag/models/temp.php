<?php
class Model_ag_temp extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->ag_temp_logistica;
	}
	...
	protected function get_lista(){
		...
		if(isset($this->params['texto'])){
			if($this->params["texto"]!=''){
				...
				$criteria = $helper->paramsSearch($this->params["texto"], array('producto.nomb','producto.generico'));
			}
		}
		$sort = array('num'=>1);
	...
	protected function get_lote(){
		global $f;
		$data = $this->db->find(array(
			'producto._id'=>$this->params['producto'],
			'cant'=>array('$ne'=>0)
		))->sort(array('fecven'=>-1))->limit(1);
		foreach ($data as $obj) {
		    $this->items = $obj;
		}
	}
	...
	protected function save_clear_all(){
		global $f;
		$this->db->remove(array());
	}
}
?>