<?php
class Controller_ad_padi extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("ad/padi")->params($params)->get("lista") );
	}
	function execute_get(){
		global $f;
		$items = $f->model("ad/padi")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
		if(isset($f->request->data['all'])){
			foreach ($items['consulta'] as $i=>$cons) {
				$items['consulta'][$i]['_paciente'] = $f->model("ad/paci")->params(array("_id"=>$cons['paciente']['_id']))->get("one")->items;
			}
		}
	...
	function execute_get_parte(){
		global $f;
		$items = $f->model("ad/padi")->params()->get("parte")->items;
		$f->response->json( $items );
	}
	function execute_get_reporte(){
		...
		$items = $f->model("ad/padi")->params($params)->get("reporte")->items;
		$f->response->view("ad/quince.report.php",array('diario'=>$items));
	}
	function execute_get_reporte_diagnostico(){
		...
		$items = $f->model("ad/padi")->params($params)->get("reporte")->items;
		$f->response->view("ad/diagnostico.report.php",array('diario'=>$items));
	}
	function execute_get_reporte_pacientes(){
		...
		$items = $f->model("ad/padi")->params($params)->get("reporte")->items;
		$f->response->view("ad/pacientes.report.php",array('diario'=>$items));
	}
	function execute_get_mensual(){
		...
		$items = $f->model("ad/padi")->params($params)->get("mensual")->items;
		$f->response->view("ad/mes.report.php",array('diario'=>$items));
	}
 	function execute_get_doctor(){
		...
		$model = $f->model("ad/padi")->params($params)->get("doctor")->items;
		$items = array();
		if($model!=null){
			foreach($model as $item){
				...
				if($diario['paciente']['sexo']==0 && $diario['esta'] != '5' &&  $item['medico']['_id'] != '5977ae6c3e603746248b4568' ){
					//if($diario['paciente']['sexo']==0 && $diario['esta'] != '5' && $diario['esta'] != '5'){
						$items[$item['medico']['_id']->{'$id'}]['mujeres']++;
					}

					if($diario['paciente']['sexo']==1 && $diario['esta'] != '5' &&  $item['medico']['_id'] != '5977ae6c3e603746248b4568' ){
						$items[$item['medico']['_id']->{'$id'}]['varones']++;
					}

					
					//if($diario['esta'] == 2){
					if($diario['esta'] == 2  &&  $item['medico']['_id'] != '5977ae6c3e603746248b4568' ){
						$items[$item['medico']['_id']->{'$id'}]['nuevos']++;
					}

					//if($diario['esta'] == 3 ){
					if($diario['esta'] == 3 &&  $item['medico']['_id'] != '5977ae6c3e603746248b4568' ){
						$items[$item['medico']['_id']->{'$id'}]['contin']++;
					}

					if($diario['esta'] == 4 &&  $item['medico']['_id'] != '5977ae6c3e603746248b4568' ){
						$items[$item['medico']['_id']->{'$id'}]['reingr']++;
					}

					if($diario['esta'] == 1 &&  $item['medico']['_id'] != '5977ae6c3e603746248b4568' ){
					}

					if($edad<11  && $diario['esta'] != '5' &&  $item['medico']['_id'] != '5977ae6c3e603746248b4568' ){
						$items[$item['medico']['_id']->{'$id'}]['ninos']++;
					}

					if($edad>=11 && $edad<18 && $diario['esta'] != '5' &&  $item['medico']['_id'] != '5977ae6c3e603746248b4568' ){
						$items[$item['medico']['_id']->{'$id'}]['jovenes']++;
					}

					if($edad>=18 && $edad<60 && $diario['esta'] != '5' &&  $item['medico']['_id'] != '5977ae6c3e603746248b4568' ){
						$items[$item['medico']['_id']->{'$id'}]['adultos']++;
					}

					if($edad>=60 && $diario['esta'] != '5' &&  $item['medico']['_id'] != '5977ae6c3e603746248b4568' ){
		...
		$f->response->view("ad/doct.report.php",array('diario'=>$items,'params'=>$params));
	}
	...
	function execute_save(){
		...
		if(!isset($f->request->data['_id'])){
			...
			$data['estado'] = 'H';
			$model = $f->model("ad/padi")->params(array('data'=>$data))->save("insert")->items;
			...
			$model = $f->model("ad/padi")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$data))->save("update")->items;
		}
	...
	function execute_save_diario(){
		...
		if(isset($f->request->data['_id'])){
			...
			if(!isset($data['index'])){
				$model = $f->model("ad/padi")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>array('$push'=>array('consulta'=>$diario))))->save("custom")->items;
			}else{
				$model = $f->model("ad/padi")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>array('$set'=>array('consulta.'.$data['index']=>$diario))))->save("custom")->items;
			}
			$f->model('ac/log')->params(array(
				'modulo'=>'ad',
				'bandeja'=>'Parte Diario',
	...
	function execute_edit(){
		global $f;
		$f->response->view("ad/padi.edit");
	}
	function execute_details(){
		global $f;
		$f->response->view("ad/padi.details");
	}
	function execute_delete(){
		global $f;
		$f->model('ad/padi')->params(array('_id'=>new MongoId($f->request->data['_id'])))->delete('padi');
		$f->response->print("true");
	}
	function execute_delete_consulta(){
		global $f;
		$index = $f->request->data['index'];
		$parte = $f->model('ad/padi')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('one')->items;
		if($parte!=null){
			if(isset($parte['consulta'])){
				...
				$model = $f->model("ad/padi")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>array('consulta'=>$consultas)))->save("update")->items;
			}
		}
		$f->response->print("true");
	}
	function execute_print(){
		global $f;
		$diario = $f->model('ad/padi')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('one')->items;
		...
		if(isset($diario['consulta']) && count($diario['consulta'])>0){
			foreach($diario['consulta'] as $i=>$consulta){
				$paciente = $f->model('ad/paci')->params(array('_id'=>$consulta['paciente']['_id']))->get('one')->items;
		...
		//print_r($diario);
		$f->response->view("ad/padi.print",array('diario'=>$diario));

	}
	function execute_string_number(){
		global $f;
		$model = $f->model('ad/padi')->get('num_string')->items;
		if($model!=null){
			foreach($model as $i=>$item){
				$set = array(
					'num'=>floatval($item['num'])
				);
				$f->model('ad/padi')->params(array('_id'=>$item['_id'],'data'=>$set))->save('update');
