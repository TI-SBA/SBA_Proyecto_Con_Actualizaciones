<?php
class Controller_ad_psiq extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("ad/psiq")->params($params)->get("lista") );
	}
	function execute_get(){
		global $f;
		$items = $f->model("ad/psiq")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
		/*if($items!=null){
...
//		print_r($items);
	...
	function execute_get_codigo(){
		global $f;
		$items = $f->model("ad/psiq")->params()->get("codigo")->items;
		$f->response->json( $items );
	}
	function execute_get_reporte(){
		...
		$items = $f->model("ad/psiq")->params($params)->get("reporte")->items;
		$f->response->view("ad/psiq.report.php",array('psiquiatrica'=>$items));
	}
	function execute_save(){
		...
		if(!isset($f->request->data['_id'])){
			...
			$model = $f->model("ad/psiq")->params(array('data'=>$data))->save("insert")->items;
		}
		else{
			$vari = $f->model("ad/psiq")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
			$model = $f->model("ad/psiq")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$data))->save("update")->items;
	...
	function execute_editc(){
		...
		try{
			$ficha= $f->model("ad/psiq")->params(array("_id"=>new MongoId($data['_id'])))->get("one")->items;
			if(is_null($ficha))
			{
	...
	function execute_details(){
		global $f;
		$f->response->view("ad/psiq.details");
	}
	function execute_delete(){
		global $f;
		$f->model('ad/psiq')->params(array('_id'=>new MongoId($f->request->data['_id'])))->delete('psiq');
		$f->response->print("true");
	}
	function execute_print(){
		global $f;
		$psiquiatrica = $f->model('ad/psiq')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('one')->items;
		$f->response->view("ad/psiq.print",array('psiquiatrica'=>$psiquiatrica));
