<?php
class Model_ad_padi extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->ad_ParteDiario;
	}
	...
	protected function get_mensual(){
		...
		$filter = array();
		if(isset($this->params)) $filter = $this->params;
		$data = $this->db->find($filter, array('fech'=>1,'consulta'=>1,'medico'=>1));
		foreach ($data as $obj) {
		    $this->items[] = $obj;
		}
	...
	protected function get_doctor(){
		...
		$filter = array();
		if(isset($this->params)) $filter = $this->params;
		$data = $this->db->find($filter, array('fech'=>1,'medico'=>1,'consulta'=>1));
		foreach ($data as $obj) {
		    $this->items[] = $obj;
		}
	}
