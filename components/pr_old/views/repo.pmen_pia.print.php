...
class pmen extends FPDF
{
	...
	function Header(){
		...
		$this->setXY(5,37);$this->MultiCell(85,3.3,"TIPO DE TRANSACCION \n GENERICA \n   SUB GENERICA - NIVEL 1 \n     SUB GENERICA - NIVEL 2 \n      ESPECIFICA - NIVEL 1 \n       ESPECIFICA - NIVEL 2",'1','L');//85
		$this->Rect(90, 37, 20, 5);
		$this->setXY(110,37);$this->MultiCell(180,5,"",'1','C');
		$this->SetFont('arial','B',6);
		$this->Rect(90, 42, 20, 15);$this->setXY(90,42);$this->MultiCell(20,5,"PRESUPUESTO AUTORIZADO (PIA) ".$this->filtros["periodo"],'0','C');
	...
	function Publicar($items){
		...
		foreach($items as $fuente){
			...
			foreach($fuente["items"] as $item){
				...
				$this->Rect(25, $y, 65, 5);$this->SetXY(25,$y);$this->MultiCell(65,5,substr($item["clasificador"]["nomb"],0,45),'0','L');//65	
				$x_i = 110;
				$total_class = 0;
				for($n=1;$n<=12;$n++){
				...
				if(strlen($item["clasificador"]["cod"])==1){
					$total_fuen_pia+=$total_class;
					$total_acti_pia+=$total_class;
				}
			...
			$this->SetFont('arial','B',6);
			$this->SetXY(5,$y);$this->MultiCell(85,5,"SUB-TOTAL ACTIVIDADES",'1','C');
			$this->SetXY(90,$y);$this->MultiCell(20,5,number_format($total_acti_pia,2),'1','R');
			$x_i = 110;
			for($n=1;$n<=12;$n++){
			...
			$y+=5;
			$this->SetXY(5,$y);$this->MultiCell(85,5,"SUB-TOTAL PROYECTOS",'1','C');
			$this->SetXY(90,$y);$this->MultiCell(20,5,number_format($total_proy_pia,2),'1','R');
			$x_i = 110;
			for($n=1;$n<=12;$n++){
			...
			$y+=5;
			$this->SetXY(5,$y);$this->MultiCell(85,5,"SUB-TOTAL FUENTE DE FINANCIAMIENTO - ".$fuente["fuente"]["cod"],'1','C');
			$this->SetXY(90,$y);$this->MultiCell(20,5,number_format($total_fuen_pia,2),'1','R');
			$x_i = 110;
			for($n=1;$n<=12;$n++){
		...
		$this->SetFont('arial','B',6);
		$this->SetXY(5,$y);$this->MultiCell(85,5,"TOTAL ACTIVIDADES",'1','C');
		$this->SetXY(90,$y);$this->MultiCell(20,5,number_format($all_acti_pia,2),'1','R');
		$x_i = 110;
		for($n=1;$n<=12;$n++){
		...
		$y+=5;
		$this->SetXY(5,$y);$this->MultiCell(85,5,"TOTAL PROYECTOS",'1','C');
		$this->SetXY(90,$y);$this->MultiCell(20,5,number_format($all_proy_pia,2),'1','R');
		$x_i = 110;
		for($n=1;$n<=12;$n++){
		...
		$y+=5;
		$this->SetXY(5,$y);$this->MultiCell(85,5,"TOTAL TODAS LAS FUENTES DE FINANCIAMIENTO",'1','C');
		$this->SetXY(90,$y);$this->MultiCell(20,5,number_format($all_fuen_pia,2),'1','R');
		$x_i = 110;
		for($n=1;$n<=12;$n++){
