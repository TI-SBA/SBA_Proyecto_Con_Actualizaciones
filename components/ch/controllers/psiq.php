<?php
class Controller_ch_psiq extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("ch/psiq")->params($params)->get("lista") );
	}
	function execute_get(){
		global $f;
		$items = $f->model("ch/psiq")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
	...
	function execute_get_codigo(){
		global $f;
		$items = $f->model("ch/psiq")->params()->get("codigo")->items;
		$f->response->json( $items );
	}
	function execute_get_reporte(){
		...
		$items = $f->model("ch/psiq")->params($params)->get("reporte")->items;
	
		$f->response->view("ch/psiq.report.php",array('psiquiatrica'=>$items));
	}
	...
	function execute_save(){
		...
		if(!isset($f->request->data['_id'])){
			...
			$data['estado'] = 'H';
			$model = $f->model("ch/psiq")->params(array('data'=>$data))->save("insert")->items;
			...
			$vari = $f->model("ch/psiq")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
			$f->model('ac/log')->params(array(
				'modulo'=>'IN',
				'bandeja'=>'Tipo de Local',
				'descr'=>'Se actualizó el Tipo de Local <b>'.$vari['nomb'].'</b>.'
			))->save('insert');
			$model = $f->model("ch/psiq")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$data))->save("update")->items;
		}
		$f->response->json($model);
	}
	function execute_evolu(){
		global $f;
		$f->response->view("ch/evolu.edit");
	}
	function execute_edit(){
		global $f;
		$f->response->view("ch/psiq.edit");
	}
	function execute_editc(){
		...
		try{
			$ficha= $f->model("ch/psiq")->params(array("_id"=>new MongoId($data['_id'])))->get("one")->items;
			if(is_null($ficha))
			{
	...
	function execute_details(){
		global $f;
		$f->response->view("ch/psiq.details");
	}
	function execute_delete(){
		global $f;
		$f->model('ch/psiq')->params(array('_id'=>new MongoId($f->request->data['_id'])))->delete('psiq');
		$f->response->print("true");
	}
	function execute_print(){
		global $f;
		$psiquiatrica = $f->model('ch/psiq')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('one')->items;
		$f->response->view("ch/psiq.print",array('psiquiatrica'=>$psiquiatrica));
