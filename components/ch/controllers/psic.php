<?php
class Controller_ch_psic extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("ch/psic")->params($params)->get("lista") );
	}
	function execute_get(){
		global $f;
		$items = $f->model("ch/psic")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
	...
	function execute_get_codigo(){
		global $f;
		$items = $f->model("ch/psic")->params()->get("codigo")->items;
		$f->response->json( $items );
	}
	function execute_get_reporte(){
		...
		$items = $f->model("ch/psic")->params($params)->get("reporte")->items;
	
		$f->response->view("ch/psic.report.php",array('psicologica'=>$items));
	}
	...
	function execute_save(){
		...
		if(!isset($f->request->data['_id'])){
			...
			$data['estado'] = 'H';
			$model = $f->model("ch/psic")->params(array('data'=>$data))->save("insert")->items;
			...
			$vari = $f->model("ch/psic")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
			$f->model('ac/log')->params(array(
				'modulo'=>'IN',
				'bandeja'=>'Tipo de Local',
				'descr'=>'Se actualizó el Tipo de Local <b>'.$vari['nomb'].'</b>.'
			))->save('insert');
			$model = $f->model("ch/psic")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$data))->save("update")->items;
		}
	...
	function execute_permiso(){																											//Permiso que retorna comprobante si existe
		...
		try{
			$ficha= $f->model("ch/psic")->params(array("_id"=>new MongoId($data['_id'])))->get("one")->items;
			if(is_null($ficha))
			{
	...
	function execute_details(){
		global $f;
		$f->response->view("ch/psic.details");
	}
	function execute_delete(){
		global $f;
		$f->model('ch/psic')->params(array('_id'=>new MongoId($f->request->data['_id'])))->delete('psic');
		$f->response->print("true");
	}
	function execute_print(){
		global $f;
		$psicologica = $f->model('ch/psic')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('one')->items;
		$f->response->view("ch/psic.print",array('psicologica'=>$psicologica));
