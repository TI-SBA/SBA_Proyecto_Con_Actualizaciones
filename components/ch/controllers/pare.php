<?php
class Controller_ch_pare extends Controller {
...
*/	
	function execute_cita_paciente(){
		global $f;
		$data = $f->request->data;
		...
		try {
			#Verificacion de la data de entrada
			if(!isset($data['DNI']))
				throw new Exception("Debug: no se recibio ningun DNI");
			if(empty($data['DNI']))
				throw new Exception("Debug: el campo DNI esta vacío");
			#Verificacion de paciente
			$paci = $f->model('ch/paci')->params(array('num'=>$data['DNI']))->get('one_dni')->items;
			if(is_null($paci))
				throw new Exception("Error: no se encontro un paciente con el DNI proporcionado");
			if(!isset($paci['paciente']))
				throw new Exception("Debug: no existe el campo paciente");
			if(!isset($paci['paciente']['fullname']))
				throw new Exception("Debug: no existe el campo de nombre en paciente");
			if(empty($paci['paciente']['fullname']))
				throw new Exception("Debug: el campo de nombre esta en vacio");
			#Verificacion de historia clinica
			$histo = $f->model('ch/hist')->params(array('_id'=>$paci['paciente']['_id']))->get('por_entidad')->items;
			if(is_null($histo))
				throw new Exception("Error: no se encontro un historial para el DNI proporcionado");
			if(!isset($histo['paciente']['fullname']))
				throw new Exception("Debug: no existe el campo de nombre en historia clinica");
			if(empty($histo['paciente']['fullname']))
				throw new Exception("Debug: el campo de nombre esta en vacio en historia clinica");
			
			$a_response= array(
				'DNI' => $data['DNI'], 
				'nombre' => $paci['paciente']['fullname'],
				'histo_clinic' => $histo,
			);
