<?php
class Controller_ch_char extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("ch/char")->params($params)->get("lista") );
	}
	function execute_get(){
		global $f;
		$items = $f->model("ch/char")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
		$f->response->json( $items );
	}
	function execute_get_codigo(){
		global $f;
		$items = $f->model("ch/char")->params()->get("codigo")->items;
		$f->response->json( $items );
	}
	...
	function execute_save(){
		...
		if(!isset($f->request->data['_id'])){
			...
			*/
			...
			$data['estado'] = 'H';
			$model = $f->model("ch/char")->params(array('data'=>$data))->save("insert")->items;
			...
			$vari = $f->model("ch/char")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
			$f->model('ac/log')->params(array(
				'modulo'=>'IN',
				'bandeja'=>'Tipo de Local',
				'descr'=>'Se actualizó el Tipo de Local <b>'.$vari['nomb'].'</b>.'
			))->save('insert');
			$model = $f->model("ch/char")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$data))->save("update")->items;
		}
		$f->response->json($model);
	}
	function execute_edit(){
		global $f;
		$f->response->view("ch/char.edit");
	}
	function execute_details(){
		global $f;
		$f->response->view("ch/char.details");
	}
	function execute_delete(){
		global $f;
		$f->model('ch/char')->params(array('_id'=>new MongoId($f->request->data['_id'])))->delete('char');
		$f->response->print("true");
	}
	function execute_print(){
		global $f;
		$charla = $f->model('ch/char')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('one')->items;
		$f->response->view("ch/char.print",array('charla'=>$charla));
