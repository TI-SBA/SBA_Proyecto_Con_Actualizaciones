<?php
class Controller_ch_paho extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("ch/paho")->params($params)->get("lista") );
	}
	function execute_lista_(){
		...
		if(isset($f->request->data['sort']))
			$params['sort'] = array($f->request->data['sort']=>floatval($f->request->data['sort_i']));
		$lista=$f->model("ch/paho")->params($params)->get("lista")->items;
		$list_r=array();
	...
	function execute_get_reporte_alta(){
		...
		$items = $f->model("ch/paho")->params($params)->get("alta")->items;
		$f->response->view("ch/paho.alta.report.php",array('paciente'=>$items));
	}
	function execute_all(){
		global $f;
		$model = $f->model('ch/paho')->get('all');
	...
	function execute_get_ingreso(){
		...
		$items = $f->model("ch/paho")->params($params)->get("ingreso")->items;
		$f->response->json( $items );
	}
	function execute_get_codigo(){
		global $f;
		$items = $f->model("ch/paho")->params()->get("codigo")->items;
		$f->response->json( $items );
	}
	...
	function execute_save(){
		...
		if(!isset($f->request->data['_id'])){
			...
			$model = $f->model("ch/paho")->params(array('data'=>$data))->save("insert")->items;
			
		}else{
			$vari = $f->model("ch/paho")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
			$model = $f->model("ch/paho")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$data))->save("update")->items;
		}
	...
	function execute_alta(){
		global $f;
		$f->response->view("ch/paho.alta");
	}
	function execute_details(){
		global $f;
		$f->response->view("ch/paho.details");
	}
	function execute_delete(){
		global $f;
		$f->model('ch/paho')->params(array('_id'=>new MongoId($f->request->data['_id'])))->delete('paho');
		$f->response->print("true");
	}
	function execute_print(){
		global $f;
		$paciente = $f->model('ch/paho')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('alta')->items;
		//print_r($paciente);
		//die();
		$f->response->view("ch/paho.alta.print",array('paciente'=>$paciente));


	}
	function execute_agregarfullname(){
		global $f;
		$model = $f->model('ch/paho')->params(array('limit'=>1,'filter'=>array('paciente.fullname'=>array('$exists'=>false))))->get('all')->items;
		if($model!=null){
			foreach($model as $item){
				$fullname = $item['paciente']['nomb'].' '.$item['paciente']['appat'].' '.$paciente['apmat'];
				$f->model("ch/paho")->params(array('_id'=>$item['_id'],'data'=>array('paciente.fullname'=>$fullname)))->save("update")->items;
			}
			echo 'true';
		}
	}
