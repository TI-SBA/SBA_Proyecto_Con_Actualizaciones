<?php
class Controller_ch_pasi extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("ch/pasi")->params($params)->get("lista") );
	}
	function execute_get(){
		global $f;
		$items = $f->model("ch/pasi")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
	...
	function execute_get_parte(){
		global $f;
		$items = $f->model("ch/pasi")->params()->get("parte")->items;
		$f->response->json( $items );
	}
	function execute_get_reporte(){
		...
		$items = $f->model("ch/pasi")->params($params)->get("reporte")->items;
	...
	function execute_get_mensual(){
		...
		$items = $f->model("ch/pasi")->params($params)->get("mensual")->items;
 	...
 	function execute_get_doctor(){
		...
		$model = $f->model("ch/pasi")->params($params)->get("doctor")->items;
		$items = array();
		if($model!=null){
...
//					die();
	...
	function execute_save(){
		...
		if(!isset($f->request->data['_id'])){
			...
			$data['estado'] = 'H';
			$model = $f->model("ch/pasi")->params(array('data'=>$data))->save("insert")->items;
			...
			$model = $f->model("ch/pasi")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$data))->save("update")->items;
		}
	...
	function execute_save_diario(){
		...
		if(isset($f->request->data['_id'])){
			...
			if(!isset($data['index'])){
				$model = $f->model("ch/pasi")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>array('$push'=>array('consulta'=>$diario))))->save("custom")->items;
			}else{
				$model = $f->model("ch/pasi")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>array('$set'=>array('consulta.'.$data['index']=>$diario))))->save("custom")->items;
			}
			$f->model('ac/log')->params(array(
				'modulo'=>'MH',
				'bandeja'=>'Parte Diario',
	...
	function execute_edit(){
		global $f;
		$f->response->view("ch/pasi.edit");
	}
	function execute_details(){
		global $f;
		$f->response->view("ch/pasi.details");
	}
	function execute_delete(){
		global $f;
		$f->model('ch/pasi')->params(array('_id'=>new MongoId($f->request->data['_id'])))->delete('pasi');
		$f->response->print("true");
	}
	function execute_delete_consulta(){
		global $f;
		$index = $f->request->data['index'];
		$parte = $f->model('ch/pasi')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('one')->items;
		if($parte!=null){
			if(isset($parte['consulta'])){
				...
				$model = $f->model("ch/pasi")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>array('consulta'=>$consultas)))->save("update")->items;
			}
		}
		$f->response->print("true");
	}
	function execute_print(){
		global $f;
		$diario = $f->model('ch/pasi')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('one')->items;
		...
		//print_r($diario);
		$f->response->view("ch/pasi.print",array('diario'=>$diario));

	}
	function execute_string_number(){
		global $f;
		$model = $f->model('ch/pasi')->get('num_string')->items;
		if($model!=null){
			foreach($model as $i=>$item){
				$set = array(
					'num'=>floatval($item['num'])
				);
				$f->model('ch/pasi')->params(array('_id'=>$item['_id'],'data'=>$set))->save('update');
