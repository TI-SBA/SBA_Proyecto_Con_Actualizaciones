<?php
class Model_ch_psiq extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->mh_fichaspsiquiatricas;
	}
	...
	protected function get_reporte(){
		...
		$data = $this->db->find($filter, array('clin'=>1,'paciente'=>1,'paciente.paciente.appat'=>1,'paciente.paciente.apmat'=>1,'edad'=>1,'moti'=>1))->sort(array('clini'=>-1));
		foreach ($data as $obj) {
		    $this->items[] = $obj;
		}
	}
	...
	protected function delete_psiq(){
		global $f;
