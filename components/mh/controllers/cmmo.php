<?php
class Controller_mh_cmmo extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("mh/cmmo")->params($params)->get("lista") );
	}
	function execute_get(){
		global $f;
		$items = $f->model("mh/cmmo")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
	...
	function execute_log(){// En preparacion para usarse en un log
		...
		try{
			if(isset($data['cama_old']['_id'])){
				$cama = $f->model("mh/cama")->params(array("_id"=>new MongoId($data['cama_old']['_id'])))->get("one")->items;
				if(is_null($cama))
					throw new Exception("Error: No existe el _id en las camas");
			}
			if(isset($data['hosp']['_id'])){
				$hosp = $f->model("mh/hosp")->params(array("_id"=>new MongoId($data['hosp']['_id'])))->get("one")->items;
				if(is_null($hosp))
			...
			if($cama['uso'] == "V" && $data['uso'] == 'V')
			{
				...
				$model = $f->model("mh/cmmo")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$data))->save("update")->items;
	...
	*/
	function execute_paciente(){
		...
		try{
			...
			$cama_movimiento = $f->model("mh/cmmo")->params(array("_id"=>new MongoId($data['_id'])))->get("one")->items;
			...
			#camas
			$cama = $f->model("mh/cama")->params(array("_id"=>new MongoId($cama_movimiento['cama']['_id'])))->get("one")->items;
			if(is_null($cama))
			...
			$hospi = $f->model("mh/hospi")->params(array("_id"=>new MongoId($data['paciente']['_id'])))->get("one")->items;
			if(is_null($hospi))
			...
			$cama_movimientos = $f->model("mh/cmmo")->params(array('_id'=>new MongoId($data['_id']),'data'=>$to_movimientos))->save("update")->items;
			$cama = $f->model("mh/cama")->params(array('_id'=>$to_movimientos['cama']['_id'],'data'=>$to_cama))->save("update")->items;
			$response['status'] = 'success';
			$response['message'] = 'Fueron actualizacion los cambios correctamente.';
		}catch(Exception $e){
			$response['status'] = 'error';
	...
	/*function execute_movimiento(){
		...
		try{
			...
			#camas
			$cama = $f->model("mh/cama")->params(array("_id"=>new MongoId($cama_movimiento['cama_old']['_id'])))->get("one")->items;
			if(is_null($cama))
				throw new Exception("Error: no se encuentra la cama");
			#movimiento
			...
			//$cama_movimientos = $f->model("mh/cmmo")->params($to_movimientos)->save("insert")->items;
			$cama_movimientos = $f->model("mh/cmmo")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$to_movimientos))->save("update")->items;
			$cama = $f->model("mh/cama")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$to_movimientos))->save("update")->items;
			$response['status'] = 'success';
			$response['message'] = 'Fueron actualizacion los cambios correctamente.';
		}
		catch(Exception $e){
	...
	function execute_estadisticas(){
		...
		try{
			...
			#Camas vacias
			$c_v_i_v = count($f->model("mh/cmmo")->get("vacias_intensivo_varones")->items);
			$c_v_i_d = count($f->model("mh/cmmo")->get("vacias_intensivo_damas")->items);
			$c_v_in_v = count($f->model("mh/cmmo")->get("vacias_intermedio_varones")->items);
			$c_v_in_d = count($f->model("mh/cmmo")->get("vacias_intermedio_damas")->items);
			/*
			#Camas usadas
			$c_u_i_v = $f->model("mh/cama")->get("usadas_intensivo_varones")->items;
			$c_u_i_d = $f->model("mh/cama")->get("usadas_intensivo_damas")->items;
			$c_u_in_v = $f->model("mh/cama")->get("usadas_intermedio_varones")->items;
			$c_u_in_d = $f->model("mh/cama")->get("usadas_intermedio_damas")->items;
			*/
			$response['data'] = array(
			...
			$response['data'] = ($f->model("mh/cmmo")->params($to_custom)->get("custom")->items);
			$response['status'] = 'success';
			$response['message'] = 'se obtuvieron las estadisticas correctamente.';
			}
	...
	function execute_traslado(){
		...
		try{
			...
			$vieja_cama = $f->model("mh/cama")->params(array("_id"=>new MongoId($data['cama_old']['_id'])))->get("one")->items;
			$nueva_cama = $f->model("mh/cama")->params(array("_id"=>new MongoId($data['cama']['_id'])))->get("one")->items;
			...
			if(!isset($data['_id']))
				throw new Exception("Error: no se recibio el _id de movimientos");
			$cama_antiguo_movimiento = $f->model("mh/cmmo")->params(array("_id"=>new MongoId($data['cama_old']['_id']),"sala"=>$vieja_cama['sala'],"pabellon"=>$vieja_cama['pabellon']))->get("cama")->items;
			...
			#Nuevo movimiento
			$cama_nuevo_movimiento = $f->model("mh/cmmo")->params(array("_id"=>new MongoId($data['_id'])))->get("one")->items;
			...
			$paciente = $f->model("mh/paci")->params(array("_id"=>new MongoId($data['paciente']['paciente']['_id'])))->get("one_entidad")->items;
			if(is_null($paciente))
			...
			$movimiento_antiguo = $f->model("mh/cmmo")->params(array('_id'=>new MongoId($cama_antiguo_movimiento['_id']),'data'=>$antiguo_traslado))->save("update")->items;
			$movimiento_nuevo = $f->model("mh/cmmo")->params(array('_id'=>($cama_nuevo_movimiento['_id']),'data'=>$nuevo_traslado))->save("update")->items;
			$cama_antigua = $f->model("mh/cama")->params(array('_id'=>$cama_antiguo_movimiento['cama']['_id'],'data'=>$antigua_cama))->save("update")->items;
			$cama_nueva = $f->model("mh/cama")->params(array('_id'=>$cama_nuevo_movimiento['cama']['_id'],'data'=>$nueva_cama))->save("update")->items;
			$response['status'] = 'success';
			$response['message'] = 'Fueron actualizacion los cambios correctamente.';
		}		
		catch(Exception $e){
	...
	function execute_edit(){
		global $f;
		$f->response->view("mh/cmmo.edit");
	}
	function execute_tras(){
		global $f;
		$f->response->view("mh/cmmo.tras");
	}
