<?php
class Controller_mh_paci_corr extends Controller {
	function corregir(){
		global $f;
		$data = $f->request->data;
		$response=array(
		...
		try{
			...
			{
				$his_cli = $f->datastore->mh_pacientes->find(array("his_cli"=>intval($data['his_cli'])));
				$last_pac = $f->datastore->mh_pacientes->find(array("his_cli"=>intval($data['his_cli'])))->sort(array('$natural'=>1))->limit(1);
				$pac_def;
				#CONTADORES
				$c_social=0;
				$c_pardia=0;
				...
				foreach ($his_cli as $paciente){
					if($paciente['paciente']['_id']->{'$id'}!=$pac_def['_id']->{'$id'}){
						$social = $f->datastore->mh_FichaSocial->find(array("paciente._id"=>($paciente['_id'])));
						$c_social=$c_social+($social->count());

						$ParDia = $f->datastore->mh_ParteDiario->find(array("consulta.paciente._id"=>($paciente['_id'])));
						$c_pardia=$c_pardia+($ParDia->count());

						$camas = $f->datastore->mh_camas->find(array("paciente._id"=>($paciente['_id'])));
						$c_camas=($c_camas+$camas->count());

						$camos = $f->datastore->mh_camos->find(array("paciente._id"=>($paciente['_id'])));
						$c_camos=$c_camos+($camos->count());

						$psic = $f->datastore->mh_fichaspsicologicas->find(array("paciente._id"=>($paciente['_id'])));
						$c_psic=$c_psic+($psic->count());

						$psiq = $f->datastore->mh_fichaspsiquiatricas->find(array("paciente._id"=>($paciente['_id'])));
						$c_psiq=$c_psiq+($psiq->count());

						$hosp = $f->datastore->mh_pacientes_hospitalizados->find(array("paciente._id"=>($paciente['_id'])));
...
//					}
					...
					$ParDia = $f->datastore->mh_ParteDiario->find(array("consulta.paciente.his_cli"=>($paciente['his_cli'])));
//					$c_pardia=$c_pardia+($ParDia->count());
						...
						if(isset($par_dia['consulta'])){
							foreach ($par_dia['consulta'] as $key => $consulta) {
								...
								if($par_dia['consulta'][$key]['paciente']['his_cli']==$pac_def['his_cli']){
									...
									//var_dump($par_dia['consulta'][$key]['paciente']['_id'].'</br>');
									//var_dump($pac_def['_id'].'</br>');
									//$parte=$f->datastore->mh_ParteDiario->update(array("_id"=>$par_dia['_id']),array('$set'=>array('consulta.'.$key.'.paciente._id'=>$pac_def['_id'])));
									//print_r($parte);
									//print_r($par_dia);
								}
							}
							//print_r($par_dia);
							//$parte=$f->datastore->mh_ParteDiario->update(array('_id'=>$par_dia['_id']),$par_dia);
						}
					}
					$camas = $f->datastore->mh_camas->find(array("paciente._id"=>($paciente['_id'])));
//					$c_camas=($c_camas+$camas->count());
