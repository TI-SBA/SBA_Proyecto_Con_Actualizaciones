<?php
class Controller_mh_hist extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("mh/hist")->params($params)->get("lista") );
	}
	function execute_get(){
		global $f;
		$items = $f->model("mh/hist")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
	...
	function execute_save(){
		...
		if(!isset($f->request->data['_id'])){
			...
			$data['estado'] = 'H';
			$model = $f->model("mh/hist")->params(array('data'=>$data))->save("insert")->items;
			...
			$vari = $f->model("mh/hist")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
			$f->model('ac/log')->params(array(
				'modulo'=>'IN',
				'bandeja'=>'Tipo de Local',
				'descr'=>'Se actualizó el Tipo de Local <b>'.$vari['nomb'].'</b>.'
			))->save('insert');
			$model = $f->model("mh/hist")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$data))->save("update")->items;
		}
	...
	function execute_edit(){
		global $f;
		$f->response->view("mh/hist.edit");
	}
	function execute_details(){
		global $f;
		$f->response->view("mh/hist.details");
	}
	function execute_delete(){
		global $f;
		$f->model('mh/hist')->params(array('_id'=>new MongoId($f->request->data['_id'])))->delete('hist');
		$f->response->print("true");
	}
	function execute_print(){
		global $f;
		$hist = $f->model('mh/hist')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('one')->items;
		$f->response->view("mh/hist.print",array('hist'=>$hist));
	...
	function execute_importar(){
		...
		foreach($items as $key => $item){
		    ...
		    $f->datastore->mh_historias_clinicas->insert($data);
		    $bd->mh_pacientes->update(array('_id'=>$item['_id']),array('$set'=>array('check1'=>true)));
		}
