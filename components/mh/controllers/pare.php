<?php
class Controller_mh_pare extends Controller {
...
*/	
	function execute_cita_paciente(){
		...
		try {
			...
			$paci = $f->model('mh/paci')->params(array('num'=>$data['DNI']))->get('one_dni')->items;
			if(is_null($paci))
			...
			$histo = $f->model('mh/hist')->params(array('_id'=>$paci['paciente']['_id']))->get('por_entidad')->items;
			if(is_null($histo))
