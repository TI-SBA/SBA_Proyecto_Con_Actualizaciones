<?php
class Controller_mh_hospi extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("mh/hospi")->params($params)->get("lista") );
	}
	...
	function execute_get_ingreso(){
		...
		$items = $f->model("mh/hospi")->params($params)->get("ingreso")->items;
		$f->response->json( $items );
	}
	...
	function execute_get_codigo(){
		global $f;
		$items = $f->model("mh/hospi")->params()->get("codigo")->items;
		$f->response->json( $items );
	}
	...
	function execute_save(){
		...
		if(isset($data['paciente']))
			$data['paciente']['_id'] = new MongoId($data['paciente']['_id']);
		if(isset($data['apoderado']) && isset($data['apoderado']['_id']))
		...
		if(!isset($f->request->data['_id'])){
			...
			$model = $f->model("mh/hospi")->params(array('data'=>$data))->save("insert")->items;
			$f->model('ac/log')->params(array(
				'modulo'=>'MH',
				'bandeja'=>'Hospitalizaciones',
				'descr'=>'Se creó el un nuevo paciente hospitalizado <b>'.$data['nomb'].'</b>.'
			))->save('insert');
		}else{
			$vari = $f->model("mh/hospi")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
			$f->model('ac/log')->params(array(
				'modulo'=>'MH',
				'bandeja'=>'Hospitalizaciones',
				'descr'=>'Se actualizó el paciente hospitalizado <b>'.$vari['nomb'].'</b>.'
			))->save('insert');
			$model = $f->model("mh/hospi")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$data))->save("update")->items;
		}
		$f->response->json(array('success'=>true));
	}
	...
	function execute_edit(){
		global $f;
		$f->response->view("mh/hospi.edit");
	}
	function execute_details(){
		global $f;
		$f->response->view("mh/hospi.details");
	}
	function execute_delete(){
		global $f;
		$f->model('mh/hospi')->params(array('_id'=>new MongoId($f->request->data['_id'])))->delete('hospi');
	...
	function execute_print_legacy(){
		...
		$f->response->view("mh/hospi.print",array('hospitalizacion'=>$hospitalizacion));

	}

	function execute_print(){
		global $f;
		$hospitalizacion = $f->model('mh/hospi')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('one')->items;
		$f->response->view("mh/hospi.print",array('hospitalizacion'=>$hospitalizacion));
	}
	...
	function execute_agregarfullname(){
		global $f;
		$model = $f->model('mh/hospi')->params(array('limit'=>1,'filter'=>array('paciente.fullname'=>array('$exists'=>false))))->get('all')->items;
		if($model!=null){
			foreach($model as $item){
				$fullname = $item['paciente']['nomb'].' '.$item['paciente']['appat'].' '.$paciente['apmat'];
				$f->model("mh/hospi")->params(array('_id'=>$item['_id'],'data'=>array('paciente.fullname'=>$fullname)))->save("update")->items;
			}
			echo 'true';
		}
	}
