<?php
class Controller_mh_pajo extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("mh/pajo")->params($params)->get("lista") );
	}
	function execute_lista_(){
		...
		if(isset($f->request->data['sort']))
			$params['sort'] = array($f->request->data['sort']=>floatval($f->request->data['sort_i']));
		$lista=$f->model("mh/pajo")->params($params)->get("lista")->items;
		$list_r=array();
	...
	function execute_get_reporte_alta(){
		...
		$items = $f->model("mh/pajo")->params($params)->get("alta")->items;
		$f->response->view("mh/pajo.alta.report.php",array('paciente'=>$items));
	}
	function execute_all(){
		global $f;
		$model = $f->model('mh/pajo')->get('all');
		$f->response->json($model->items);
	}
	function execute_get(){
		global $f;
		$items = $f->model("mh/pajo")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
	...
	function execute_get_ingreso(){
		...
		$items = $f->model("mh/pajo")->params($params)->get("ingreso")->items;
		$f->response->json( $items );
	}
	function execute_get_codigo(){
		global $f;
		$items = $f->model("mh/pajo")->params()->get("codigo")->items;
		$f->response->json( $items );
	}
	...
	function execute_save(){
		...
		if(!isset($f->request->data['_id'])){
			...
			$model = $f->model("mh/pajo")->params(array('data'=>$data))->save("insert")->items;
			
		}else{
			$vari = $f->model("mh/pajo")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
			$model = $f->model("mh/pajo")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$data))->save("update")->items;
		}
	...
	function execute_alta(){
		global $f;
		$f->response->view("mh/pajo.alta");
	}
	function execute_details(){
		global $f;
		$f->response->view("mh/pajo.details");
	}
	function execute_delete(){
		global $f;
		$f->model('mh/pajo')->params(array('_id'=>new MongoId($f->request->data['_id'])))->delete('pajo');
		$f->response->print("true");
	}
	function execute_print(){
		global $f;
		$paciente = $f->model('mh/pajo')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('alta')->items;
		//print_r($paciente);
		//die();
		$f->response->view("mh/pajo.alta.print",array('paciente'=>$paciente));


	}
	function execute_agregarfullname(){
		global $f;
		$model = $f->model('mh/pajo')->params(array('limit'=>1,'filter'=>array('paciente.fullname'=>array('$exists'=>false))))->get('all')->items;
		if($model!=null){
			foreach($model as $item){
				$fullname = $item['paciente']['nomb'].' '.$item['paciente']['appat'].' '.$paciente['apmat'];
				$f->model("mh/pajo")->params(array('_id'=>$item['_id'],'data'=>array('paciente.fullname'=>$fullname)))->save("update")->items;
			}
			echo 'true';
		}
	}
