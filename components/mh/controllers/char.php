<?php
class Controller_mh_char extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("mh/char")->params($params)->get("lista") );
	}
	function execute_get(){
		global $f;
		$items = $f->model("mh/char")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
		$f->response->json( $items );
	}
	function execute_get_codigo(){
		global $f;
		$items = $f->model("mh/char")->params()->get("codigo")->items;
		$f->response->json( $items );
	}
	...
	function execute_save(){
		...
		if(!isset($f->request->data['_id'])){
			...
			*/
			...
			$data['estado'] = 'H';
			$model = $f->model("mh/char")->params(array('data'=>$data))->save("insert")->items;
			...
			$vari = $f->model("mh/char")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
			$model = $f->model("mh/char")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$data))->save("update")->items;
		}
		$f->response->json($model);
	}
	function execute_edit(){
		global $f;
		$f->response->view("mh/char.edit");
	}
	function execute_details(){
		global $f;
		$f->response->view("mh/char.details");
	}
	function execute_delete(){
		global $f;
		$f->model('mh/char')->params(array('_id'=>new MongoId($f->request->data['_id'])))->delete('char');
		$f->response->print("true");
	}
	function execute_print(){
		global $f;
		$charla = $f->model('mh/char')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('one')->items;
		$f->response->view("mh/char.print",array('charla'=>$charla));
