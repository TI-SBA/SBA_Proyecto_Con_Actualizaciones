<?php
class Model_mh_paci extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->mh_pacientes;
	}
	...
	protected function get_fichasalud(){
		...
		$data = $this->db->find($filter, array('his_cli'=>1,'paciente.fullname'=>1,'domicilios.direccion'=>1,'apoderado.appat'=>1,'apoderado.apmat'=>1,'apoderado.nomb'=>1,'fecha_na'=>1,'modulo'=>1,'categoria'=>1,'fecreg'=>1));
		foreach ($data as $obj) {
		    $this->items[] = $obj;
		}			
	}
	protected function get_custom_data(){
		global $f;
		$data = $this->db->find($this->params['filter'],$this->params['fields'])->sort( $this->params['sort'] );
		foreach ($data as $obj) {
			$this->items[] = $obj;
			print_r($obj);
		}
	...
	protected function get_all(){
		...
		$data = $this->db->find($filter,$fields)->limit(5286);
...
//		isset($this->$data["his_cli"] = floatval($data["his_cli"]));
	...
	protected function save_update_2(){
		global $f;
		$this->params['data']['_id'];
