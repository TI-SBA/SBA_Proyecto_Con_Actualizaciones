<?php
class Model_ts_poli extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->ts_polizas;
	}
	...
	protected function get_lista(){
		global $f;
		$filter = array();
		if(isset($this->params['orga'])) $filter['organizacion._id'] = $this->params['orga'];
		$fields = array();
	...
	protected function get_search(){
		...
		if($this->params["texto"]!=''){
			...
			$criteria = $helper->paramsSearch($this->params["texto"], array('cod','descr','cliente.nomb','cliente.appat','cliente.apmat'));
		}else $criteria = array();
	...
	protected function get_all(){
		global $f;
		$data = $this->db->find(array('estado'=>'R'));
		foreach ($data as $ob) {
		    $this->items[] = $ob;
		}
	}
