<?php
class Model_ts_cjba extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->ts_caja_bancos;
	}
	...
	protected function get_all(){
		...
		if(isset($this->params["ano"]))$filter["periodo.anio"]=$this->params["ano"];
	...
	protected function get_all_periodo(){
		global $f;
		$data = $this->db->find(array('periodo'=>$this->params['periodo'],'cuenta_banco._id'=>$this->params['ctban']));
		foreach ($data as $ob) {
		    $this->items[] = $ob;
		}
	}
