<?php
class Controller_ho_cont extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("ho/pame")->params($params)->get("lista") );
	}
	function execute_get(){
		global $f;
		$items = $f->model("ho/pame")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
		$f->response->json( $items );
	}
	function execute_modulo(){
		global $f;
		$f->response->view("ho/cont.modulo");
	}
	function execute_save(){
		...
		if(!isset($f->request->data['_id'])){
			...
			$data['estado'] = 'A';
			$model = $f->model("ho/pame")->params(array('data'=>$data))->save("insert")->items;
			...
			$vari = $f->model("ho/pame")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
			$f->model('ac/log')->params(array(
				'modulo'=>'HO',
				'bandeja'=>'Control de Medicinas',
				'descr'=>'Se actualizó el Control de Medicinas para <b>'.$vari['paciente']['nomb'].'</b>.'
			))->save('insert');
			$model = $f->model("ho/pame")->params(array('_id'=>new MongoId($f->request->data['_id']),'data'=>$data))->save("update")->items;
		}
	...
	function execute_save_dis(){
		global $f;
		$data = $f->request->data;
		$cont = $f->model('ho/pame')->params(array('_id'=>new MongoId($data['_id'])))->get('one')->items;
		foreach($cont['medicinas'] as $i=>$medi){
			if(floatval($data['medicinas'][$i]['cant'])>0){
				$f->model('ho/pame')->params(array(
					'_id'=>$cont['_id'],
					'data'=>array(
						'$inc'=>array(
	...
	function execute_save_devolver(){
		global $f;
		$control = $f->model('ho/pame')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('one')->items;
	...
	function execute_edit(){
		global $f;
		$f->response->view("ho/cont.edit");
	}
	function execute_details(){
		global $f;
		$f->response->view("ho/cont.details");
	}
	function execute_print(){
		global $f;
		$item = $f->model("ho/pame")->params(array("_id"=>new MongoId($f->request->data['_id'])))->get("one")->items;
		$f->response->view("ho/cont.print.xls",array('data'=>$item));
	}
}
?>