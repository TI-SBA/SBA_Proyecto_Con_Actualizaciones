<?php
class Controller_ho_hosp extends Controller {
	function execute_lista(){
		...
		$f->response->json( $f->model("ho/hosp")->params($params)->get("lista") );
	}
	...
	function execute_get(){
		...
		$items['tarifas'] = $f->model('ho/tari')->params(array('filter'=>array('estado'=>'H')))->get('all')->items;
	...
	function execute_get_hospi_dia(){
		...
		$items = $f->model("ho/hosp")->params($params)->get("hospi_dia")->items;
		$f->response->view("ho/hospi_dia.php",array('hospi'=>$items));
	}
	...
	function execute_get_hospi(){
		...
		$items = $f->model("ho/hosp")->params($params)->get("recidia")->items;
		$f->response->view("ho/reporte.hospi.php",array('hospi'=>$items));
	}
	...
	/*function execute_save_rec(){
		...
		$caja = $f->model('cj/caja')->params(array('modulo'=>'HO'))->get('mod')->items;
		$caja = array(
			'_id'=>$caja['_id'],
			'nomb'=>$caja['nomb'],
			'local'=>$caja['local']
		...
		if(floatval($data['importe']>0)){
			...
			$tmp = $data['importe'];
			unset($data['importe']);
			$f->model('ho/hosp')->params(array('_id'=>$data['_id'],'data'=>$data))->save('update');
			$f->model('ho/hosp')->params(array('_id'=>$data['_id'],'data'=>array('$push'=>array('recibos'=>$recibo_))))->save('custom');
			$f->model('ho/hosp')->params(array('_id'=>$data['_id'],'data'=>array('$inc'=>array('importe'=>floatval($tmp)))))->save('custom');
		}
	...
	*/
	...
	function execute_delete(){
		global $f;
		$item = $f->model('ho/hosp')->params(array('_id'=>new MongoId($f->request->data['_id'])))->get('one')->items;
	...
	function execute_edit(){
		global $f;
		$f->response->view("ho/hosp.edit");
	}
	function execute_edit_alta(){
		global $f;
		$f->response->view("ho/hosp.alta.edit");
	}
	function execute_string_number(){
		global $f;
		$model = $f->model('ho/hosp')->get('hist_cli_string')->items;
		if($model!=null){
			foreach($model as $i=>$item){
				$set = array(
					'hist_cli'=>floatval($item['hist_cli'])
				);
				$f->model('ho/hosp')->params(array('_id'=>$item['_id'],'data'=>$set))->save('update');
