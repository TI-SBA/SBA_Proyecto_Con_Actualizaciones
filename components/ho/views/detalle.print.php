...
class repo extends FPDF
{
	function Header(){
		...
		$this->SetXY(5,30);$this->MultiCell(200,5,"Reporte de Ingresos de Hospitalizaciones",'0','C');
		
	}	
	function Publicar($detalle){
		$x=5;
		$y=25;
		$i_ini=$y;
		$pag_b = 275;
		...
		for ($i=0; $i <count($detalle); $i++) { 
			for($j = 0;$j<count($detalle[$i]['items']);$j++){
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Alquiler Cancha Deportiva por Hora'){
							$cancha++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Búsqueda Historia'){
							$busqueda++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Carnet de Atención'){
							$carnet++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Certificado Médico'){
							$certificado++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Constancia Médica'){
							$constancia++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Consulta Especializada - Psicología - Continuador'){
							$con_es_psic_cont++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Consulta Especializada - Psicología - Indigente'){
							$con_es_psic_ind++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Consulta Especializada - Psicología - Nuevo'){
							$con_es_psic_nuev++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Consulta Especializada - Psiquiatría - Continuadores'){
							$con_es_psiq_cont++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Consulta Especializada - Psiquiatría - Indigentes'){
							$con_es_psiq_ind++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Consulta Especializada - Psiquiatría - Nuevo'){
							$con_es_psiq_nuev++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Consulta Especializada - Terapeuta - Continuador'){
							$con_es_tera_cont++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Consulta Especializada - Terapeuta - Indigente'){
							$con_es_tera_ind++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Consulta Especializada - Terapeuta - Nuevo'){
							$con_es_tera_nuev++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Consulta Médica - Nuevo'){
							$cons_medi_nuevo++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Consulta Médica Continuador'){
							$cons_medi_conti++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Consulta Médica Indigente'){
							$cons_medi_indi++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Copias Certificadas por Hoja'){
							$cop_cer_hoja++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Historia Clínica'){
							$hist_clinica++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Informe Médico'){
							$infor_medico++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Manualidades'){
							$manual++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Servicio de Ambulancia'){
							$ambulancia++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Venta de Humus y otros ( papas, maiz, alfalfa,etc.)'){
							$humus++;								
				}
				if($detalle[$i]['items'][$j]['servicio']['nomb'] == 'Venta de ganado (vacuno,bovino.porcino,otros)'){
							$ganado++;								
				}
		...
		$y = $y+5;
		$this->SetXY(40,$y);$this->MultiCell(80,5,"Copias Certificadas por Hoja ".$cop_cer_hoja,'1','L');
		...
		$y = $y+5;
		$this->SetXY(40,$y);$this->MultiCell(80,5,"Venta de Humus y otros ( papas, maiz, alfalfa,etc.) ",'1','L');
		$this->SetXY(120,$y);$this->MultiCell(20,5,"".$humus,'1','L');
		$t_17 = 0;
		$t_17 = $humus * 5;
		$this->SetXY(140,$y);$this->MultiCell(20,5,$t_17,'1','L');
		$y = $y+5;
		$this->SetXY(40,$y);$this->MultiCell(80,5,"Venta de ganado (vacuno,bovino.porcino,otros) ",'1','L');
		$this->SetXY(120,$y);$this->MultiCell(20,5,"".$ganado,'1','L');
		$t_18 = 0;
		$t_18 = $ganado * 9;
		$this->SetXY(140,$y);$this->MultiCell(20,5,$t_18,'1','L');
		$this->SetFont('Arial','B',8);
		$total = 0;
