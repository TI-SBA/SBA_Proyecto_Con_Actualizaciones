<?php
class Model_pe_cuad extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->pe_cuadros;
	}
	protected function get_one(){
		global $f;
		$this->items = $this->db->findOne(array('_id'=>$this->params['_id']));
	}
	protected function get_orga_peri(){
		global $f;
		$this->items = $this->db->findOne(array(
			'organizacion._id'=>$this->params['_id'],
	...
	protected function get_search(){
		...
		if($this->params["texto"]!=''){
			...
			$criteria = $helper->paramsSearch($this->params["texto"], array('nomb','periodo','organizacion.nomb'));
		}else $criteria = array();
	...
	protected function get_conso(){
		global $f;
		$data = $this->db->find(array('periodo'=>$this->params['periodo'],'estado'=>'A'));
		foreach ($data as $ob) {
		    $this->items[] = $ob;
		}
	}
	...
	protected function save_reset_vig(){
		...
		) , array('$set'=>array('estado'=>'C')) , array("multiple" => true) );
	}
}
?>