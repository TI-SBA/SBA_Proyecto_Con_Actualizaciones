...
class bole extends FPDF
{
	function Header(){
		...
		$this->setXY(5,15);$this->MultiCell($ancho,5,"BOLETA DE PAGO - CAS",'0','C');
		...
		$this->setXY(5,30);$this->MultiCell(40,5,"DNI:",'0','L');
		$this->setXY(90,30);$this->MultiCell(40,5,"CUI AFP:",'0','L');
		
		$this->setXY(5,35);$this->MultiCell($ancho/3,5,"Pagos",'1','C');
		$this->setXY(5+($ancho/3),35);$this->MultiCell($ancho/3,5,"Descuentos",'1','C');
		$this->setXY(5+(2*$ancho/3),35);$this->MultiCell($ancho/3,5,"Patronal",'1','C');
	}		
	function Publicar($items){
		...
		$this->setXY(15,30);$this->MultiCell(70,5,$items['trabajador']['docident'][0]['num'],'0','L');
		$this->setXY(110,30);$this->MultiCell(40,5,$items['trabajador']['roles']['trabajador']['cod_aportante'],'0','L');
		$ancho = 205.9/3;
		$ancho_monto = 20;
		...
		for($i=0;$i<count($items['conceptos']);$i++){
			if($items['conceptos'][$i]['concepto']['tipo']=="P"&&$items['conceptos'][$i]['subtotal']!="0.00"){
				$this->setXY(5+$ancho_nomb,$y_1);$this->MultiCell($ancho_monto,$alto,$items['conceptos'][$i]['subtotal'],'0','R');
		...
		for($i=0;$i<count($items['conceptos']);$i++){
			if($items['conceptos'][$i]['concepto']['tipo']=="D"&&$items['conceptos'][$i]['subtotal']!="0.00"){
				$this->setXY($ancho+5+$ancho_nomb,$y_2);$this->MultiCell($ancho_monto,$alto,$items['conceptos'][$i]['subtotal'],'0','R');
		...
		for($i=0;$i<count($items['conceptos']);$i++){
			if($items['conceptos'][$i]['concepto']['tipo']=="A"&&$items['conceptos'][$i]['subtotal']!="0.00"){
				$this->setXY(2*$ancho+5+$ancho_nomb,$y_3);$this->MultiCell($ancho_monto,$alto,$items['conceptos'][$i]['subtotal'],'0','R');
