...
class repo extends FPDF
{
	...
	function Header(){
		...
		$this->SetXY(10,20);$this->MultiCell(190,5,$meses[$this->filtros["mes"]]." ".$this->filtros["ano"]."\nDESCUENTOS: P.PARTICULARES, FALTAS, TARDANZAS",'0','C');
		...
		$this->SetXY(105,35);$this->MultiCell(30,5,"PERMISOS PART. (min)",'1','C');
		$this->SetXY(135,35);$this->MultiCell(30,10,"FALTAS (dia)",'1','C');
		$this->SetXY(165,35);$this->MultiCell(30,10,"TARD. (min)",'1','C');
	}		
	function Publicar($items){
		...
		foreach($items as $orga){
			...
			$this->SetXY(15,$y);$this->MultiCell(180,5,$orga["programa"]["nomb"],'0','L');
			$y+=5;
			...
			foreach($orga["trabajadores"] as $trab){
				...
				$tot_1 += $trab["MIN_PERMISO"];	
				$tot_2 += $trab["DIAS_INA"];
				$tot_3 += $trab["MIN_TAR"];
				$t_1 += $trab["MIN_PERMISO"];	
				$t_2 += $trab["DIAS_INA"];
				$t_3 += $trab["MIN_TAR"];
				$this->SetXY(15,$y);$this->MultiCell(150,5,$trab["trabajador"]["appat"]." ".$trab["trabajador"]["apmat"]." ".$trab["trabajador"]["apmat"],'0','L');
				$this->SetXY(105,$y);$this->MultiCell(30,5,number_format($trab["MIN_PERMISO"],2),'0','R');
				$this->SetXY(135,$y);$this->MultiCell(30,5,number_format($trab["DIAS_INA"],2),'0','R');
				$this->SetXY(165,$y);$this->MultiCell(30,5,number_format($trab["MIN_TAR"],2),'0','R');
				$y=$this->GetY();
			}
