...
class repo extends FPDF
{
	function Header(){
		...
		$this->SetXY(5,30);$this->MultiCell(200,5,	"LISTA DE TIPO DE DOCUMENTOS",'0','C');
		
	}	
	
 		
	function Publicar($tido){
		...
		$y=$y+9;
		for($i=0;$i<count($tido);$i++){
			$this->SetFont('Arial','',7);
			
			$this->SetXY(95,$y);$this->MultiCell(35,9,"".$tido[$i]["desc"],'0','C');
			$this->SetXY(130,$y);$this->MultiCell(33,9,"".date('d/m/Y',$tido[$i]["fecreg"]->sec),'0','C');
			$this->SetXY(60,$y);$this->MultiCell(35,9,"".$tido[$i]["docu"],'1','C');
