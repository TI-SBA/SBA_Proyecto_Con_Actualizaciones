<?php
class Model_dd_tido extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->dd_tipos_documentos;
	}
	...
	protected function get_reporte(){
		...
		$data = $this->db->find($filter, array('docu'=>1,'desc'=>1,'fecreg'=>1));
		foreach ($data as $obj) {
		    $this->items[] = $obj;
		}
	...
	protected function get_lista(){
		...
		if(isset($this->params['texto'])){
			if($this->params["texto"]!=''){
				...
				$criteria = array(
					'$or'=>array(
						array('tipo'=>new MongoRegex('/'.$parametro.'/i')),
						array('docu'=>new MongoRegex('/'.$parametro.'/i'))
						)
				);
			}
		}
	...
	protected function delete_tido(){
		global $f;
