<?php
class Model_dd_depu extends Model {
	private $db;
	public $items;
	
	public function __construct() {
		global $f;
		$this->db = $f->datastore->dd_documentos_depurados;
	}
	protected function get_one(){
		global $f;
		$this->items = $this->db->findOne(array('_id'=>$this->params['_id']));
	}
	protected function get_depuracion(){
		global $f;
		$filter = array();
		if(isset($this->params)) $filter = $this->params;
		$data = $this->db->find($filter, array('nomb'=>1,'desc'=>1,'dire'=>1,'ofic'=>1,'docu'=>1,'metr'=>1,'casa'=>1,'femi'=>1));
		foreach ($data as $obj) {
		    $this->items[] = $obj;
		}
	...
	protected function delete_depu(){
		global $f;
