...
class Controller_pr_pres extends Controller {
		...
		if(count($model->items)>0){
			for($i=0;$i<count($model->items);$i++){
					$cursor2=$f->model("pr/pres")->params(array(
						...
						"cod"=>$model->items[$i]["cod"],
						"tipo"=>$f->request->tipo,
						"periodo"=>$f->request->periodo,
						"mes"=>$f->request->mes,
						"programa"=>$f->request->programa,
						"etapa"=>"A",
	...
	function execute_save(){
		...
		if(!isset($f->request->data['_id'])){
			...
			$data['clasificador']['_id'] = new MongoId($data['clasificador']['_id']);
			$data['meta']['_id'] = new MongoId($data['meta']['_id']);
			$data['prog']['_id'] = new MongoId($data['prog']['_id']);
	...
	function execute_save_v1(){
		...
		if(!isset($f->request->data['_id'])){
			...
			$prog=$data['prog']['_id'];
			$model = $f->model("mg/prog")->params(array("_id"=>new MongoId($data['prog']['_id'])))->get("one")->items;
			...
			if(!is_null($model)){
				$data['programa']=array();
				$data['programa']['nomb']=$model['nomb'];
				$data['programa']['_id']=$model['_id'];
			}
