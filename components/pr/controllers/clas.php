...
class Controller_pr_clas extends Controller {
	...
	function execute_all(){
		...
		if(isset($f->request->data['autocomplete'])){
			...
			$filter['cod'] = $regex;
			$model = $f->model('pr/clas')->params(array('fields'=>$fields,'filter'=>$filter))->get('all');
		}
		//$model = $f->model('pr/clas')->params(array('fields'=>$fields))->get('all');
	...
	function execute_lista(){		
		...
		$model = $f->model("pr/clas")->params($params)->get("lista");
