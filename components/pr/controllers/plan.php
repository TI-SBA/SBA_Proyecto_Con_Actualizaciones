...
class Controller_pr_plan extends Controller {
	...
	/*function execute_prog_lista(){
		global $f;
		$model = $f->model("pr/plan")->params(array(
			"page"=>$f->request->page,
	...
	function execute_prog_lista_v1(){
		global $f;
		$model = $f->model("pr/plan")->params(array("page"=>$f->request->page,"page_rows"=>$f->request->page_rows,"etapa"=>"","programa"=>new MongoId($f->request->programa),"periodo"=>$f->request->periodo))->get("lista_v1");
		$f->response->json( $model );
	}
	...
	function execute_ejec_lista_v1(){
		...
		$model = $f->model("pr/plan")->params(array(
			...
			"programa"=>new MongoId($f->request->programa),
			"periodo"=>$f->request->periodo
		))->get("lista_v1");
		$model2 = $f->model("pr/plan")->params(array("periodo"=>$f->request->periodo,"programa"=>new MongoId($f->request->programa),"trimestre"=>$f->request->trimestre))->get("eval_v1");
		$model->eval = $model2->items;
		$f->response->json( $model );
	}
	...
	function execute_ejec_print_v1(){
		global $f;
		$data = $f->request->data;																													//Recepcion del formulario en 
		$response=array(
			'status'=>'error',
		...
		try {
			$model = $f->model("pr/plan")->params(array("page"=>$f->request->page,"page_rows"=>$f->request->page_rows,"etapa"=>$f->request->etapa,"programa"=>new MongoId($f->request->programa),"periodo"=>$f->request->periodo))->get("lista_v1");
			if(is_null($model->items)) throw new Exception("Error: No se encontro planes ejecutados para realizar un informe");
			$model->trim_data = $f->request->trimestre;
			$model->filtros = $f->request->data;
			$f->response->view("pr/planejec.print",$model);
		} catch (Exception $e) {
	...
	*/
	...
	function execute_save_activ_v1(){
		...
		if(!isset($f->request->data['_id'])){
			...
			$data["programa"]["_id"] = new MongoId($data["programa"]["_id"]->{'$id'});
			$data["unidad"]["_id"] = new MongoId($data["unidad"]["_id"]->{'$id'});
			if(isset($data["programa"]["actividad"]["_id"])) $data["programa"]["actividad"]["_id"] = $data["programa"]["actividad"]["_id"]->{'$id'};
			if(isset($data["programa"]["componente"]["_id"])) $data["programa"]["componente"]["_id"] = $data["programa"]["componente"]["_id"]->{'$id'};
			$data['trabajador'] = $f->session->userDB;
			$f->model("pr/plan")->params(array('data'=>$data))->save("insert");
			$f->model('ac/log')->params(array(
				'modulo'=>'PR',
	...
	function execute_save_create_eval_v1(){
		...
		if(!isset($f->request->data['_id'])){
			for($i=1;$i<=4;$i++){
				$data2 = array();
				$data2['fecreg'] = new MongoDate();
				$data2['periodo'] = $f->request->periodo;
				$data2['trimestre'] = "".$i;
				$data2['estado'] = "P";
				$data2['programa']['_id'] = new MongoId($f->request->programa_id);
	...
	function execute_planevac_print_v1(){
		global $f;
		$model = $f->model("pr/plan")->params(array("periodo"=>$f->request->periodo,"programa"=>new MongoId($f->request->programa),"trimestre"=>$f->request->trimestre))->get("eval_v1");
		$model->trim_data = $f->request->trimestre;
		$model->filtros = $f->request->data;
