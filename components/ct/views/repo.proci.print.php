...
class repo extends FPDF
{
	...
	function Publicar($items){
		...
		foreach($items as $item){
			$ppto = array_sum($item["total_ppto"]);
			$total_ppto = $total_ppto + $ppto;
			$ingr = array_sum($item["total_ingr"]);
			$total_ingr = $total_ingr + $ingr;
			$acum = array_sum($item["total_acum"]);
			$total_acum = $total_acum + $acum;
			$sald = array_sum($item["total_sald"]);
			$total_sald = $total_sald + $sald;
			$this->Rect(5, $y, 100, $alto);$this->SetXY(5,$y);$this->MultiCell(100,5,"FUENTE: ".$item["cod"]." ".strtoupper($item["rubro"]),'0','L');	
			$this->Rect(105, $y, 25, $alto);$this->SetXY(105,$y);$this->MultiCell(25,5,number_format($ppto,2,".", ","),'0','R');	
			$this->Rect(130, $y, 25, $alto);$this->SetXY(130,$y);$this->MultiCell(25,5,number_format($ingr,2,".", ","),'0','R');	
			$this->Rect(155, $y, 25, $alto);$this->SetXY(155,$y);$this->MultiCell(25,5,number_format($acum,2,".", ","),'0','R');
			$this->Rect(180, $y, 25, $alto);$this->SetXY(180,$y);$this->MultiCell(25,5,number_format($sald,2,".", ","),'0','R');
			$y=$y+5;
			foreach($item["items"] as $row){
				...
				$ppto = 0;
				for($j=0;$j<count($row["pim"]);$j++){
					$ppto = $ppto = $row["pim"][$j]["importe"];
				}
				$this->Rect(105, $y, 25, $alto);$this->SetXY(105,$y);$this->MultiCell(25,5,number_format($ppto,2,".", ","),'0','R');	
				$this->Rect(130, $y, 25, $alto);$this->SetXY(130,$y);$this->MultiCell(25,5,number_format($row["ingreso"],2,".", ","),'0','R');	
				$this->Rect(155, $y, 25, $alto);$this->SetXY(155,$y);$this->MultiCell(25,5,number_format($row["acumulado"],2,".", ","),'0','R');
				$this->Rect(180, $y, 25, $alto);$this->SetXY(180,$y);$this->MultiCell(25,5,number_format($row["saldo"],2,".", ","),'0','R');
				$y=$y+5;
			}			
		}
