...
class pmen extends FPDF
{
	...
	function Header(){
		...
		for($n=$mes_ini;$n<=$mes_fin;$n++){
			$this->setXY($x_m,42);$this->MultiCell(60,15,strtoupper($meses[$n]),'1','C');
			$x_m+=60;
		}
	...
	function Publicar($items,$meses){
		...
		foreach($items as $fuente){
			...
			foreach($fuente["acti"] as $item){
				...
				for($n=$mes_ini;$n<=$mes_fin;$n++){
					...
					$this->SetXY($x_i,$y);$this->MultiCell(60,5,$suma,'1','R');										
					$x_i += 60;
				}
				if(strlen($item["clasificador"]["cod"])==1){
			...
			for($n=$mes_ini;$n<=$mes_fin;$n++){
				...
				$this->SetXY($x_i,$y);$this->MultiCell(60,5,number_format($total_acti[$n],2),'1','R');										
				$x_i += 60;
			...
			for($n=$mes_ini;$n<=$mes_fin;$n++){
				...
				$this->SetXY($x_i,$y);$this->MultiCell(60,5,number_format($total_proy[$n],2),'1','R');										
				$x_i += 60;
			...
			for($n=$mes_ini;$n<=$mes_fin;$n++){
				...
				$this->SetXY($x_i,$y);$this->MultiCell(60,5,number_format($total_fuen[$n],2),'1','R');										
				$x_i += 60;
		...
		$all_acum_acti = 0;
		for($n=$mes_ini;$n<=$mes_fin;$n++){
			$this->SetXY($x_i,$y);$this->MultiCell(60,5,number_format($all_acti[$n],2),'1','R');										
			$x_i += 60;
		...
		$all_acum_proy = 0;
		for($n=$mes_ini;$n<=$mes_fin;$n++){
			$this->SetXY($x_i,$y);$this->MultiCell(60,5,number_format($all_proy[$n],2),'1','R');										
			$x_i += 60;
		...
		$all_acum_fuen = 0;
		for($n=$mes_ini;$n<=$mes_fin;$n++){
			$this->SetXY($x_i,$y);$this->MultiCell(60,5,number_format($all_fuen[$n],2),'1','R');										
			$x_i += 60;
