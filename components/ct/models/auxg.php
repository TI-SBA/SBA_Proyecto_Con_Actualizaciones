<?php
class Model_ct_auxg extends Model {
	private $col;
	public $items;
	
	public function __construct() {
		global $f;
		$this->col = $f->datastore->ct_auxiliares_gastos;
	}
	...
	protected function get_lista_ot(){
		global $f;
		$fields = array();
		...
		$filter = array(
			...
			'saldo.especifica.cod' => $codigo,
			'clase'=> array('$in'=>array("NC","OC","OS","CP"))
		);
		$order = array('fecreg'=>1);
		$data = $this->col->find($filter,$fields)->sort($order);
		foreach ($data as $obj) {
	...
	protected function get_all(){
		...
		if(isset($this->params["clasificador"]))$filter["saldo.especifica._id"] = new MongoId($this->params["clasificador"]);
		if(isset($this->params["hasta_mes"])) $filter["saldo.periodo.mes"] = array('$lte'=>floatval($this->params["hasta_mes"]));	
		/*if(isset($this->params["meta"])){
			$filter["meta"] = $this->params["meta"];
		}else{
