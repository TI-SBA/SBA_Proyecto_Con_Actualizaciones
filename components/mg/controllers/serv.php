...
class Controller_mg_serv extends Controller {
	...
	function execute_lista_cm(){
		...
		if(isset($f->request->data['_id']))
			$params['_id'] = new MongoId($f->request->data['_id']);
		if(isset($f->request->data['sort']))
			$params['sort'] = array($f->request->data['sort']=>floatval($f->request->data['sort_i']));
		$f->response->json( $f->model("mg/serv")->params($params)->get("lista_cm") );
	}
	...
	function execute_get_orga(){
		...
		//$orga['funcion'] = $f->model("pr/estr")->params(array("_id"=>$orga['subprograma']['funcion']['_id']))->get("one")->items; //comment 17012020 jovad
	...
	function execute_alll_orga(){
		global $f;
		$model = $f->model('mg/serv')->get('alll_orga');
		$f->response->json($model->items);
	}
