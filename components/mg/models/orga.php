...
class Model_mg_orga extends Model {
	...
	protected function get_lista(){
		...
		if(isset($this->params['actividad']))
			$criteria['actividad'] = $this->params['actividad'];
		if(isset($this->params['componente']))
			$criteria['componente'] = $this->params['componente'];
	...
	protected function get_listanodos(){
		...
		if($va==null){
			...
			if($f->request->ide != "0"){
				...
				//Mostrar la Raiz
				foreach ($va as $ob) {
					if(isset($ob['organizaciones']) && $ob['organizaciones'] == null){
						$this->items[$i] = new StdClass;
	...
	/*protected function get_listanodos_legacy(){
		global $f;
		$i = 0;
		$va = $this->orga->find();
		if($va==null){
	...
	/*protected function get_lo_legacy(){
		global $f;
		$i = 0;
		$va = $this->orga->find();
		if($va==null){
	...
	protected function get_lo(){
		...
		if($va==null){
			...
			$data = $this->orga->find();
			foreach ($data as $obj) {
				if(isset($obj['organizaciones'])){
					$d = (array)$obj['organizaciones'];
					$so = (array)$obj['_id'];
					if(!isset($d['padre'])){
						$this->items[$i] = new StdClass;
						$this->items[$i]->data = $obj['nomb'];
						$this->items[$i]->attributes = new StdClass;
						$this->items[$i]->attributes->id = $so['$id'];
						$this->items[$i]->attributes->name = $obj['nomb'];
					    	$this->items[$i]->icons = "images/n1.png";
					    	$i++;
	...
	/*protected function get_nodos_legacy(){
		global $f;
		$i = 0;
		$nodos;
		$criteria = array(
