...
//		$this->Image(IndexPath.DS.'templates/ts/Recibo.jpg',1,1,200,150);	
	...
	function Publicar($items){
		...
		if(isset($items['hospitalizacion'])){
			...
			foreach($items["items"] as $row){
				...
				if(!isset($row['tari'])){
                    ...
                    $this->setXY(160,$y);$this->MultiCell(20,7,number_format($row["costo"],2),'0','R');//150
        ...
        $this->setXY(40,123);$this->MultiCell(10,5,strtoupper(strftime("%d",$items['fecreg']->sec)),'0','L');
        $this->setXY(60,123);$this->MultiCell(40,5,strtoupper($meses[intval(Date::format($items["fecreg"]->sec, 'n'))-1]),'0','L');
        $this->setXY(97,123);$this->MultiCell(10,5,"7",'0','L');
        $this->SetFont('courier','',10);
