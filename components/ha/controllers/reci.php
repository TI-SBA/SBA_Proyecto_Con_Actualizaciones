...
*			Caja en Adicciones				*
*********************************************/
class Controller_ha_reci extends Controller {
	...
	function execute_save(){
		...
		$f->model('ac/log')->params(array(
			'modulo'=>'HA',
			'bandeja'=>'Tarifa de Hospitalizaciones - Adicciones',
			'descr'=>'Se creó el Recibo de Caja de Moises Heresi - Adicciones <b>'.$data['num'].'</b>.'
		))->save('insert');
	...
	function execute_anular(){
		...
		if($comprobante!=null){
			if(isset($comprobante['hospitalizacion'])){
				$hospitalizacion = $f->model('ad/hospi')->params(array('_id'=>$comprobante['hospitalizacion']['_id']))->get('one')->items;
				if($hospitalizacion!=null){
					$f->model('ad/hospi')->params(array('_id'=>$hospitalizacion['_id']),array('$unset'=>array('recibo'=>1,'recibos'=>1)))->save('custom');	
				}
			}
		}
	...
	function execute_edit(){
		global $f;
		$f->response->view("ha/reci.edit");
	}
	function execute_details(){
		global $f;
		$f->response->view("ha/reci.details");
	}
	function execute_tari(){
		global $f;
		$f->response->view("ha/reci.tari");
	}
	function execute_generar(){
		global $f;
		$f->response->view("ha/reci.generar");
	}
	function execute_print(){
        ...
        $comp['paciente'] = $f->model("ad/paci")->params(array("filter"=>array("paciente._id"=>$comp['cliente']['_id'])))->get("all")->items;
		...
		if(isset($f->request->data['xls'])){
			switch ($comp['tipo']) {
				case 'B':
					$f->response->view("ha/comp.bole.xls",array("data"=>$comp));
					break;
				case 'F':
					$f->response->view("ha/comp.fact.xls",array("data"=>$comp));
					break;
				case 'R':
					$f->response->view("ha/comp.reci.xls",array("data"=>$comp));
					break;
			}
		}else{
			switch ($comp['tipo']) {
				case 'B':
					$f->response->view("ha/comp.bole.pdf",array("items"=>$comp,'print'=>$print));
					break;
				case 'F':
					$f->response->view("ha/comp.fact.pdf",array("items"=>$comp,'print'=>$print));
					break;
				case 'R':
					$f->response->view("ha/comp.reci.pdf",array("items"=>$comp,'print'=>$print));
					break;
			}
		}
	}
