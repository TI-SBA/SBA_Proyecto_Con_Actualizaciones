<?php
class Controller_in_repr extends Controller {
	function execute_index() {
		global $f;
		$f->response->print("<div style='height:30px;line-height:30px;'>");
		$f->response->view("ci/ci.search");
		$f->response->print('<button name="btnAgregar">Nuevo Representante Legal</button>');
	...
	function execute_lista(){
		global $f;
		$model = $f->model("mg/entidad")->params(array("roles"=>"representante_legal","page"=>$f->request->page,"page_rows"=>$f->request->page_rows))->get("lista");
	...
	function execute_search(){
		...
		$model = $f->model("mg/entidad")->params(array(
			...
			"rol"=>array('nomb'=>'roles.representante_legal','value'=>array('$exists'=>true))
		))->get("search");
		$f->response->json( $model );
	}
