<?php
class Controller_in_ardario extends Controller {
	function execute_index() {
		global $f;
		$f->response->print("<div style='height:30px;line-height:30px;'>");
		$f->response->view("ci/ci.search");
		$f->response->print('<button name="btnAgregar">Nuevo Arrendatario</button>');
	...
	function execute_get(){
		...
		$arren = $f->model("in/oper")->params(array("arrendatario"=>new MongoId($f->request->data['_id'])))->get("arrenprop");
		$f->response->json( array('enti'=>$enti->items,'arre'=>$arren->items) );
	}
	function execute_lista(){
		global $f;
		$model = $f->model("mg/entidad")->params(array("roles"=>"arrendatario","page"=>$f->request->page,"page_rows"=>$f->request->page_rows))->get("lista");
	...
	function execute_search(){
		...
		$model = $f->model("mg/entidad")->params(array(
			...
			"rol"=>array('nomb'=>'roles.arrendatario','value'=>array('$exists'=>true))
		))->get("search");
		$f->response->json( $model );
	}
